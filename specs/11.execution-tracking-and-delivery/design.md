...
前端实现遵循 `docs/DESIGN.md`：执行进度使用时间线（timeline）组件展示状态变化序列；结果版本标识使用中性 tag（「最新版本」高亮，历史版本弱化但可查看）。任务标题、阶段导航和阶段内容共用 `1280px` 内容轨道。主交付区优先占据该轨道宽度，默认高度接近 `70vh` 并提供全屏入口。任务—Agent 关系图使用 React Flow 只读呈现，节点和连线只能读取正式候选、分配与执行接口；允许缩放、平移和查看候选，但禁止改线或拖动节点改写业务含义。分配前展示候选关系；等待接单或已接单后切换为执行关系，只投影有效 assignment 对应的 Agent，未选、拒绝和取消记录不进入主图。节点使用紧凑尺寸并按候选数量扩展画布高度，由 `fitView` 负责自适应缩放。五个任务阶段使用 ARIA Tab 与单一 `tabpanel`，同一时刻只渲染所选阶段；历史阶段可选择，未来阶段禁用，权威状态推进时自动选择新阶段，但普通轮询不得打断用户正在查看的历史阶段。旧 `/tasks/:id/plan` 地址仅兼容跳回正式任务详情，不再维护浏览器 `sessionStorage` 中的第二套“执行方案”。验收页的结算金额与平台服务费使用等宽数字对齐展示，金额相关文案不使用装饰性字体。返工操作作为次要操作按钮，避免视觉上与「验收」（主要操作）等重。
