...
- [x] T-004: 实现 `ValidateHardConstraints()`（健康状态/准入状态/币种/截止时间/受控上线期单次报价上限；`[v3]` 预算偏好不参与候选资格）与验收模式前置校验 ~30min
...
- [x] T-008: 编写测试：私密任务越权、脱敏字段、模式锁定、自动验收前置条件，以及 `[v3]` 普通 Agent 不因整单预算被隐藏、受控上线期 Agent 自身报价超风险上限仍被排除 ~30min
