...
前端实现遵循 `docs/DESIGN.md`：候选匹配相关的强调色使用 violet（`secondary`，规范中专门保留给「AI、Agent 匹配、推荐」语义，不与托管/成功状态的蓝、绿混用）。候选卡片中的报价、评分、历史完成量等数值右对齐排列以便扫描比较；`[v2]` 新入驻标识（受控上线期）复用 [[3.agent-health-lifecycle]] 定义的样式，不重新设计一套。列表模式需满足移动端和无障碍访问（PRD §8.2 要求）。
