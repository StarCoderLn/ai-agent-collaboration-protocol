...
体验库已迁移至 0050，8 个创始钱包已质押并同步。真实案件的 VRF、付费申诉、终审结算、异常恢复和保证金领取均已闭环；在线新增奖励通知与分页视觉验收也已完成，当前仅真实文件附件仍待跨端验收，详见 [链上仲裁进度](../docs/dao-chain-arbitration.md)。当前制品与状态契约见
...
| 13 | dispute-and-arbitration | 争议、证据、资金冻结、平台/DAO 仲裁、多 Agent 结算或退款 | 6, 11 | V1 T-001～T-012 已验收；Sepolia 恢复、申诉、终审结算、保证金及奖励体验已通过，仅 T-016 仍有外部验收项 |
...
- `[2026-09-10 付费申诉验收]` 1.5 USDC 新任务完成钱包支付 1 USDC 保证金与 0.1 USDC 服务费、三人首审、五人终审、50% 统一结算、8 名仲裁员奖励及保证金领取。链上 `usdcCredit` 最终为 0；T-018/T-020 已完成，T-021 随后完成浏览器视觉验收，仅 T-016 保留任务清单列明的外部验收项。
