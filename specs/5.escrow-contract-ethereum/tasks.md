...
- [x] T-002: 实现 `Escrow` 存储结构与 `deposit()`（含重复托管拒绝，触发 `Deposited`） ~30min

### 功能 3: 结算与退款

- [x] T-003: 实现 `release()`（权限校验、防重入、终态迁移，触发 `Released`；`[v4]` `payee` 收成交额扣手续费后的净额、`feeReceiver` 收手续费、发布者收托管上限与成交额的差额） ~30min
- [x] T-004: 实现 `refund()`（权限校验、防重入、终态迁移，触发 `Refunded`） ~30min
...
- [x] T-006: 编写单元测试：正常路径、权限拒绝、重复操作拒绝、暂停期间操作拒绝、`[v4]` Agent 净额 + 手续费 + 发布者差额退款严格等于托管额，并拒绝成交额超过托管额或手续费超过成交额；非 `TREASURY_ROLE` 无法调用 `setFeeReceiver()` ~30min
- [x] T-007: 编写重入攻击模拟测试（恶意 payee 合约尝试重入 `release`） ~30min
- [x] T-008: 编写测试网部署脚本，输出合约地址与 ABI 供 [[6.escrow-sync-and-wallet]] 使用 ~30min
