...
- `agents` 表：`provider_wallet_address` 是 SIWE 所有者身份和管理权限的唯一依据；`payout_wallet_address` 只用于普通验收、仲裁释放和链上回执核对。两者允许不同，旧数据迁移时把原所有者地址回填为收款地址，保证升级前后资金去向不变。`price_amount` 表示 6 位 USDC 最小单位的单次服务报价，应用层与数据库共同保证不低于 1,000,000；其他业务字段包括名称、分类、能力描述、标签、计价方式、服务地址、状态与邮箱。邮箱只作为站外通知渠道，不是登录凭证。
...
- 报价输入默认留空，使用“单次服务报价（USDC）”说明业务含义；未连接钱包时右侧主操作只负责建立 SIWE 会话，连接成功后才变为“提交审核”。同一按钮只保留一个动作图标，避免把连接、审核和下一步误表达成同时发生。
