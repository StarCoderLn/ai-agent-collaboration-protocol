...
| 2026-08-22 | v6   | 用户确认冻结提供者钱包认证方案（SIWE）与 `web/apps/server` 独立 Next.js API-only 部署形态（design.md v5）；新增 T-009～T-012 承接脚手架、SIWE 认证、路由挂载+事务收敛、`GET /api/agents/:id` 四项此前未被任何 task 承接的实现缺口。任务总数由 8 增至 12，超出常规单 feature 8 项上限（该上限用于 `/yd:prd` 初次拆分，本次是既有 feature 实现阶段发现的必要基础设施缺口补充，非重新拆分，故不新开 feature） |
...
| 2026-09-15 | v21  | T-009 架构纠偏完成：Marketplace API 保留独立部署边界并迁入 better-t-stack Hono；87 个路由复用既有 handler，Lambda 改为 Hono Adapter + esbuild，移除 Next.js API-only/LWA/standalone 运行时；全量业务测试、类型检查、本机构建、Lambda 构建、CDK synth 与真实 `/api/health` 已验证。 |
...
- [x] T-009: 搭建 `web/apps/server` 独立 Hono Marketplace API，生成路由注册表并复用标准 Request/Response handler；补齐 Node 本机入口和 AWS Lambda 部署配置（Hono Adapter + esbuild + CDK，`infra/`） ~30min
...
T-001～T-013 全部完成。87 个 `/api/**` 路由已挂载到 Hono，路径参数由唯一适配器转换为既有 handler 的异步 `params` 契约；创建/编辑/凭证替换的业务写入、审计和幂等提交仍位于同一 PostgreSQL 事务。快速接入连接测试只探测同域 `/healthz`，不触发模型调用。跨源 CORS、SIWE 会话地址归一化和非法 UUID 守卫均保留原有测试。前端请求继续携带 `credentials:"include"`。T-009 的 Hono 本机 bundle、Hono Lambda 单文件制品、CDK synth 与真实 `GET /api/health` 均已验证；没有保留 Next.js API-only、React、LWA 或 standalone 运行依赖。

本地 PostgreSQL 垂直集成已经通过；仍未验证的项目均需要真实 AWS 账号和环境权限，
不属于本 feature 可在本机代替完成的验收：

- 真实 AWS KMS 调用以及 Lambda 连接部署环境 PostgreSQL 的行为。
- 真实 AWS 账号执行 `cdk deploy`（`web/apps/server/infra/README.md` 「已验证 / 未验证」一节列了具体清单：Lambda 冷启动、Function URL 真实可达性、真实浏览器跨源请求链路均未验证）。
