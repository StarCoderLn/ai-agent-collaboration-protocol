...
- 架构类型: 多服务架构
- 涉及层: 业务服务（Hono + AWS Lambda，任务 CRUD 与状态机权威实现）、PostgreSQL、Next.js 前端
...
- 快速发布主表单呈现任务标题、可选补充说明、服务分类和截止日期；可见范围放入折叠高级设置，不展示技能标签、预算或内部任务合同。补充说明为空时用标题填充服务端非空的 `description` 字段，直接作为执行输入，不默认增加收费 PRD。
- `[2026-09-06]` 发布页对明确缺少主题或用途的中英文短指令提示补充，不用模型假装已理解所有自由文本。不展示自由发挥选项；仅提交用户填写的说明，未填写则使用标题，不追加授权文案。未指定呈现细节由 Agent 合理补足，不冒充用户已确认的要求。托管前可展开原始范围复核；平台不自动生成完整 PRD 或新增购买入口。新旧节点输入契约见 `docs/workflow-artifacts.md`。
