...
func (p *HTTPProber) Probe(ctx context.Context, _ string, endpoint, secret string) Observation {
	if p.Client == nil || secret == "" {
	...
	headers, err := protocol.Sign(protocol.SignRequest{Method: http.MethodGet, Path: "/healthz"}, secret, protocol.CallTypeProduction)
	if err != nil {
		return Observation{Result: domain.ProbeAgentInternalError, ResultCode: string(protocol.ErrCodeAgentInternalError)}
	}
