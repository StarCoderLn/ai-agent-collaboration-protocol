...
type TaskTransitionRepository struct{ Pool *pgxpool.Pool }

func (r *TaskTransitionRepository) ClaimDue(ctx context.Context, now time.Time, lease time.Duration, limit int) ([]tasktransition.Event, error) {
	if r.Pool == nil {
		return nil, errors.New("task transition repository requires pool")
	}
	rows, err := r.Pool.Query(ctx, `
		WITH due AS (
			SELECT candidate.id
			  FROM task_transition_outbox candidate
			 WHERE (
				(candidate.status='pending' AND candidate.next_attempt_at <= $1)
				OR (candidate.status='processing' AND candidate.locked_until <= $1)
			 )
			   AND NOT EXISTS (
				 SELECT 1 FROM task_transition_outbox earlier
		...
		UPDATE task_transition_outbox event
		   SET status='processing', locked_until=$3
		  FROM due WHERE event.id=due.id
		RETURNING event.id::text, event.task_id::text, event.assignment_id::text,
		          event.event_type, event.attempt_no`, now, limit, now.Add(lease))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var events []tasktransition.Event
	for rows.Next() {
		var event tasktransition.Event
		if err = rows.Scan(&event.ID, &event.TaskID, &event.AssignmentID, &event.EventType, &event.AttemptNo); err != nil {
			return nil, err
		}
...
func (r *TaskTransitionRepository) MarkDelivered(ctx context.Context, eventID string, deliveredAt time.Time) error {
	...
	if result.RowsAffected() != 1 {
		return errors.New("task transition event is not processing")
	}
	return nil
}

func (r *TaskTransitionRepository) MarkFailure(
	ctx context.Context,
	...
	result, err := r.Pool.Exec(ctx, `
		UPDATE task_transition_outbox event
		 ...
		 WHERE event.id=$1 AND event.status='processing' AND config.id=TRUE`, eventID, errorCode, retryAt, terminal)
	if err != nil {
		return err
	}
	if result.RowsAffected() != 1 {
