...
export async function loadArbitrationSettlementContext(
  ...
  if (terms === undefined) throw new ArbitrationSettlementError("ARBITRATION_SETTLEMENT_NOT_READY", "任务托管或工作流账本尚未就绪");
  ...
  if (lines.rows.length === 0 || lines.rows.some((line) => line.agent_id === null || line.agreed_amount_minor === null)) {
    throw new ArbitrationSettlementError("ARBITRATION_SETTLEMENT_NOT_READY", "工作流报价尚未完整冻结");
  }
  ...
  return {
    ...
    lines: lines.rows.map((line) => ({
      ...
      agreedAmountMinor: BigInt(requiredPresent(line.agreed_amount_minor, "ARBITRATION_AMOUNT_REQUIRED")),
 ...
 */
export function buildArbitrationSettlementPlan(input: Readonly<{
  ...
  responsibility: "agent_at_fault" | "agent_not_at_fault" | "shared" | "not_determined";
  reason: string;
  ...
  if (
    ...
    || input.releaseAmountMinor + input.refundAmountMinor !== context.escrowAmountMinor
    || input.releaseBasisPoints < 0
    || input.releaseBasisPoints > 10_000
  ) throw new ArbitrationSettlementError("ARBITRATION_PAYOUT_NOT_CONSERVED", "仲裁释放额与退款额不守恒");

  const agreedTotal = context.lines.reduce((sum, line) => sum + line.agreedAmountMinor, BigInt(0));
  if (agreedTotal !== context.escrowAmountMinor) {
    throw new ArbitrationSettlementError("ARBITRATION_QUOTE_NOT_CONSERVED", "多 Agent 冻结报价与托管金额不一致");
  }
  const allocated = allocateGross(context.lines, input.releaseAmountMinor, agreedTotal);
  if (allocated.length > 32) {
    throw new ArbitrationSettlementError("ARBITRATION_PAYOUT_COUNT_INVALID", "仲裁分账超过单笔链上结算上限");
  }
  ...
  const platformFeeMinor = payouts.reduce((sum, payout) => sum + BigInt(payout.feeAmountMinor), BigInt(0));
  const decisionHash = digest({
    ...
    authority: input.authority.source === "dao"
  ...
  const settlementManifestHash = payouts.length === 0 ? null : keccak256(coder.encode(
    ["bytes32", "bytes32", "uint256", "tuple(address,uint256,uint256)[]"],
    [decisionHash, textHash(context.workflowRunId), input.releaseAmountMinor,
      payouts.map((payout) => [payout.payee, payout.grossAmountMinor, payout.feeAmountMinor])],
  ));
  return {
    payouts,
...
function allocateGross(
  ...
  return lines.flatMap((line, index) => {
    const gross = index === lines.length - 1
      ? target - assigned
      : target * line.agreedAmountMinor / agreedTotal;
    assigned += gross;
    return gross === BigInt(0) ? [] : [{ payee: line.payoutWalletAddress, grossAmountMinor: gross }];
  });
}
...
function normalizeAddress(value: string): string {
  try {
    ...
    throw new ArbitrationSettlementError("ARBITRATION_PAYOUT_ADDRESS_INVALID", "Agent 收款钱包地址无效");
  }
}
