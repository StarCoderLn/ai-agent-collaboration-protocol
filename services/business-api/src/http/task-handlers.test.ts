...
function deps(overrides: Partial<TaskHttpDeps> = {}): TaskHttpDeps {
	return {
		...
		create: vi.fn(async () => ({
			statusCode: 201,
			body: { taskId: "task-1", status: "draft" },
		})),
		edit: vi.fn(async () => ({
			statusCode: 200,
			body: { taskId: "task-1", status: "draft", statusVersion: "1" },
		})),
		archive: vi.fn(async () => ({
			statusCode: 200,
			body: { taskId: "task-1", archived: true },
		})),
		updateMatchCriteria: vi.fn(async () => ({
			statusCode: 200,
			body: { taskId: "task-1", status: "matching", statusVersion: "2" },
		})),
		submit: vi.fn(async () => ({
			statusCode: 200,
			body: { taskId: "task-1", status: "planning" },
		})),
		preview: vi.fn(async () => ({
			statusCode: 200,
			body: { taskId: "task-1", valid: true },
		})),
		listCategories: vi.fn(async () => ({
			statusCode: 200,
			body: { categories: [] },
		})),
		suggestTags: vi.fn(async (query: string) => ({
			statusCode: 200,
			body: { query, suggestions: [] },
		})),
...
describe("task http handlers", () => {
	...
	it("distinguishes invalid sessions from authentication service failures", async () => {
		const unauthenticated = createTaskHttpHandlers(
			deps({
		...
		const unavailable = createTaskHttpHandlers(
			deps({
				resolveActorId: async () => {
	...
	it("routes authenticated partial draft edits and public taxonomy reads", async () => {
		...
		const editResponse = await handlers.edit(
			new Request(
				"http://api.local/api/tasks/11111111-1111-4111-8111-111111111111",
				{
					method: "PATCH",
					headers: {
						"content-type": "application/json",
		...
		await handlers.tags(
			new Request("http://api.local/api/tags/suggest?q=next"),
		);
		expect(dependencies.suggestTags).toHaveBeenCalledWith("next");
	});

	it("routes soft deletion through the authenticated idempotent archive command", async () => {
		const dependencies = deps();
		const context = {
			params: Promise.resolve({ id: "11111111-1111-4111-8111-111111111111" }),
		};
		const response = await createTaskHttpHandlers(dependencies).archive(
			new Request(
				"http://api.local/api/tasks/11111111-1111-4111-8111-111111111111",
				{
					method: "DELETE",
		...
		expect(dependencies.archive).toHaveBeenCalledWith(
			"11111111-1111-4111-8111-111111111111",
			"publisher",
			"archive-1",
		);
	...
	it("routes the authenticated matching-criteria patch through its narrow command", async () => {
		...
		const response = await createTaskHttpHandlers(
			dependencies,
		).updateMatchCriteria(
			new Request(
				"http://api.local/api/tasks/11111111-1111-4111-8111-111111111111/match-criteria",
				{
					method: "PATCH",
					headers: {
						"content-type": "application/json",
						"idempotency-key": "criteria-1",
					},
	...
	it("maps expected domain errors and hides unexpected internals", async () => {
		const expected = createTaskHttpHandlers(
			deps({
				preview: async () => {
					throw new TaskServiceError(409, "TASK_VERSION_CONFLICT", "请刷新");
				},
			}),
		);
		const unexpected = createTaskHttpHandlers(
			deps({
				preview: async () => {
		...
		const conflict = await expected.preview(
			new Request("http://api.local"),
			context,
		);
		expect(conflict.status).toBe(409);
		const failure = await unexpected.preview(
			new Request("http://api.local"),
			context,
		);
