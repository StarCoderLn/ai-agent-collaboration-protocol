...
describe("workflow execution handlers", () => {
  it("只允许内部服务上报，并把任务、节点、幂等键与请求指纹完整传给事务仓储", async () => {
    const reportStatus = vi.fn(async () => ({
      statusCode: 200,
      body: { taskId: TASK_ID, workflowNodeId: NODE_ID, nodeStatus: "executing" },
    }));
    ...
    const denied = await handlers.reportStatus(new Request("http://api.local", {
      method: "POST",
    ...
    const accepted = await handlers.reportStatus(new Request("http://api.local", {
      method: "POST",
      headers: {
  ...
  it("在进入仓储前拒绝非法节点标识和非法 JSON，避免产生无法归属的执行事实", async () => {
    ...
    const headers = { authorization: "Bearer internal-secret", "content-type": "application/json" };

    const invalidTarget = await handlers.submitResults(new Request("http://api.local", {
      method: "POST", headers, body: "{}",
    }), { params: Promise.resolve({ id: TASK_ID, nodeId: "not-a-node-id" }) });
    expect(invalidTarget.status).toBe(404);

    const invalidJson = await handlers.submitResults(new Request("http://api.local", {
      method: "POST", headers, body: "{",
    }), CONTEXT);
  ...
  it("发布者验收会保留钱包身份、节点归属和幂等键，并返回可携带 Cookie 的跨源响应", async () => {
    const accept = vi.fn(async () => ({ statusCode: 200, body: { nodeStatus: "accepted" } }));
    ...
    const response = await handlers.accept(new Request("http://api.local", {
      method: "POST",
      headers: { "content-type": "application/json", "idempotency-key": "workflow-accept-1" },
    ...
    expect(accept).toHaveBeenCalledWith(TASK_ID, NODE_ID, raw, ACTOR_ID, "workflow-accept-1");
  });
  ...
  it("区分无效会话和认证服务故障，避免把数据库故障误报成用户未登录", async () => {
    const invalidHandlers = createPublisherWorkflowExecutionHandlers({
      ...
      previewAcceptance: vi.fn(), accept: vi.fn(), rework: vi.fn(), allowedOrigin: "http://web.local",
    });
    const unavailableHandlers = createPublisherWorkflowExecutionHandlers({
      resolveActorId: vi.fn(async () => { throw new Error("database unavailable"); }),
      previewAcceptance: vi.fn(), accept: vi.fn(), rework: vi.fn(), allowedOrigin: "http://web.local",
    });

    const invalid = await invalidHandlers.previewAcceptance(
      new Request(`http://api.local?resultId=${RESULT_ID}`), CONTEXT,
    );
    ...
    expect(await unavailable.json()).toMatchObject({ error_code: "AUTH_SERVICE_UNAVAILABLE", retryable: true });
  });
});
