...
export function createWorkflowFeedbackHandlers(deps: WorkflowFeedbackHttpDeps) {
	return {
		submit: async (request: Request, context: WorkflowFeedbackRouteContext): Promise<Response> => {
			...
			if (target === null || target.nodeId === null) {
				return failure(deps, 404, "WORKFLOW_NODE_NOT_FOUND", "任务阶段不存在", false);
			}
			let raw: unknown;
			try {
		...
		list: async (request: Request, context: WorkflowFeedbackRouteContext): Promise<Response> => {
			const actor = await resolveActor(request, deps);
			if (actor instanceof Response) return actor;
			const target = await routeTarget(context, false);
			if (target === null) return failure(deps, 404, "TASK_NOT_FOUND", "任务不存在", false);
			try {
...
async function routeTarget(
	...
	if (!isUuid(id) || (requireNode && (nodeId === undefined || !isUuid(nodeId)))) return null;
...
function serviceFailure(deps: WorkflowFeedbackHttpDeps, error: unknown): Response {
	return error instanceof WorkflowFeedbackServiceError
		? failure(deps, error.statusCode, error.code, error.message, false)
		: failure(deps, 500, "WORKFLOW_FEEDBACK_INTERNAL_ERROR", "反馈服务暂时不可用", true);
}

function result(deps: WorkflowFeedbackHttpDeps, value: WorkflowFeedbackResult): Response {
	return withCredentialedCors(Response.json(value.body, { status: value.statusCode }), deps.allowedOrigin);
}
