...
import { AgentConnectionTestError, type AgentConnectionProbe } from "../agents/agent-connection-test";
import {
  createAgentConnectionTestHandler,
  type AgentConnectionTestHttpDeps,
} from "./agent-connection-test-handler";

function createDeps(overrides: Partial<AgentConnectionTestHttpDeps> = {}): AgentConnectionTestHttpDeps {
  return {
    resolveActorId: vi.fn(async () => "0x1234567890123456789012345678901234567890"),
    probe: {
...
describe("createAgentConnectionTestHandler", () => {
  it("拒绝没有有效钱包会话的连接测试", async () => {
    const probe: AgentConnectionProbe = { probe: vi.fn(async () => ({ latencyMs: 1 })) };
    const handler = createAgentConnectionTestHandler(createDeps({
      resolveActorId: vi.fn(async () => { throw new SessionInvalidError(); }),
      probe,
    }));

    const response = await handler(request({ serviceEndpoint: "https://agent.example.com/run" }));
  ...
  it("把认证服务故障与未登录区分为可重试的 503", async () => {
    const handler = createAgentConnectionTestHandler(createDeps({
      resolveActorId: vi.fn(async () => { throw new Error("session database unavailable"); }),
    }));
  ...
  it("公开 Agent 无访问密钥时也能通过连接测试", async () => {
    ...
    await expect(response.json()).resolves.toEqual({ status: "connected", latencyMs: 18 });
    expect(deps.probe.probe).toHaveBeenCalledWith({ serviceEndpoint: "https://agent.example.com/run" });
  });
  ...
  it("保留探测器给出的鉴权失败语义，便于表单精确提示", async () => {
    const handler = createAgentConnectionTestHandler(createDeps({
      probe: {
        probe: vi.fn(async () => {
          throw new AgentConnectionTestError(422, "AGENT_AUTH_FAILED", "访问密钥无效，请检查后重试", false);
        }),
      },
    }));
