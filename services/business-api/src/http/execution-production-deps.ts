...
import type { InternalExecutionHttpDeps, PublisherExecutionHttpDeps } from "./execution-handlers";
...
export function createProductionInternalExecutionDeps(): InternalExecutionHttpDeps {
  ...
  return {
    ...
    reportStatus: (taskId, raw, key, fingerprint) => withTransaction(pool, (client) =>
      createExecutionService(new PgExecutionRepository(client)).reportStatus(taskId, raw, key, fingerprint)),
    submitResults: (taskId, raw, key, fingerprint) => withTransaction(pool, (client) =>
      createExecutionService(new PgExecutionRepository(client)).submitResults(taskId, raw, key, fingerprint)),
  };
}
...
export function createProductionPublisherExecutionDeps(
  ...
  const pool = getSharedPgPool();
  const reads = createExecutionService(new PgExecutionRepository(asQueryExecutor(pool)));
  return {
    resolveActorId,
    ...
    accept: (taskId, raw, actorId, key) => withTransaction(pool, (client) =>
      createExecutionService(new PgExecutionRepository(client), new Idempotency(new PgIdempotencyStore(client)))
        .accept(taskId, raw, actorId, key)),
    rework: (taskId, raw, actorId, key) => withTransaction(pool, (client) =>
      createExecutionService(new PgExecutionRepository(client), new Idempotency(new PgIdempotencyStore(client)))
