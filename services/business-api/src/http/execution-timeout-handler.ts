...
const inputSchema = z.object({ limit: z.number().int().min(1).max(500).default(100) }).strict();
...
export function createExecutionTimeoutHandler(deps: ExecutionTimeoutHttpDeps) {
  return async (request: Request): Promise<Response> => {
    ...
    if (!validBearer(request.headers.get("authorization"), deps.internalToken)) return failure(401, "UNAUTHENTICATED", false);
    let raw: unknown;
    ...
    try {
      ...
      return Response.json({ scannedAt: new Date().toISOString(), timedOutCount: taskIds.length, taskIds });
    } catch {
...
function failure(status: number, code: string, retryable: boolean): Response {
  return Response.json({ error_code: code, message: "执行超时扫描失败", retryable }, { status });
}
