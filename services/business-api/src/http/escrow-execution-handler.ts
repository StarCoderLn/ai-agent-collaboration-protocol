...
export function createEscrowExecutionHandler(deps: EscrowExecutionHttpDeps) {
  return async (request: Request): Promise<Response> => {
    ...
    try { return Response.json({ ...(await deps.run()), ranAt: new Date().toISOString() }); }
    catch { return failure(500, "ESCROW_EXECUTION_WORKER_FAILED", true); }
  };
}
...
function failure(status: number, code: string, retryable: boolean): Response {
  return Response.json({ error_code: code, message: "链上资金执行失败", retryable }, { status });
}
