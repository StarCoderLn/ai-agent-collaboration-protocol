...
/**
 * 从 Business API 发起连接测试会形成 SSRF 边界。生产环境只允许公网 HTTPS，且 DNS
...
export class HttpAgentConnectionProbe implements AgentConnectionProbe {
  ...
  async probe(input: AgentConnectionProbeInput): Promise<AgentConnectionProbeResult> {
    const endpoint = new URL(input.serviceEndpoint);
    if (endpoint.username !== "" || endpoint.password !== "") {
      throw new AgentConnectionTestError(422, "AGENT_ENDPOINT_INVALID", "Agent 执行地址不能包含用户名或密码", false);
    }
    if (!this.allowPrivateNetwork && endpoint.protocol !== "https:") {
      throw new AgentConnectionTestError(422, "AGENT_ENDPOINT_HTTPS_REQUIRED", "正式环境中的 Agent 执行地址必须使用 HTTPS", false);
    }
    if (endpoint.protocol !== "http:" && endpoint.protocol !== "https:") {
      throw new AgentConnectionTestError(422, "AGENT_ENDPOINT_INVALID", "Agent 执行地址必须使用 HTTP(S)", false);
    }

    const addresses = await lookup(endpoint.hostname, { all: true, verbatim: true });
    const allowedAddress = addresses.find((entry) => this.allowPrivateNetwork || isPublicAddress(entry.address));
    if (allowedAddress === undefined) {
      throw new AgentConnectionTestError(422, "AGENT_ENDPOINT_PRIVATE", "Agent 执行地址不能指向平台内部网络", false);
    }
    ...
    try {
      ...
      throw new AgentConnectionTestError(422, "AGENT_HEALTH_INVALID", "Agent 健康检查没有返回有效 JSON", false);
    }
    const status = typeof payload === "object" && payload !== null && "status" in payload
      ? (payload as { status?: unknown }).status
      : undefined;
    if (status !== "ok" && status !== "up") {
      throw new AgentConnectionTestError(422, "AGENT_HEALTH_INVALID", "Agent 健康检查需要返回 status: ok 或 up", false);
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    const options: RequestOptions = {
      ...
      headers: {
        ...
        ...(credentialSecret === undefined ? {} : { authorization: `Bearer ${credentialSecret}` }),
      },
    };
    ...
    const request = transport(options, (response) => {
      if (response.statusCode === 401 || response.statusCode === 403) {
        response.resume();
        reject(new AgentConnectionTestError(422, "AGENT_AUTH_FAILED", "访问密钥无效，请检查后重试", false));
        return;
      }
      if ((response.statusCode ?? 500) < 200 || (response.statusCode ?? 500) >= 300) {
        response.resume();
        reject(new AgentConnectionTestError(422, "AGENT_HEALTH_UNAVAILABLE", "Agent 的 /healthz 当前不可用", true));
        return;
      }
      const chunks: Buffer[] = [];
      let size = 0;
    ...
    request.setTimeout(TIMEOUT_MS, () => request.destroy(new Error("connection test timeout")));
