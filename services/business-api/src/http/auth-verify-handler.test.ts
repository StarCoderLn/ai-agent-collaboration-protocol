...
describe("createAuthVerifyHttpHandler", () => {
  it("sets a Set-Cookie session header and returns the walletAddress on success", async () => {
    ...
    const response = await handler(makeRequest({ message: preparedMessage, signature }));
  ...
  it("returns a generic 401 without leaking which check failed, and does not set a cookie", async () => {
    const nonceStore: NonceStore = { issue: vi.fn(), consume: vi.fn(async () => true) };
    ...
    const response = await handler(makeRequest({ message: "not a siwe message", signature: "0xdead" }));
