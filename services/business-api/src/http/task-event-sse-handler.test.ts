...
import { createTaskEventSseHandler, type TaskEventSseDeps } from "./task-event-sse-handler";
...
function dependencies(): TaskEventSseDeps {
  return {
    ...
    after: vi.fn(async (_taskId, cursor) => cursor < 11n ? [{
      id: "11", taskId: TASK_ID, statusVersion: "4", eventType: "task.execution_progress",
      payload: { progress: 60 }, createdAt: new Date("2026-08-23T00:00:00.000Z"),
    }] : []),
