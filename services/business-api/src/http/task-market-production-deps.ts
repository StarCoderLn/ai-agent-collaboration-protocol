...
import { getMarketStats, getPublisherTaskStats, getTaskDetail, listPublisherTasks, listTaskMarket } from "../tasks/task-market-service";
...
export function createProductionTaskMarketDeps(resolveActorId: TaskMarketHttpDeps["resolveActorId"]): TaskMarketHttpDeps {
  ...
  return {
    ...
    updateModes: (taskId, rawInput, actorId, idempotencyKey) => withTransaction(pool, (client) =>
      updateOwnedTaskModeSettings(taskId, rawInput, actorId, idempotencyKey, createTaskCommandDeps(client))),
    marketStats: () => getMarketStats(repository),
    publisherStats: (actorId) => getPublisherTaskStats(actorId, repository),
    publisherTasks: (actorId, url) => listPublisherTasks(actorId, url, repository),
  };
}
