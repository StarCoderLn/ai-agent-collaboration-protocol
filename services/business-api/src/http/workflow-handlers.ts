...
import { WorkflowCapabilityError, type WorkflowCapabilityResult } from "../workflows/workflow-capabilities";
...
import { WorkflowPreferenceError, type WorkflowPreferenceResult } from "../workflows/workflow-preferences";
import { withCredentialedCors } from "./cors";

export type WorkflowRouteContext = Readonly<{ params: Promise<{ id: string; nodeId?: string }> }>;
...
export interface WorkflowHttpDeps {
  ...
  updateBudgetPreference(input: Readonly<{
    taskId: string;
    actorId: string;
    rawInput: unknown;
    idempotencyKey: string | undefined;
  }>): Promise<WorkflowPreferenceResult>;
  updateNodeCapabilities(input: Readonly<{
    taskId: string;
    nodeId: string;
    actorId: string;
    rawInput: unknown;
...
export function createWorkflowHandlers(deps: WorkflowHttpDeps) {
  return {
    ...
    async updatePreference(request: Request, context: WorkflowRouteContext): Promise<Response> {
      let actorId: string;
      try {
        actorId = await deps.resolveActorId(request);
      } catch (error) {
      ...
      try {
        ...
        return response(deps, 400, "INVALID_JSON", "请求正文不是有效 JSON", false);
      }
      try {
        ...
        return withCredentialedCors(Response.json(result.body, {
        ...
        return response(deps, 503, "WORKFLOW_SERVICE_UNAVAILABLE", "预算偏好暂时无法保存", true);
      }
    },
    /** 能力修正只开放给仍在选择 Agent 的节点；服务端命令负责所有权和状态锁定。 */
    async updateNodeCapabilities(request: Request, context: WorkflowRouteContext): Promise<Response> {
      let actorId: string;
      try {
        actorId = await deps.resolveActorId(request);
      } catch (error) {
      ...
      if (!isUuid(id) || nodeId === undefined || !isUuid(nodeId)) {
        return response(deps, 404, "WORKFLOW_NODE_NOT_FOUND", "工作节点不存在或无权访问", false);
      }
      let rawInput: unknown;
      try {
        rawInput = await request.json();
      ...
      try {
        ...
        return response(deps, 503, "WORKFLOW_SERVICE_UNAVAILABLE", "能力需求暂时无法保存", true);
      }
    },
  };
}
