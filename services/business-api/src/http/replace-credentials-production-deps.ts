...
import { PgAgentRepository, PgAgentCredentialStore } from "../agents/agent-repository";
...
> {
  ...
  const credentialEncryptor = new EnvelopeEncryptor({ kmsKeyId: getRequiredEnv(KMS_KEY_ID_ENV_VAR) });
  const config = loadSiweConfigFromEnv();

  return {
    runInTransaction: (fn) =>
      withTransaction(pool, (client) => fn(buildTransactionalDeps(client, credentialEncryptor))),
