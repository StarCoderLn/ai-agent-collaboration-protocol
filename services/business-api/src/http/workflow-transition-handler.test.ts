...
describe("workflow transition handler", () => {
  it("rejects browser calls and preserves task plus node identity", async () => {
    ...
    const handler = createWorkflowTransitionHandler({ internalToken: "internal-secret", apply });
    const context = { params: Promise.resolve({ id: TASK_ID, nodeId: NODE_ID }) };

    const denied = await handler(new Request("http://api.local", { method: "POST", body: "{}" }), context);
    expect(denied.status).toBe(401);

    const accepted = await handler(new Request("http://api.local", {
      method: "POST",
