...
function getService(): EscrowService {
  ...
  const config: EscrowRuntimeConfig = {
    requiredConfirmations: resolveEscrowRequiredConfirmations({
      ...
      configuredConfirmations: getOptionalEnv("ESCROW_REQUIRED_CONFIRMATIONS", ""),
    }),
    startBlock: nonNegativeBigint("ESCROW_START_BLOCK", getOptionalEnv("ESCROW_START_BLOCK", "0")),
    maxBlockSpan: positiveBigint("ESCROW_MAX_BLOCK_SPAN", getOptionalEnv("ESCROW_MAX_BLOCK_SPAN", "500")),
    leaseMs: positiveInteger("ESCROW_CURSOR_LEASE_MS", getOptionalEnv("ESCROW_CURSOR_LEASE_MS", "60000")),
    eventBatchSize: positiveInteger("ESCROW_EVENT_BATCH_SIZE", getOptionalEnv("ESCROW_EVENT_BATCH_SIZE", "500")),
    recheckBatchSize: positiveInteger("ESCROW_RECHECK_BATCH_SIZE", getOptionalEnv("ESCROW_RECHECK_BATCH_SIZE", "100")),
    reconciliationBatchSize: positiveInteger("ESCROW_RECONCILIATION_BATCH_SIZE", getOptionalEnv("ESCROW_RECONCILIATION_BATCH_SIZE", "100")),
  };
  service = new EscrowService(new PgEscrowRepository(getSharedPgPool()), client, config);
  return service;
}

export function createProductionPublisherEscrowDeps(resolveActorId: PublisherEscrowHttpDeps["resolveActorId"]): PublisherEscrowHttpDeps {
...
export function createProductionInternalEscrowDeps(): InternalEscrowHttpDeps {
  const escrow = getService();
  return { internalToken: getRequiredEnv("DISPATCH_INTERNAL_TOKEN"), run: escrow.runWorker.bind(escrow) };
}
