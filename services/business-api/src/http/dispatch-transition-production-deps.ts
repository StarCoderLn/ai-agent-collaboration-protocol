...
/** 延迟到请求阶段调用，避免 Next build 在没有运行时环境变量时失败。 */
export function createProductionDispatchTransitionDeps(): DispatchTransitionHttpDeps {
  ...
  return {
    ...
    apply: (taskId, rawInput) => withTransaction(pool, (client) =>
      applyDispatchTransition(taskId, rawInput, new PgDispatchTransitionRepository(client))),
  };
}
