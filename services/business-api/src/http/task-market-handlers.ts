...
export type TaskMarketRouteContext = Readonly<{ params: Promise<{ id: string }> }>;
...
export interface TaskMarketHttpDeps {
  ...
  updateModes(taskId: string, rawInput: unknown, actorId: string, idempotencyKey: string | undefined): Promise<TaskServiceResult>;
...
export function createTaskMarketHttpHandlers(deps: TaskMarketHttpDeps) {
  return {
    market: (request: Request): Promise<Response> => run(deps, () => deps.listMarket(request.url)),
    detail: async (request: Request, context: TaskMarketRouteContext): Promise<Response> => {
    ...
    updateModes: async (request: Request, context: TaskMarketRouteContext): Promise<Response> => {
      ...
      return run(deps, () => deps.updateModes(id, rawInput, actor, request.headers.get("idempotency-key") ?? undefined));
    },
...
async function resolveOptionalActor(request: Request, deps: TaskMarketHttpDeps): Promise<string | null | Response> {
...
async function resolveRequiredActor(request: Request, deps: TaskMarketHttpDeps): Promise<string | Response> {
  const actor = await resolveOptionalActor(request, deps);
  if (actor === null) return response(deps, 401, errorBody("UNAUTHENTICATED", "身份认证失败", false));
  return actor;
}

async function run(deps: TaskMarketHttpDeps, action: () => Promise<TaskServiceResult>): Promise<Response> {
  try {
...
function response(deps: TaskMarketHttpDeps, status: number, body: unknown): Response {
