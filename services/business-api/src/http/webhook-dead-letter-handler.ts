...
export interface WebhookDeadLetterHttpDeps {
  ...
  list(taskId: string | null, limit: number): Promise<readonly WebhookDeadLetterView[]>;
}

/** 内部运营入口不启用浏览器 CORS，也不返回完整 Agent endpoint。 */
export function createWebhookDeadLetterHandler(deps: WebhookDeadLetterHttpDeps) {
  return async (request: Request): Promise<Response> => {
    ...
    try {
      const deliveries = await deps.list(parsed.data.taskId ?? null, parsed.data.limit);
      return Response.json({ deliveries, count: deliveries.length }, { status: 200 });
    } catch {
...
function failure(status: number, code: string, retryable: boolean): Response {
  return Response.json({ error_code: code, message: "Webhook 死信查询失败", retryable }, { status });
}
