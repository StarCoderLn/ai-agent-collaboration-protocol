...
export interface TaskEventSseDeps {
  ...
  after(taskId: string, cursor: bigint, limit: number): Promise<readonly StoredTaskEvent[]>;
...
export function createTaskEventSseHandler(deps: TaskEventSseDeps) {
  return async (request: Request, context: ExecutionRouteContext): Promise<Response> => {
    let actorId: string;
    try { actorId = await deps.resolveActorId(request); }
    catch (cause) {
      return response(deps, cause instanceof SessionInvalidError ? 401 : 503, {
        error_code: cause instanceof SessionInvalidError ? "UNAUTHENTICATED" : "AUTH_SERVICE_UNAVAILABLE",
        message: "无法建立任务事件连接",
    ...
    if (!isUuid(id) || !await deps.canRead(id, actorId)) {
      return response(deps, 404, { error_code: "TASK_NOT_FOUND", message: "任务不存在或无权访问", retryable: false });
    }
    const cursor = parseCursor(request.headers.get("last-event-id"));
    if (cursor === null) return response(deps, 400, { error_code: "INVALID_EVENT_CURSOR", message: "Last-Event-ID 格式不正确", retryable: false });
    const stream = eventStream(id, cursor, request.signal, deps);
    return withCredentialedCors(new Response(stream, {
      status: 200,
      headers: {
...
function eventStream(taskId: string, initialCursor: bigint, signal: AbortSignal, deps: TaskEventSseDeps): ReadableStream<Uint8Array> {
...
function response(deps: TaskEventSseDeps, status: number, body: unknown): Response {
