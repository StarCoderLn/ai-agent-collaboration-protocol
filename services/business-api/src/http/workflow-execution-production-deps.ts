...
import { PgWorkflowExecutionRepository, WorkflowExecutionError } from "../workflows/workflow-execution";
import type { InternalWorkflowExecutionDeps, PublisherWorkflowExecutionDeps } from "./workflow-execution-handlers";
...
export function createProductionInternalWorkflowExecutionDeps(): InternalWorkflowExecutionDeps {
  ...
  return {
    ...
    reportStatus: (taskId, nodeId, raw, key, fingerprint) => withTransaction(pool, (client) =>
      new PgWorkflowExecutionRepository(client).reportStatus(taskId, nodeId, raw, key, fingerprint)),
    submitResults: (taskId, nodeId, raw, key, fingerprint) => withTransaction(pool, (client) =>
      new PgWorkflowExecutionRepository(client).submitResults(taskId, nodeId, raw, key, fingerprint)),
  };
}
...
export function createProductionPublisherWorkflowExecutionDeps(
  ...
  return {
    ...
    accept: (taskId, nodeId, raw, actorId, key) => withTransaction(pool, async (client) => {
      return idempotentPublisherCommand(
        client,
        key,
        `workflow.accept:${taskId}:${nodeId}:${actorId.toLocaleLowerCase()}`,
        () => new PgWorkflowExecutionRepository(client).accept(taskId, nodeId, raw, actorId),
      );
    }),
    rework: (taskId, nodeId, raw, actorId, key) => withTransaction(pool, async (client) => {
      return idempotentPublisherCommand(
        client,
        key,
        `workflow.rework:${taskId}:${nodeId}:${actorId.toLocaleLowerCase()}`,
        () => new PgWorkflowExecutionRepository(client).requestRework(taskId, nodeId, raw, actorId),
      );
    }),
  };
}
...
async function idempotentPublisherCommand(
  db: Parameters<typeof withTransaction>[1] extends (client: infer Client) => unknown ? Client : never,
  ...
  if (reservation.existing !== null) {
    ...
    return { statusCode: reservation.existing.statusCode, body: reservation.existing.body };
  }
  if (!reservation.reserved) throw new WorkflowExecutionError(409, "REQUEST_IN_PROGRESS", "相同请求正在处理中，请稍后查询", true);
