    ...
    service: {
      open: async (taskId, raw, actorId, key) => {
        ...
        if (runtime !== null) {
          ...
          const intent = await pool.query<{ contract_address: string }>("SELECT contract_address FROM escrow_intents WHERE task_id=$1", [taskId]);
        ...
        const chainCases = runtime === null ? undefined : {
        ...
        return withTransaction(pool, (client) => createDisputeService(
          new PgDisputeRepository(client, chainCases), new Idempotency(new PgIdempotencyStore(client)),
