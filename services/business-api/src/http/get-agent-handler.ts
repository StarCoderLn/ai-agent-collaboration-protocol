...
import { getAgent, type GetAgentDeps } from "../agents/get-agent";
...
export function createGetAgentHttpHandler(deps: GetAgentHttpDeps) {
  return async function handleGetAgent(
    ...
    try {
      ...
      return withCredentialedCors(jsonResponse(200, toAgentJson(agent)), deps.allowedOrigin);
    } catch (err) {
...
function toAgentJson(agent: Awaited<ReturnType<typeof getAgent>>): Record<string, unknown> {
