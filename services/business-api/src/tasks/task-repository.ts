...
export type TaskCreationContext = Readonly<{
  ...
  attachmentLimit: Readonly<{ maxFiles: number; maxFileSizeBytes: bigint; allowedMimeTypes: ReadonlySet<string> }>;
  ...
  feeConfig: Readonly<{ version: string; feeBasisPoints: bigint; gasFallbackMinor: bigint }>;
}>;

export interface TaskRepository {
  loadCreationContext(categoryId: string): Promise<TaskCreationContext | null>;
  createDraft(publisherId: string, input: Omit<StoredTask, "id" | "publisherId" | "status" | "statusVersion" | "createdAt" | "updatedAt">): Promise<StoredTask>;
  findOwned(taskId: string, publisherId: string): Promise<StoredTask | null>;
  updateDraft(taskId: string, publisherId: string, expectedVersion: bigint, input: Omit<StoredTask, "id" | "publisherId" | "status" | "statusVersion" | "createdAt" | "updatedAt">): Promise<StoredTask | null>;
  updateModeSettings(
    taskId: string,
    publisherId: string,
    expectedVersion: bigint,
    settings: Readonly<{ visibility: "public" | "private"; assignmentMode: AssignmentModeConfig; acceptanceMode: AcceptanceModeConfig }>,
  ): Promise<StoredTask | null>;
  updateMatchCriteria(
    taskId: string,
    publisherId: string,
    expectedVersion: bigint,
    input: Readonly<{ categoryId: string; categoryVersion: number; tags: readonly string[]; deadline: Date }>,
  ): Promise<StoredTask | null>;
  submit(taskId: string, publisherId: string, expectedVersion: bigint, canonicalTags: readonly string[], categoryVersion: number): Promise<StoredTask | null>;
}

export type TaskCategory = Readonly<{ id: string; parentId: string | null; name: string; slug: string; version: number }>;
export type TaskTagSuggestion = Readonly<{ canonicalName: string; matchedAlias: string | null }>;

export interface TaskTaxonomyRepository {
  listCategories(): Promise<readonly TaskCategory[]>;
  suggestTags(query: string, limit: number): Promise<readonly TaskTagSuggestion[]>;
}
...
export interface TaskReadRepository {
  ...
  listPublicMarket(filters: TaskMarketFilters): Promise<readonly StoredTask[]>;
  /** 发布者工作台读取包含草稿和私密任务；调用方必须先从可信会话取得 publisherId。 */
  listOwnedTasks(publisherId: string, limit: number, offset: number): Promise<readonly StoredTask[]>;
  readMarketStats(): Promise<Readonly<Record<string, number>>>;
  readPublisherStats(publisherId: string): Promise<Readonly<Record<string, number>>>;
}
...
interface TagRow { canonical_name: string; synonyms: string[]; forbidden: boolean }
...
interface TaskRow {
  ...
  attachments: Array<{ name: string; mimeType: string; sizeBytes: string; storageRef: string }>;
  visibility: "public" | "private";
...
const DEFAULT_ATTACHMENT_LIMIT = {
  ...
  allowedMimeTypes: new Set(["application/pdf", "image/png", "image/jpeg", "text/plain"]),
} as const;

/** PostgreSQL 仓储隐藏 JSON/BigInt 编码和多表配置查询，领域服务只看到可信类型。 */
export class PgTaskRepository implements TaskRepository, TaskTaxonomyRepository, TaskReadRepository {
  constructor(private readonly db: QueryExecutor) {}

  async loadCreationContext(categoryId: string): Promise<TaskCreationContext | null> {
    ...
    const attachmentLimit = row.max_file_size_bytes === null || row.allowed_mime_types === null || row.max_files === null
      ? DEFAULT_ATTACHMENT_LIMIT
      : {
        maxFiles: row.max_files,
        maxFileSizeBytes: BigInt(row.max_file_size_bytes),
        allowedMimeTypes: new Set(row.allowed_mime_types.map((mime) => mime.toLocaleLowerCase())),
      };
    return {
  ...
  async createDraft(
    publisherId: string,
    input: Omit<StoredTask, "id" | "publisherId" | "status" | "statusVersion" | "createdAt" | "updatedAt">,
  ): Promise<StoredTask> {
    const [budgetMin, budgetMax, pricingType] = pricingColumns(input.draft.pricing);
    const result = await this.db.query<TaskRow>(
      ...
      [
        publisherId, input.draft.title, input.draft.description, input.draft.acceptanceCriteria,
        input.draft.deliverableFormat, input.draft.categoryId, input.categoryVersion, input.draft.tags,
        pricingType, budgetMin?.toString() ?? null, budgetMax?.toString() ?? null, input.draft.currency, input.draft.deadline,
        input.draft.requiredCapability, JSON.stringify(input.draft.attachments.map((attachment) => ({ ...attachment, sizeBytes: attachment.sizeBytes.toString() }))),
        input.visibility, JSON.stringify(serializeAssignmentMode(input.assignmentMode)), input.acceptanceMode.mode,
        JSON.stringify(input.acceptanceMode.mode === "automatic" ? { acceptorId: input.acceptanceMode.acceptorId, ruleVersion: input.acceptanceMode.ruleVersion } : {}),
      ],
    );
  ...
  async findOwned(taskId: string, publisherId: string): Promise<StoredTask | null> {
    const result = await this.db.query<TaskRow>(
      `SELECT * FROM tasks WHERE id = $1 AND lower(publisher_id) = lower($2)`,
      [taskId, publisherId],
    );
  ...
  async updateDraft(
    ...
    const result = await this.db.query<TaskRow>(
      `UPDATE tasks SET
       ...
       RETURNING *`,
      [
        taskId, publisherId, expectedVersion.toString(), input.draft.title, input.draft.description,
        input.draft.acceptanceCriteria, input.draft.deliverableFormat, input.draft.categoryId,
        input.categoryVersion, input.draft.tags, pricingType, budgetMin?.toString() ?? null,
        budgetMax?.toString() ?? null, input.draft.currency, input.draft.deadline,
        input.draft.requiredCapability,
        JSON.stringify(input.draft.attachments.map((attachment) => ({ ...attachment, sizeBytes: attachment.sizeBytes.toString() }))),
        input.visibility, JSON.stringify(serializeAssignmentMode(input.assignmentMode)), input.acceptanceMode.mode,
        JSON.stringify(input.acceptanceMode.mode === "automatic"
          ? { acceptorId: input.acceptanceMode.acceptorId, ruleVersion: input.acceptanceMode.ruleVersion }
          : {}),
      ],
    );
    return result.rows[0] === undefined ? null : mapTaskRow(result.rows[0]);
  ...
  async updateModeSettings(
    ...
    const result = await this.db.query<TaskRow>(
      ...
      [
        ...
        JSON.stringify(settings.acceptanceMode.mode === "automatic"
          ? { acceptorId: settings.acceptanceMode.acceptorId, ruleVersion: settings.acceptanceMode.ruleVersion }
          : {}),
      ],
    );
    return result.rows[0] === undefined ? null : mapTaskRow(result.rows[0]);
  ...
  async updateMatchCriteria(
    ...
    const result = await this.db.query<TaskRow>(
      ...
      RETURNING *`,
      [taskId, publisherId, expectedVersion.toString(), input.categoryId, input.categoryVersion, input.tags, input.deadline],
    );
    return result.rows[0] === undefined ? null : mapTaskRow(result.rows[0]);
  }
  ...
  async submit(taskId: string, publisherId: string, expectedVersion: bigint, canonicalTags: readonly string[], categoryVersion: number): Promise<StoredTask | null> {
    const result = await this.db.query<TaskRow>(
      `UPDATE tasks
          SET status = 'awaiting_escrow', status_version = status_version + 1,
              tag_names = $4, category_version = $5
        WHERE id = $1 AND lower(publisher_id) = lower($2)
          AND status = 'draft' AND status_version = $3
      RETURNING *`,
      [taskId, publisherId, expectedVersion.toString(), canonicalTags, categoryVersion],
    );
    return result.rows[0] === undefined ? null : mapTaskRow(result.rows[0]);
  }

  async listCategories(): Promise<readonly TaskCategory[]> {
    const result = await this.db.query<{ id: string; parent_id: string | null; name: string; slug: string; version: number }>(
      `SELECT id, parent_id, name, slug, version
         FROM categories WHERE active = TRUE
        ORDER BY parent_id NULLS FIRST, name, id`,
      [],
    );
    return result.rows.map((row) => ({ id: row.id, parentId: row.parent_id, name: row.name, slug: row.slug, version: row.version }));
  }

  async suggestTags(query: string, limit: number): Promise<readonly TaskTagSuggestion[]> {
    const escaped = query.replace(/[\\%_]/g, (character) => `\\${character}`);
    const result = await this.db.query<{ canonical_name: string; matched_alias: string | null }>(
    ...
    return result.rows.map((row) => ({ canonicalName: row.canonical_name, matchedAlias: row.matched_alias }));
  }

  async findForAudience(taskId: string): Promise<TaskAccessRecord | null> {
    const result = await this.db.query<TaskRow & { assigned_provider_wallets: string[] }>(
      `SELECT t.*,
    ...
    return row === undefined
      ? null
      : { task: mapTaskRow(row), assignedProviderWallets: row.assigned_provider_wallets };
  }

  async listPublicMarket(filters: TaskMarketFilters): Promise<readonly StoredTask[]> {
    const result = await this.db.query<TaskRow>(
      `SELECT * FROM tasks
        WHERE visibility='public'
          AND status NOT IN ('draft','awaiting_escrow')
        ...
        LIMIT $5 OFFSET $6`,
      [filters.keyword, filters.categoryId, filters.tag, filters.status, filters.limit, filters.offset],
    );
    return result.rows.map(mapTaskRow);
  }

  async listOwnedTasks(publisherId: string, limit: number, offset: number): Promise<readonly StoredTask[]> {
    const result = await this.db.query<TaskRow>(
      `SELECT * FROM tasks
  ...
  async readMarketStats(): Promise<Readonly<Record<string, number>>> {
    const result = await this.db.query<{
      ...
      `SELECT count(*)::text AS total,
         ...
         FROM tasks
        WHERE visibility='public' AND status NOT IN ('draft','awaiting_escrow')`,
      [],
    );
  ...
  async readPublisherStats(publisherId: string): Promise<Readonly<Record<string, number>>> {
    const result = await this.db.query<{
      total: string;
      pending: string;
      executing: string;
      ...
      `SELECT count(*)::text AS total,
			  count(*) FILTER (WHERE status IN ('draft','awaiting_escrow','matching','awaiting_agent_acceptance','pending_settlement','execution_failed'))::text AS pending,
              count(*) FILTER (WHERE status IN ('executing','rework'))::text AS executing,
              count(*) FILTER (WHERE status='awaiting_review')::text AS awaiting_review,
              count(*) FILTER (WHERE status IN ('settled','refunded'))::text AS completed,
              count(*) FILTER (WHERE status='disputed')::text AS disputed
         FROM tasks WHERE lower(publisher_id)=lower($1)`,
      [publisherId],
    );
    return numericStats(requiredStatsRow(result.rows[0]));
  }
...
function pricingColumns(pricing: TaskPricing | null): readonly [bigint | null, bigint | null, TaskPricing["type"] | null] {
...
function serializeAssignmentMode(mode: AssignmentModeConfig) {
  return mode.mode === "manual" ? { mode: "manual" } : { ...mode, priceCapMinor: mode.priceCapMinor.toString() };
}

function mapTaskRow(row: TaskRow): StoredTask {
  const pricing: TaskPricing | null = row.pricing_type === null
    ? null
    : row.pricing_type === "fixed"
      ? { type: "fixed", amountMinor: BigInt(requiredString(row.budget_min_minor)) }
      : {
        type: "range",
  ...
  const assignmentMode: AssignmentModeConfig = assignment.mode === "automatic"
    ? {
      mode: "automatic",
      priceCapMinor: BigInt(requiredString(assignment.priceCapMinor)),
      rankingBasis: requiredString(assignment.rankingBasis),
      fallbackOnFail: assignment.fallbackOnFail === "cancel" ? "cancel" : "manual",
    }
    : { mode: "manual" };
  const acceptor = asRecord(row.acceptor_config);
  const acceptanceMode: AcceptanceModeConfig = row.acceptance_mode === "automatic"
    ? { mode: "automatic", acceptorId: requiredString(acceptor.acceptorId), ruleVersion: requiredString(acceptor.ruleVersion) }
    : { mode: "manual" };
  return {
    id: row.id,
    ...
    draft: {
      ...
      attachments: row.attachments.map((attachment) => ({ ...attachment, sizeBytes: BigInt(attachment.sizeBytes) })),
    },
    categoryVersion: row.category_version,
...
function requiredString(value: unknown): string {
  if (typeof value !== "string" || value.length === 0) throw new Error("INVALID_TASK_MODE_STORAGE");
  return value;
}
...
function requiredStatsRow<Row extends Record<string, string>>(row: Row | undefined): Row {
  if (row === undefined) throw new Error("TASK_STATS_QUERY_DID_NOT_RETURN_ROW");
  return row;
}

function numericStats(row: Record<string, string>): Readonly<Record<string, number>> {
  return Object.fromEntries(Object.entries(row).map(([name, value]) => [name, Number.parseInt(value, 10)]));
}
