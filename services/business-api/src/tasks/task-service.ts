...
import { calculatePlatformFee, transitionTaskStatus } from "../platform/task-state";
import {
...
import { parseTaskDraftInput, parseTaskDraftPatch, parseTaskMatchCriteriaPatch, parseTaskModeSettings } from "./task-input";
...
export type TaskServiceResult = Readonly<{ statusCode: number; body: TaskServiceBody }>;
...
export class TaskServiceError extends Error {
  constructor(
    ...
    message: string,
    readonly issues: readonly Readonly<{ field: string; message: string; code?: string }>[] = [],
  ) { super(message); }

  toBody(): TaskServiceBody {
    return { error_code: this.code, message: this.message, retryable: false, issues: this.issues };
  }
}

export interface TaskEventWriter {
  write(event: Readonly<{ taskId: string; statusVersion: bigint; eventType: string; payload: Readonly<Record<string, unknown>>; createdAt: Date }>): Promise<void>;
}
...
export async function createTaskDraft(
  ...
  if (!parsed.success) throw new TaskServiceError(422, "VALIDATION_FAILED", "任务字段格式不正确", parsed.issues);
  if (idempotencyKey === undefined || idempotencyKey.length === 0) {
    throw new TaskServiceError(400, "IDEMPOTENCY_KEY_MISSING", "请求缺少 Idempotency-Key");
  }
  const prepared = await prepareDraftForSave(parsed.data.draft, deps.repository);
  // 自动分配/自动验收的配置规则只有一个入口；用 draft 状态复用同一校验，不复制条件。
  updateTaskModeSettings({
    status: "draft",
    before: { visibility: parsed.data.visibility, assignmentMode: parsed.data.assignmentMode, acceptanceMode: parsed.data.acceptanceMode },
    after: { visibility: parsed.data.visibility, assignmentMode: parsed.data.assignmentMode, acceptanceMode: parsed.data.acceptanceMode },
  });

  const replay = await reserveOrReplay(idempotencyKey, CREATE_OPERATION, deps.idempotency);
  ...
  await deps.auditLogWriter.write({
    ...
    targetId: created.id,
    beforeSummary: {},
    afterSummary: { status: created.status, categoryId: created.draft.categoryId, visibility: created.visibility },
  });
...
export async function editTaskDraft(
  ...
  const replay = await reserveOrReplay(idempotencyKey, `${EDIT_OPERATION}:${taskId}`, deps.idempotency);
  if (replay !== null) return replay;

  const current = await deps.repository.findOwned(taskId, actorId);
  if (current === null) throw new TaskServiceError(404, "TASK_NOT_FOUND", "任务不存在或无权访问");
  if (current.status !== "draft") throw new TaskServiceError(409, "TASK_NOT_EDITABLE", "只有草稿可以编辑");
  const mergedDraft: EditableTaskDraft = { ...current.draft, ...parsed.data.draft };
  ...
  try {
    updateTaskModeSettings({
      ...
      before: { visibility: current.visibility, assignmentMode: current.assignmentMode, acceptanceMode: current.acceptanceMode },
      after: nextSettings,
    });
  } catch (error) {
    throw modeValidationError(error);
  }
  const updated = await deps.repository.updateDraft(taskId, actorId, current.statusVersion, {
    draft: prepared.draft,
    categoryVersion: prepared.categoryVersion,
    ...nextSettings,
  });
  if (updated === null) throw new TaskServiceError(409, "TASK_VERSION_CONFLICT", "任务已被其他请求修改，请刷新后重试");
  await deps.auditLogWriter.write({
    actorId,
    actorType: "publisher",
...
export async function updateOwnedTaskModeSettings(
  ...
  if (!parsed.success) throw new TaskServiceError(422, "VALIDATION_FAILED", "模式设置格式不正确", parsed.issues);
  requireIdempotencyKey(idempotencyKey);
  const replay = await reserveOrReplay(idempotencyKey, `${MODE_OPERATION}:${taskId}`, deps.idempotency);
  if (replay !== null) return replay;
  const current = await deps.repository.findOwned(taskId, actorId);
  if (current === null) throw new TaskServiceError(404, "TASK_NOT_FOUND", "任务不存在或无权访问");
  const nextSettings = {
  ...
  const updated = await deps.repository.updateModeSettings(taskId, actorId, current.statusVersion, nextSettings);
  if (updated === null) throw new TaskServiceError(409, "TASK_VERSION_CONFLICT", "任务已被其他请求修改，请刷新后重试");
  await deps.auditLogWriter.write({
    actorId,
    actorType: "publisher",
...
export async function updateOwnedTaskMatchCriteria(
  ...
  if (!parsed.success) throw new TaskServiceError(422, "VALIDATION_FAILED", "匹配条件格式不正确", parsed.issues);
  requireIdempotencyKey(idempotencyKey);
  const replay = await reserveOrReplay(idempotencyKey, `${MATCH_CRITERIA_OPERATION}:${taskId}`, deps.idempotency);
  if (replay !== null) return replay;

  const current = await deps.repository.findOwned(taskId, actorId);
  if (current === null) throw new TaskServiceError(404, "TASK_NOT_FOUND", "任务不存在或无权访问");
  if (current.status !== "matching") {
    throw new TaskServiceError(409, "MATCH_CRITERIA_LOCKED", "只有待匹配任务可以调整匹配条件");
  }
  ...
  if (categoryId === null || deadline === null) {
    throw new TaskServiceError(409, "TASK_DATA_INCOMPLETE", "已发布任务缺少分类或截止时间");
  }
  ...
  if (deadline.getTime() < deps.now().getTime() + context.minExecutionPeriodMs) {
  ...
  const updated = await deps.repository.updateMatchCriteria(taskId, actorId, current.statusVersion, {
    categoryId,
...
export async function submitTaskDraft(
  ...
  requireIdempotencyKey(idempotencyKey);
  // 先重放幂等快照，再读取当前状态；否则首次提交已把状态改为 awaiting_escrow 后，
  // 同 key 重试会错误返回 TASK_NOT_EDITABLE，而不是首次成功响应。
  const replay = await reserveOrReplay(idempotencyKey, `${SUBMIT_OPERATION}:${taskId}`, deps.idempotency);
  if (replay !== null) return replay;
  const current = await deps.repository.findOwned(taskId, actorId);
  if (current === null) throw new TaskServiceError(404, "TASK_NOT_FOUND", "任务不存在或无权访问");
  if (current.status !== "draft") throw new TaskServiceError(409, "TASK_NOT_EDITABLE", "只有草稿可以提交");
  const complete = requireCompleteDraft(current.draft);
  const context = await requiredContext(deps.repository, complete.categoryId);
  const canonicalTags = normalizeTaskTags(complete.tags, context.canonicalByAlias);
  ...
  const expectedStatus = transitionTaskStatus(current.status, { type: "submit" });
  const submitted = await deps.repository.submit(current.id, actorId, current.statusVersion, canonicalTags, context.categoryVersion);
  if (submitted === null || submitted.status !== expectedStatus) {
    throw new TaskServiceError(409, "TASK_VERSION_CONFLICT", "任务已被其他请求修改，请刷新后重试");
  }
  const feeMinor = calculatePlatformFee(maxBudget(draft), context.feeConfig);
  const createdAt = deps.now();
  await deps.eventWriter.write({
    taskId: submitted.id,
    statusVersion: submitted.statusVersion,
    eventType: "task.submitted",
    payload: { status: submitted.status, feeMinor: feeMinor.toString(), feeRuleVersion: context.feeConfig.version },
    createdAt,
  });
  await deps.auditLogWriter.write({
    actorId,
    ...
    beforeSummary: { status: current.status, version: current.statusVersion.toString() },
    afterSummary: { status: submitted.status, version: submitted.statusVersion.toString(), feeRuleVersion: context.feeConfig.version },
  });
  const result: TaskServiceResult = {
    ...
    body: {
      ...
      preview: {
        escrowAmountMinor: maxBudget(draft).toString(),
        platformFeeMinor: feeMinor.toString(),
        agentReceivesMinor: (maxBudget(draft) - feeMinor).toString(),
        feeBasisPoints: context.feeConfig.feeBasisPoints.toString(),
        minimumPlatformFeeMinor: context.feeConfig.gasFallbackMinor.toString(),
        feeRuleVersion: context.feeConfig.version,
...
export async function previewOwnedTask(taskId: string, actorId: string, deps: Pick<TaskCommandDeps, "repository" | "now">): Promise<TaskServiceResult> {
  const task = await deps.repository.findOwned(taskId, actorId);
  if (task === null) throw new TaskServiceError(404, "TASK_NOT_FOUND", "任务不存在或无权访问");
  ...
  const context = await requiredContext(deps.repository, completeness.draft.categoryId);
  const canonicalTags = normalizeTaskTags(completeness.draft.tags, context.canonicalByAlias);
  ...
  const feeMinor = calculatePlatformFee(amountMinor, context.feeConfig);
  return {
    statusCode: 200,
    body: {
      taskId: task.id,
      ...
      platformFeeMinor: feeMinor.toString(),
      agentReceivesMinor: (amountMinor - feeMinor).toString(),
      feeBasisPoints: context.feeConfig.feeBasisPoints.toString(),
      minimumPlatformFeeMinor: context.feeConfig.gasFallbackMinor.toString(),
      feeRuleVersion: context.feeConfig.version,
      irreversibleWarning: "链上托管确认后，资金只能经验收、退款或仲裁路径释放",
...
export async function listTaskCategories(repository: TaskTaxonomyRepository): Promise<TaskServiceResult> {
  const rows = await repository.listCategories();
  return { statusCode: 200, body: { categories: buildCategoryTree(rows) } };
}

export async function suggestTaskTags(query: string, repository: TaskTaxonomyRepository): Promise<TaskServiceResult> {
  const normalized = query.trim();
  if ([...normalized].length > 60) throw new TaskServiceError(422, "VALIDATION_FAILED", "标签查询最多 60 个字符");
...
function validateForContext(draft: StoredTask["draft"], now: Date, context: TaskCreationContext): readonly TaskFieldError[] {
...
async function prepareDraftForSave(
  ...
  repository: TaskRepository,
): Promise<Readonly<{ draft: EditableTaskDraft; categoryVersion: number | null }>> {
  if (draft.categoryId === null) {
    if (draft.attachments.length > 0) {
      throw validationError([{ field: "categoryId", code: "ATTACHMENT_CATEGORY_REQUIRED", message: "请先选择任务分类，再添加附件" }]);
    }
    return { draft, categoryVersion: null };
  }
  const context = await requiredContext(repository, draft.categoryId);
  const attachmentErrors = validateTaskAttachments(draft.attachments, context.attachmentLimit);
  if (attachmentErrors.length > 0) throw validationError(attachmentErrors);
  return {
    draft: { ...draft, tags: normalizeTaskTags(draft.tags, context.canonicalByAlias) },
    categoryVersion: context.categoryVersion,
  };
}

async function requiredContext(repository: TaskRepository, categoryId: string): Promise<TaskCreationContext> {
  const context = await repository.loadCreationContext(categoryId);
  if (context === null) throw new TaskServiceError(422, "CATEGORY_NOT_FOUND", "任务分类不存在或已停用");
  return context;
}

async function reserveOrReplay(key: string, operation: string, idempotency: Idempotency): Promise<TaskServiceResult | null> {
  const reserved = await idempotency.checkAndReserve(key, operation);
  if (reserved.existing !== null) return { statusCode: reserved.existing.statusCode, body: asBody(reserved.existing.body) };
  if (!reserved.reserved) throw new TaskServiceError(409, "IDEMPOTENCY_REQUEST_IN_PROGRESS", "相同请求正在处理中");
  return null;
}
...
function maxBudget(draft: TaskDraft): bigint {
  return draft.pricing.type === "fixed" ? draft.pricing.amountMinor : draft.pricing.maxAmountMinor;
}
function validationError(errors: readonly TaskFieldError[]): TaskServiceError {
  return new TaskServiceError(422, "VALIDATION_FAILED", "任务未满足发布条件", errors);
}
function asBody(value: unknown): TaskServiceBody {
  return typeof value === "object" && value !== null && !Array.isArray(value) ? value as TaskServiceBody : { value };
}
...
function serializeDraft(draft: EditableTaskDraft): Readonly<Record<string, unknown>> {
  return {
    ...draft,
    ...
    attachments: draft.attachments.map((attachment) => ({ ...attachment, sizeBytes: attachment.sizeBytes.toString() })),
  };
}

function draftAuditSummary(task: StoredTask): Readonly<Record<string, unknown>> {
  return {
    status: task.status,
...
function matchCriteriaAuditSummary(task: StoredTask): Readonly<Record<string, unknown>> {
  return {
    status: task.status,
    version: task.statusVersion.toString(),
    categoryId: task.draft.categoryId,
...
function modeValidationError(error: unknown): TaskServiceError {
  ...
  const message = code === "INVALID_AUTOMATIC_ASSIGNMENT_CONFIG"
    ? "自动分配需要有效的价格上限和排序依据"
    : code === "AUTOMATIC_ACCEPTOR_REQUIRED"
      ...
      : "任务分配或验收模式配置无效";
  return new TaskServiceError(code === "TASK_MODE_LOCKED" ? 409 : 422, code, message);
}

function jsonSafeAuditSettings(settings: ReturnType<typeof updateTaskModeSettings>["settings"]): Readonly<Record<string, unknown>> {
  return {
    visibility: settings.visibility,
    assignmentMode: settings.assignmentMode.mode === "manual"
      ? { mode: "manual" }
      : { ...settings.assignmentMode, priceCapMinor: settings.assignmentMode.priceCapMinor.toString() },
...
function buildCategoryTree(rows: readonly TaskCategory[]): readonly CategoryNode[] {
