...
import { redactTaskForAudience, type TaskAudience, type TaskRecordForAudience } from "../platform/visibility";
...
export class IncompleteTaskProjectionError extends Error {
  ...
  constructor() { super("半成品草稿不能投影为公开任务"); }
}
...
export function projectStoredTask(task: StoredTask, audience: TaskAudience): Readonly<Record<string, unknown>> {
  ...
  const record: TaskRecordForAudience = {
    ...
    attachments: complete.draft.attachments.map(({ name, storageRef }) => ({ name, storageRef })),
...
export function projectStoredTaskPublicPreview(task: StoredTask): Readonly<Record<string, unknown>> {
 ...
 */
export function projectStoredTaskOwnerSummary(task: StoredTask): Readonly<Record<string, unknown>> {
  const pricing = task.draft.pricing === null
    ? null
    : task.draft.pricing.type === "fixed"
      ? { type: "fixed", amountMinor: task.draft.pricing.amountMinor.toString() }
      : {
        type: "range",
...
function jsonSafeValue(value: unknown): unknown {
  ...
  if (typeof value === "object" && value !== null) {
    return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, jsonSafeValue(child)]));
  }
  return value;
}
