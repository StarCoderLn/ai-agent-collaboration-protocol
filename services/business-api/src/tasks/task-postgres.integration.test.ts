...
import { getMarketStats, getTaskDetail, listPublisherTasks, listTaskMarket } from "./task-market-service";
import { createTaskDraft, editTaskDraft, PgTaskEventWriter, previewOwnedTask, submitTaskDraft, updateOwnedTaskMatchCriteria, updateOwnedTaskModeSettings, type TaskCommandDeps } from "./task-service";
...
integration("task PostgreSQL vertical slice", () => {
  ...
  it("persists an incomplete draft, protects non-draft completeness, then edits and submits atomically", async () => {
    ...
    try {
      ...
      const created = await createTaskDraft({ title: "先保存一个尚未完成的真实草稿" }, "publisher-integration", "pg-create-1", deps);
      const taskId = requiredString(created.body.taskId);
      expect((await deps.repository.findOwned(taskId, "publisher-integration"))?.draft.categoryId).toBeNull();

      // 绕过服务层直接推进半成品时，数据库条件约束仍会拒绝，证明完整性不是只靠 UI。
      await client.query("SAVEPOINT incomplete_transition");
      await expect(client.query("UPDATE tasks SET status='awaiting_escrow' WHERE id=$1", [taskId])).rejects.toMatchObject({ code: "23514" });
      await client.query("ROLLBACK TO SAVEPOINT incomplete_transition");

      const completePatch = {
        description: "实现真实 PostgreSQL 草稿保存、发布预览和提交状态迁移，并提供可重复的集成测试证据。",
        acceptanceCriteria: "真实数据库中状态版本单调递增，事件、审计与任务写入位于同一事务。",
      ...
      const edited = await editTaskDraft(taskId, completePatch, "publisher-integration", "pg-edit-1", deps);
      ...
      expect(submitted.body).toMatchObject({ status: "awaiting_escrow", statusVersion: "2" });
      const eventRows = await client.query<{ status_version: string; event_type: string }>(
        "SELECT status_version::text, event_type FROM task_events WHERE task_id=$1",
        [taskId],
      );
      expect(eventRows.rows).toEqual([{ status_version: "2", event_type: "task.submitted" }]);
      ...
      expect(auditRows.rows.map((row) => row.action).sort()).toEqual(["task.create", "task.edit", "task.submit"]);
    } finally {
  ...
  it("lists incomplete and private drafts only for their authenticated publisher", async () => {
    ...
    try {
      ...
      const privateDraft = await createTaskDraft({
        ...
        visibility: "private",
      }, publisherA, "pg-owned-private-a", deps);
      await createTaskDraft({ title: "发布者 B 的草稿" }, publisherB, "pg-owned-private-b", deps);
  ...
  it("serves the public market projection, separate stats, and database-guarded mode settings", async () => {
    ...
    try {
      ...
      const created = await createTaskDraft({
        ...
        description: "验证公开市场列表、详情脱敏、模式配置约束与独立统计查询不会泄漏发布者和附件。",
        acceptanceCriteria: "列表和详情返回完全相同的公开投影，数据库拒绝无验收器的自动验收。",
        ...
        attachments: [],
        visibility: "public",
      }, "0x1111111111111111111111111111111111111111", "pg-market-create", deps);
      const taskId = requiredString(created.body.taskId);
      ...
      }, "0x1111111111111111111111111111111111111111", "pg-mode-1", deps);
      await submitTaskDraft(taskId, "0x1111111111111111111111111111111111111111", "pg-market-submit", deps);
      // Feature 6 的链事件集成测试负责证明 awaiting_escrow→matching；本用例只准备市场可见状态。
      await client.query("UPDATE tasks SET status='matching', status_version=status_version+1 WHERE id=$1", [taskId]);

      const market = await listTaskMarket("http://api.local/api/market/tasks?status=matching", repository);
      const publicTask = (market.body.tasks as readonly Record<string, unknown>[]).find((entry) => entry.id === taskId);
      ...
      await expect(getTaskDetail(taskId, null, repository)).resolves.toMatchObject({ body: { task: publicTask, access: "public" } });
      ...
      await client.query("UPDATE tasks SET status='executing' WHERE id=$1", [taskId]);
      await expect(updateOwnedTaskModeSettings(taskId, { visibility: "private" }, "0x1111111111111111111111111111111111111111", "pg-mode-locked", deps)).rejects.toMatchObject({
  ...
  it("reads the seeded category tree and resolves tag synonyms from PostgreSQL", async () => {
    ...
    expect(categories).toEqual(expect.arrayContaining([
      expect.objectContaining({ id: CATEGORY_ID, name: "产品与开发", parentId: null }),
      expect.objectContaining({ name: "视频制作", parentId: "40000000-0000-4000-8000-000000000003" }),
    ]));
    await expect(repository.suggestTags("nextjs", 10)).resolves.toContainEqual({ canonicalName: "next.js", matchedAlias: "nextjs" });
  });
  ...
  it("updates only matching-safe criteria with atomic event and audit evidence", async () => {
    ...
    try {
      ...
      const result = await updateOwnedTaskMatchCriteria(taskId, {
        tags: ["NextJS", "Agent"],
        deadline: "2026-08-23T04:00:00.000Z",
      }, "publisher-criteria", "pg-criteria-update", postgresDeps(client));
      expect(result.body).toMatchObject({ taskId, status: "matching", statusVersion: "8" });

      const evidence = await client.query<{
        tag_names: string[]; deadline: Date; status_version: string; event_count: string; audit_count: string;
      }>(
      ...
      expect(evidence.rows[0]?.deadline.toISOString()).toBe("2026-08-23T04:00:00.000Z");
    } finally {
      await client.query("ROLLBACK");
      client.release();
    }
  ...
  it("applies dispatch outbox facts through the authoritative state machine exactly once", async () => {
    ...
    try {
      ...
      const locked = await repository.apply(taskId, {
        eventId: lockedEventId, assignmentId, eventType: "assignment_locked",
      });
      expect(locked).toMatchObject({ status: "awaiting_agent_acceptance", statusVersion: 1n, replayed: false });

      await client.query("UPDATE task_assignments SET status='accepted' WHERE id=$1", [assignmentId]);
      const accepted = await repository.apply(taskId, {
        eventId: acceptedEventId, assignmentId, eventType: "agent_accepted",
      });
      expect(accepted).toMatchObject({ status: "executing", statusVersion: 2n, replayed: false });

      const replay = await repository.apply(taskId, {
        eventId: lockedEventId, assignmentId, eventType: "assignment_locked",
      });
      expect(replay).toMatchObject({ status: "awaiting_agent_acceptance", statusVersion: 1n, replayed: true });
      const current = await client.query<{ status: string; status_version: string }>(
        "SELECT status,status_version::text FROM tasks WHERE id=$1",
        [taskId],
      );
      expect(current.rows[0]).toEqual({ status: "executing", status_version: "2" });
  ...
  it("returns a rejected or timed-out assignment to matching through the authoritative state machine", async () => {
    ...
    try {
      ...
      const result = await new PgDispatchTransitionRepository(client).apply(taskId, {
        eventId,
        assignmentId,
        eventType: "assignment_failed",
      });
      expect(result).toMatchObject({ taskId, status: "matching", statusVersion: 4n, replayed: false });
      const stored = await client.query<{ status: string; event_count: string }>(
        `SELECT task.status,
                (SELECT count(*)::text FROM task_events event
  ...
  it("persists monotonic progress, versioned result batches, rework and immutable acceptance money", async () => {
    ...
    try {
      ...
      const progressInput = { agentId, assignmentId, progress: 40, reportedAt: "2026-08-23T01:00:00.000Z" };
      await expect(callbacks.reportStatus(taskId, progressInput, "status:task:not-ready-001", "0".repeat(64)))
        .rejects.toMatchObject({ code: "EXECUTION_NOT_READY", statusCode: 409, retryable: true });
      await client.query("UPDATE tasks SET status='executing' WHERE id=$1", [taskId]);
      const progress = await callbacks.reportStatus(taskId, progressInput, "status:task:request-001", "a".repeat(64));
      expect(progress.body).toMatchObject({ status: "executing", statusVersion: "1", progress: 40 });
      const replay = await callbacks.reportStatus(taskId, progressInput, "status:task:request-001", "a".repeat(64));
      expect(replay).toEqual(progress);
      await expect(callbacks.reportStatus(taskId, { ...progressInput, progress: 20 }, "status:task:request-002", "b".repeat(64)))
        .rejects.toMatchObject({ code: "PROGRESS_REGRESSION" });

      const needsInput = await callbacks.reportStatus(taskId, {
        agentId, assignmentId, state: "needs_input", progress: 45,
        reportedAt: "2026-08-23T01:02:00.000Z",
        estimatedCompletionAt: "2026-08-23T02:00:00.000Z",
        message: "请确认导出文件是否还需要包含 JSON Schema。",
      }, "status:task:needs-input-001", "e".repeat(64));
      expect(needsInput.body).toMatchObject({
        statusVersion: "2", executionState: "needs_input", progress: 45,
        estimatedCompletionAt: "2026-08-23T02:00:00.000Z",
        attentionMessage: "请确认导出文件是否还需要包含 JSON Schema。",
      });
      await expect(callbacks.readStatus(taskId, "publisher-execution")).resolves.toMatchObject({
        body: { executionState: "needs_input", progress: 45, attentionMessage: "请确认导出文件是否还需要包含 JSON Schema。" },
      });

      const firstBatch = await callbacks.submitResults(taskId, {
        agentId, assignmentId,
        results: [
          { kind: "inline", summary: "方案 A", mimeType: "text/markdown", generatedAt: "2026-08-23T01:10:00.000Z", content: "# A\n完整实现" },
          { kind: "inline", summary: "方案 B", mimeType: "text/plain", generatedAt: "2026-08-23T01:11:00.000Z", content: "备选实现" },
        ],
      }, "result-submit:task:request-001", "c".repeat(64));
      expect(firstBatch.body).toMatchObject({ status: "awaiting_review", statusVersion: "3", batchNo: 1 });
      const firstResults = firstBatch.body.results as readonly Record<string, unknown>[];
      const firstResultId = requiredString(firstResults[0]?.id);

      const publisher = createExecutionService(repository, new Idempotency(new PgIdempotencyStore(client), 60_000, () => NOW));
      const rework = await publisher.rework(taskId, { resultId: firstResultId, reason: "请补充失败路径和恢复说明，确保验收条件可执行。" }, "publisher-execution", "rework-task-request-001");
      expect(rework.body).toMatchObject({ status: "rework", statusVersion: "4", requestNo: 1 });

      const secondBatch = await callbacks.submitResults(taskId, {
        agentId, assignmentId,
        results: [{ kind: "inline", summary: "修订方案", mimeType: "text/markdown", generatedAt: "2026-08-23T01:20:00.000Z", content: "# 修订\n包含恢复路径" }],
      }, "result-submit:task:request-002", "d".repeat(64));
      const secondResults = secondBatch.body.results as readonly Record<string, unknown>[];
      const acceptedResultId = requiredString(secondResults[0]?.id);
      const acceptancePreview = await publisher.previewAcceptance(taskId, { resultId: acceptedResultId }, "publisher-execution");
      ...
      const expectedSettlement = acceptancePreview.body.settlement as {
        grossAmountMinor: string; platformFeeMinor: string; agentAmountMinor: string; feeRuleVersion: string;
      };
      await expect(publisher.accept(taskId, {
        resultId: acceptedResultId,
        expectedStatusVersion: "4",
        expectedSettlement,
      }, "publisher-execution", "accept-task-stale-preview-001")).rejects.toMatchObject({
        code: "ACCEPTANCE_PREVIEW_STALE", statusCode: 409, retryable: true,
      });
      const acceptanceInput = { resultId: acceptedResultId, expectedStatusVersion: "5", expectedSettlement };
      const accepted = await publisher.accept(taskId, acceptanceInput, "publisher-execution", "accept-task-request-001");
      ...
      const evidence = await client.query<{ versions: string; latest_results: string; acceptances: string; deliveries: string; invalid_delivery_keys: string; settlement_jobs: string; settlement_payee: string; rework_reason: string }>(
      ...
      expect(evidence.rows[0]).toEqual({
        versions: "1,2,3,4,5,6", latest_results: "1", acceptances: "1", deliveries: "6", invalid_delivery_keys: "0", settlement_jobs: "1",
      ...
      await client.query("UPDATE tasks SET status='settled',status_version=7 WHERE id=$1", [taskId]);
      await emitTaskEvent(client, {
        taskId, statusVersion: 7n, eventType: "task.settlement_confirmed",
        payload: { status: "settled", txHash: `0x${"12".repeat(32)}` }, createdAt: NOW,
      });
      const scoring = createScoringService(
        new PgScoringRepository(client), new Idempotency(new PgIdempotencyStore(client), 60_000, () => NOW), () => NOW,
      );
      const ratingInput = { quality: 5, communication: 5 };
      const rated = await scoring.submitRating(taskId, ratingInput, "publisher-execution", "rating-task-request-001");
      expect(rated.body).toMatchObject({ taskId, agentId, statusVersion: "8" });
      await expect(scoring.submitRating(taskId, ratingInput, "publisher-execution", "rating-task-request-001"))
        .resolves.toEqual(rated);
      await expect(scoring.submitRating(
        taskId, { ...ratingInput, disputeRate: 0 }, "publisher-execution", "rating-task-request-invalid",
      ...
      await expect(scoring.readLatestScore(agentId)).resolves.toMatchObject({
        body: { agentId, ruleVersion: "score-v1", sampleSize: 1, lowSample: true },
      });
      const ratingEvidence = await client.query<{
        ratings: string; snapshots: string; versions: string; input_evidence: Record<string, unknown>;
      }>(
      ...
      expect(ratingEvidence.rows[0]).toMatchObject({
        ratings: "1", snapshots: "1", versions: "1,2,3,4,5,6,7,8",
        input_evidence: {
  ...
  it("persists a sanitized Agent failure exactly once without creating a settlement job", async () => {
    ...
    try {
	  ...
	  const publisherBeforeFailure = await statsRepository.readPublisherStats("publisher-failure");

      const callbacks = createExecutionService(new PgExecutionRepository(client));
      const failure = {
        agentId,
        assignmentId,
      ...
      const first = await callbacks.reportStatus(taskId, failure, "status:failure:request-001", "f".repeat(64));
      const replay = await callbacks.reportStatus(taskId, failure, "status:failure:request-001", "f".repeat(64));
      ...
      const evidence = await client.query<{
        task_status: string; event_count: string; failure_code: string;
        failed_at: Date; settlement_jobs: string; callback_count: string;
      }>(
      ...
      expect(evidence.rows[0]?.failed_at.toISOString()).toBe(failure.reportedAt);
	  const marketAfterFailure = await statsRepository.readMarketStats();
	  const publisherAfterFailure = await statsRepository.readPublisherStats("publisher-failure");
	  expect(marketAfterFailure.execution_failed).toBe((marketBeforeFailure.execution_failed ?? 0) + 1);
	  expect(marketAfterFailure.executing).toBe((marketBeforeFailure.executing ?? 0) - 1);
	  expect(publisherAfterFailure.pending).toBe((publisherBeforeFailure.pending ?? 0) + 1);
	  expect(publisherAfterFailure.executing).toBe((publisherBeforeFailure.executing ?? 0) - 1);
    } finally {
      await client.query("ROLLBACK");
      client.release();
    }
  ...
  it("times out matching, awaiting-agent and executing tasks but never awaiting escrow", async () => {
    ...
    await client.query("BEGIN");
    try {
      const statuses = ["matching", "awaiting_agent_acceptance", "executing", "awaiting_escrow"] as const;
      const ids = statuses.map((_, index) => `85000000-0000-4000-8000-00000000000${index + 1}`);
      for (const [index, status] of statuses.entries()) {
        await client.query(
          ...
          [ids[index], `超时状态测试任务 ${index + 1}`, CATEGORY_ID, new Date("2026-08-22T23:00:00.000Z"), status],
        );
      }
      ...
      expect(stored.rows.map((row) => row.status)).toEqual(["timed_out", "timed_out", "timed_out", "awaiting_escrow"]);
  ...
  it("queries a task dead letter without exposing its endpoint path or changing business state", async () => {
    ...
    try {
      ...
      const created = await createTaskDraft({
        ...
        description: "验证通知基础设施连续投递失败并进入死信之后，不会回滚或改写已经提交的任务业务状态。",
        acceptanceCriteria: "运营查询能看到次数和错误码，但不能看到 Agent 端点路径。",
        ...
        assignmentMode: { mode: "manual" },
        acceptanceMode: { mode: "manual" },
      }, "publisher-dead-letter", "pg-dead-letter-create", deps);
      const taskId = requiredString(created.body.taskId);
      await submitTaskDraft(taskId, "publisher-dead-letter", "pg-dead-letter-submit", deps);
      ...
      const statusBefore = await client.query<{ status: string; status_version: string }>(
        "SELECT status,status_version::text FROM tasks WHERE id=$1", [taskId],
      );
      const deliveries = await new PgTaskEventReader(client).deadLetters(taskId, 20);
      const pulledStatus = await createExecutionService(new PgExecutionRepository(client))
        .readStatus(taskId, "publisher-dead-letter");
      const statusAfter = await client.query<{ status: string; status_version: string }>(
        "SELECT status,status_version::text FROM tasks WHERE id=$1", [taskId],
      );

      expect(deliveries).toHaveLength(1);
      expect(deliveries[0]).toMatchObject({
        taskId, taskEventId: eventId, agentId, endpoint: "https://agent.example/…",
        attemptNo: 5, lastErrorCode: "CONN_TIMEOUT",
      });
      ...
      expect(pulledStatus.body).toMatchObject({
        taskId, status: "awaiting_escrow", statusVersion: "1", progress: 0,
        executionState: "running", lastEventId: eventId,
      });
      expect(statusAfter.rows).toEqual(statusBefore.rows);
      expect(statusAfter.rows[0]).toMatchObject({ status: "awaiting_escrow", status_version: "1" });
    } finally {
      await client.query("ROLLBACK");
      client.release();
    }
...
function postgresDeps(client: PoolClient): TaskCommandDeps {
  return {
    ...
    idempotency: new Idempotency(new PgIdempotencyStore(client), 60_000, () => NOW),
...
function requiredString(value: unknown): string {
  if (typeof value !== "string" || value.length === 0) throw new Error("EXPECTED_NON_EMPTY_STRING");
  return value;
}

function requiredMarketStats(value: unknown): Readonly<{ total: number; matching: number }> {
