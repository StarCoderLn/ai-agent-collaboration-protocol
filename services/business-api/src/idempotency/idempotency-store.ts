...
export interface IdempotencyStore {
  reserve(key: string, operationType: string, expiresAt: Date): Promise<ReserveOutcome>;
...
export class PgIdempotencyStore implements IdempotencyStore {
  ...
  async reserve(key: string, operationType: string, expiresAt: Date): Promise<ReserveOutcome> {
    ...
    return { inserted: false, committed: true, snapshot: row.response_snapshot };
  }
...
export class Idempotency {
  ...
  async checkAndReserve(key: string, operationType: string): Promise<CheckAndReserveResult> {
    if (!key) {
