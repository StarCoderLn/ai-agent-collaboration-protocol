...
describe("formal workflow planner", () => {
  it("新软件任务只为设计与开发报价，不默认购买 PRD", () => {
    ...
    expect(plan.nodes.map((node) => node.status)).toEqual(["selecting", "selecting"]);
    expect(plan.nodes.map((node) => node.budgetWeight)).toEqual([30, 50]);
    expect(plan.nodes.map((node) => node.inputContract)).toEqual(["TaskContract", "TaskContract+DesignArtifact"]);
  ...
  it("明确购买 PRD 时仍可独立执行，视频任务同样无需收费 PRD 前置", () => {
    const prd = planFormalWorkflow({ taskCategoryId: "40000000-0000-4000-8000-000000000021", taskTags: ["coding"], requiredCapability: "编写产品 PRD" });
    ...
    const video = planFormalWorkflow({ taskCategoryId: "40000000-0000-4000-8000-000000000013", taskTags: [], requiredCapability: "活动短片" });
  ...
  it("独立设计任务不附带 PRD 或开发服务", () => {
    const plan = planFormalWorkflow({ taskCategoryId: "40000000-0000-4000-8000-000000000022", taskTags: ["ui/ux"], requiredCapability: "电商首页设计稿" });
    expect(plan.nodes.map((node) => node.key)).toEqual(["design"]);
    expect(plan.nodes[0]).toMatchObject({ inputContract: "TaskContract", outputContract: "DesignArtifact", budgetWeight: 100 });
  });
