...
describe("workflow preferences", () => {
  it("把预算偏好按权重分配，但不写入任何冻结金额列", async () => {
    ...
    expect(result.body.nodePreferences.map((item) => item.pricePreferenceMinor))
      .toEqual(["12000000", "18000000", "30000000"]);
    const mutationSql = db.executedSql.filter((sql) => sql.startsWith("UPDATE task_workflow"));
    expect(mutationSql.join("\n")).not.toMatch(/budget_cap_minor|agreed_amount_minor|quoted_total_minor/);
  });

  it("已有节点选择后锁定预算偏好，且非法金额不会进入数据库", async () => {
    const locked = new PreferenceDb({ selectedAgentId: "44444444-4444-4444-8444-444444444444" });
    await expect(updateOwnedWorkflowBudgetPreference(locked, {
      taskId: TASK_ID,
      actorId: PUBLISHER,
      rawInput: { budgetPreferenceMinor: "60000000" },
      idempotencyKey: "budget-preference-unit-2",
      updatedAt: UPDATED_AT,
    })).rejects.toMatchObject({ code: "WORKFLOW_PREFERENCE_LOCKED", statusCode: 409 });

    const untouched = new PreferenceDb();
    await expect(updateOwnedWorkflowBudgetPreference(untouched, {
      taskId: TASK_ID,
      actorId: PUBLISHER,
  ...
  it("归一能力同义词并允许清空软能力，不影响预算与报价", async () => {
    ...
    expect(db.executedSql.join("\n")).not.toMatch(/SET[^;]*(budget_cap_minor|agreed_amount_minor|price_preference_minor)/);

    const cleared = new PreferenceDb();
    await expect(updateOwnedWorkflowNodeCapabilities(cleared, {
      taskId: TASK_ID,
      nodeId: NODE_ID,
      actorId: PUBLISHER,
      rawInput: { tags: [] },
  ...
  it("能力修改对越权与已选节点使用同一安全锁定边界", async () => {
    await expect(updateOwnedWorkflowNodeCapabilities(new PreferenceDb(), {
      taskId: TASK_ID,
      nodeId: NODE_ID,
      ...
      updatedAt: UPDATED_AT,
    })).rejects.toMatchObject({ code: "WORKFLOW_NODE_NOT_FOUND", statusCode: 404 });

    await expect(updateOwnedWorkflowNodeCapabilities(new PreferenceDb({ nodeStatus: "selected" }), {
      taskId: TASK_ID,
      nodeId: NODE_ID,
      actorId: PUBLISHER,
      rawInput: { tags: ["next.js"] },
      ...
      updatedAt: UPDATED_AT,
    })).rejects.toMatchObject({ code: "WORKFLOW_CAPABILITY_LOCKED", statusCode: 409 });
  });
});
...
class PreferenceDb implements QueryExecutor {
  ...
  async query<Row>(text: string, params: readonly unknown[]): Promise<{ rows: Row[]; rowCount: number | null }> {
    ...
    if (sql.includes("FROM tasks task") && sql.includes("JOIN task_workflow_nodes node")) {
      return result([{
    ...
    if (sql.includes("FROM tasks task") && sql.includes("JOIN task_workflow_runs run") && !sql.includes("task_workflow_nodes")) {
      return result([{
        id: RUN_ID,
        publisher_id: PUBLISHER,
        task_status: "planning",
    ...
    if (sql.includes("FROM task_workflow_nodes") && sql.includes("price_preference_weight")) {
      return result([
        { id: NODE_ID, price_preference_weight: 20, selected_agent_id: this.options.selectedAgentId ?? null },
        { id: "33333333-3333-4333-8333-333333333334", price_preference_weight: 30, selected_agent_id: null },
        { id: "33333333-3333-4333-8333-333333333335", price_preference_weight: 50, selected_agent_id: null },
      ]);
    }
    if (sql === "SELECT canonical_name,synonyms,forbidden FROM tags") {
      return result([
        { canonical_name: "next.js", synonyms: ["nextjs", "next js"], forbidden: false },
        { canonical_name: "ui/ux", synonyms: ["ui", "界面设计"], forbidden: false },
      ]);
    }
    if (sql.startsWith("UPDATE task_workflow_nodes") && sql.includes("SET tags=")) {
      this.savedNodeTags = params[2] as readonly string[];
      return result([], 1);
    }
    if (sql.startsWith("UPDATE task_workflow_runs")
