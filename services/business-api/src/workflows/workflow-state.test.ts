...
const nodes = (statuses: readonly WorkflowNodeSnapshot["status"][]): WorkflowNodeSnapshot[] =>
...
describe("formal workflow state", () => {
  ...
  it("按分配、接单、交付和验收顺序推进节点", () => {
    ...
    expect(transitionWorkflowNode("executing", { type: "results_submitted" })).toBe("awaiting_review");
    expect(transitionWorkflowNode("awaiting_review", { type: "result_accepted" })).toBe("accepted");
  });

  it("拒绝乱序验收，不能把未交付节点直接标记完成", () => {
    expect(() => transitionWorkflowNode("executing", { type: "result_accepted" })).toThrow(
  ...
  it("共同上游验收后同时解锁并行节点，下游仍等待所有分支", () => {
    ...
    expect(first.map((node) => node.status)).toEqual(["accepted", "matching", "matching", "blocked"]);
    const second = unlockReadyWorkflowNodes(
      first.map((node) => node.id === "node-2" ? { ...node, status: "accepted" } : node),
      [
        { sourceNodeId: "node-1", targetNodeId: "node-2" },
        { sourceNodeId: "node-1", targetNodeId: "node-3" },
        { sourceNodeId: "node-2", targetNodeId: "node-4" },
  ...
  it("拒绝环形依赖，避免工作流永远无法解锁", () => {
    expect(() => assertValidWorkflowGraph(nodes(["blocked", "blocked"]), [
      { sourceNodeId: "node-1", targetNodeId: "node-2" },
  ...
  it("从节点事实计算工作流聚合状态", () => {
    expect(aggregateWorkflowStatus(nodes(["accepted", "accepted"]))).toBe("completed");
    expect(aggregateWorkflowStatus(nodes(["accepted", "awaiting_review"]))).toBe("awaiting_review");
    expect(aggregateWorkflowStatus(nodes(["executing", "disputed"]))).toBe("disputed");
    expect(aggregateWorkflowStatus(nodes(["execution_failed", "blocked"]))).toBe("failed");
  });
});
