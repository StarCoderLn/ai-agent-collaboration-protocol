...
integration("workflow selection PostgreSQL contract", () => {
  ...
  it("keeps partial selections in planning, freezes the exact sum, and prepares escrow from that sum", async () => {
    await withRollbackClient(pool, async (client) => {
      ...
      const firstInput = selectionInput(fixture, fixture.nodeIds[0], "select-first");
      ...
      await repository.select(selectionInput(fixture, fixture.nodeIds[1], "select-second"));
      const finalSelection = await repository.select(selectionInput(fixture, fixture.nodeIds[2], "select-third"));
  ...
  it("allows reselection after a verified pre-broadcast failure and locks once Deposit is submitted", async () => {
    await withRollbackClient(pool, async (client) => {
      ...
      const database = asNestedTransactionDatabase(client);
      const repository = new PgWorkflowSelectionRepository(database);
      await repository.select(selectionInput(fixture, fixture.nodeIds[0], "revise-first"));
      await repository.select(selectionInput(fixture, fixture.nodeIds[1], "revise-second"));
      await repository.select(selectionInput(fixture, fixture.nodeIds[2], "revise-third"));

      const replacementAgentId = await addReplacementCandidate(client, fixture, fixture.nodeIds[1], "17000000");
      ...
      await escrow.markSubmissionFailed(fixture.taskId, PUBLISHER, "用户取消了尚未广播的托管交易");
      await expect(
        repository.select({
          ...selectionInput(fixture, fixture.nodeIds[1], "revise-after-failed-escrow"),
          agentId: fixture.agentId,
        }),
      ).resolves.toMatchObject({
        body: {
      ...
      await expect(
        repository.select({
          ...selectionInput(fixture, fixture.nodeIds[1], "revise-failed-second-time"),
          agentId: replacementAgentId,
        }),
      ).resolves.toMatchObject({ body: { quotedTotalMinor: "57000000" } });
      await expect(
        repository.select({
          ...selectionInput(fixture, fixture.nodeIds[1], "revise-failed-third-time"),
          agentId: fixture.agentId,
        }),
      ).resolves.toMatchObject({ body: { quotedTotalMinor: "60000000" } });
      await expect(escrow.resetFailedIntent(fixture.taskId, PUBLISHER)).resolves.toMatchObject({
        amountMinor: 60_000_000n,
        status: "prepared",
      });

      const txHash = `0x${"91".repeat(32)}`;
      await expect(escrow.recordSubmission(fixture.taskId, PUBLISHER, txHash, 57_000_000n)).rejects.toMatchObject({
        code: "ESCROW_SUBMISSION_NOT_ALLOWED",
        statusCode: 409,
      });
      await escrow.recordSubmission(fixture.taskId, PUBLISHER, txHash, 60_000_000n);
      await expect(
        repository.select({
          ...selectionInput(fixture, fixture.nodeIds[1], "revise-after-submission"),
          agentId: replacementAgentId,
        }),
      ).rejects.toMatchObject({
        code: "WORKFLOW_SELECTION_LOCKED",
  ...
  it("persists an optional budget preference separately from all frozen money fields", async () => {
    await withRollbackClient(pool, async (client) => {
      ...
      expect(result.body.nodePreferences.map((item) => item.pricePreferenceMinor)).toEqual([
        "12000000",
        "18000000",
        "30000000",
      ]);
      // 相同幂等键重放第一次快照，不能重复增加 run/node 版本。
      await expect(updateOwnedWorkflowBudgetPreference(client, input)).resolves.toEqual(result);
  ...
  it("locks budget and capability preferences after any Agent quote is frozen", async () => {
    await withRollbackClient(pool, async (client) => {
      const fixture = await insertSelectionFixture(client);
      const repository = new PgWorkflowSelectionRepository(asNestedTransactionDatabase(client));
      await repository.select(selectionInput(fixture, fixture.nodeIds[0], "lock-preferences"));
  ...
  it("rejects unauthorized, non-candidate, and malformed snapshots without changing any node", async () => {
    await withRollbackClient(pool, async (client) => {
      ...
      await expect(
        repository.select(selectionInput(fixture, fixture.nodeIds[0], "invalid-snapshot")),
      ).rejects.toMatchObject({
...
async function insertSelectionFixture(client: PoolClient): Promise<SelectionFixture> {
  ...
  for (const [nodeId, nodeKey, kind, title, weight, position] of definitions) {
    await client.query(
      ...
      [nodeId, runId, taskId, nodeKey, kind, title, CATEGORY_ID, weight, position],
    );
...
async function addReplacementCandidate(
  ...
  await client.query(
    ...
    [fixture.taskId, nodeId, JSON.stringify([{ agentId, quoteMinor, name: "替换候选 Agent" }])],
  );
  return agentId;
}

function selectionInput(fixture: SelectionFixture, nodeId: string, keySuffix: string) {
  return {
    taskId: fixture.taskId,
    nodeId,
    agentId: fixture.agentId,
  ...
  return {
    ...
    connect: async () => ({
      query: async (text, params) => {
        if (text === "BEGIN") return client.query("SAVEPOINT workflow_selection_nested");
        if (text === "COMMIT") return client.query("RELEASE SAVEPOINT workflow_selection_nested");
        if (text === "ROLLBACK") {
