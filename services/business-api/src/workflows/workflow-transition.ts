...
export interface WorkflowTransitionRepository {
  apply(taskId: string, workflowNodeId: string, input: WorkflowTransitionInput): Promise<AppliedWorkflowTransition>;
}

export class WorkflowTransitionError extends Error {
  constructor(
    readonly code: "VALIDATION_FAILED" | "WORKFLOW_NODE_NOT_FOUND" | "ASSIGNMENT_NOT_FOUND" | "EVENT_ID_REUSED" | "TRANSITION_NOT_READY",
    message: string,
...
export async function applyWorkflowTransition(
  ...
  if (!parsed.success) {
    throw new WorkflowTransitionError("VALIDATION_FAILED", "工作节点派发事件格式不正确", 422, false);
  }
