...
integration("workflow feedback PostgreSQL contract", () => {
	...
	it("writes an accepted node review and exposes it in the Agent scoring history", async () => {
		await withRollbackClient(pool, async (client) => {
			...
			await expect(repository.list(fixture.taskId, PUBLISHER)).resolves.toMatchObject({
				body: {
			...
			const scoreEvidence = await client.query<{ input_evidence: { ratingIds: string[] } }>(
			...
			expect(scoreEvidence.rows[0]?.input_evidence.ratingIds).toContain(feedbackId);
	...
	it("rejects unauthorized, unaccepted, and duplicate feedback", async () => {
		await withRollbackClient(pool, async (client) => {
			...
			await expect(repository.submit(
				...
				input,
				SUBMITTED_AT,
			)).rejects.toMatchObject({ code: "WORKFLOW_NODE_NOT_ACCEPTED", statusCode: 409 });
			...
			await expect(repository.submit(
				...
				input,
				SUBMITTED_AT,
			)).rejects.toMatchObject({ code: "WORKFLOW_FEEDBACK_ALREADY_SUBMITTED", statusCode: 409 });
		});
	});
});

async function insertFeedbackFixture(client: PoolClient): Promise<FeedbackFixture> {
