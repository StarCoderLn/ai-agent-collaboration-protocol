...
import { parseWorkflowFeedbackInput, type WorkflowFeedbackInput } from "./workflow-feedback-input";
	...
	return {
		submit: async (
			...
			if (key === undefined || key.length < 8 || key.length > 200) {
				throw new WorkflowFeedbackServiceError(400, "IDEMPOTENCY_KEY_REQUIRED", "提交反馈需要有效的幂等键");
			}
			if (idempotency === undefined) throw new Error("WORKFLOW_FEEDBACK_IDEMPOTENCY_NOT_CONFIGURED");
			...
			const reservation = await idempotency.checkAndReserve(key, `workflow.feedback:${taskId}:${nodeId}`);
			...
			if (!reservation.reserved) {
				throw new WorkflowFeedbackServiceError(409, "IDEMPOTENCY_REQUEST_IN_PROGRESS", "相同反馈正在提交中");
			}
			const result = await repository.submit(taskId, nodeId, actorId, parsed.data, now());
