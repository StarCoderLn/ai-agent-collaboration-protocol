...
const voteSchema = z.object({
  ...
  const valid = value.decision === "release"
  ...
  if (!valid) context.addIssue({ code: "custom", path: ["releaseBasisPoints"], message: "裁决类型与结算比例不一致" });
});
...
export class DaoService {
  ...
  async syncMembership(actorId: string, raw: unknown, now: Date): Promise<Readonly<Record<string, unknown>>> {
    const parsed = transactionHashSchema.safeParse(
      typeof raw === "object" && raw !== null && "txHash" in raw ? raw.txHash : undefined,
    );
    if (!parsed.success) throw new DaoServiceError(422, "VALIDATION_FAILED", "会员交易哈希格式无效");
    // 浏览器只上报交易哈希，资格、质押额和退出状态必须由服务端在对应回执区块重建。
    // 这条可信边界防止用户通过篡改请求体伪造 DAO 成员资格。
    let snapshot: DaoMembershipSnapshot;
    try {
      snapshot = await this.chain.verifyMembershipTransaction(parsed.data, actorId);
    } catch (error) {
      throw chainError(error);
    }
    if (snapshot.minimumStakeMinor !== this.config.minimumStakeMinor) {
      throw new DaoServiceError(409, "DAO_STAKE_CONFIG_MISMATCH", "链上最低质押额与平台配置不一致");
    }
  ...
  async vote(roundId: string, actorId: string, raw: unknown, now: Date): Promise<Readonly<Record<string, unknown>>> {
    if (!z.uuid().safeParse(roundId).success) throw new DaoServiceError(404, "DAO_CASE_NOT_FOUND", "仲裁案件不存在");
    const parsed = voteSchema.safeParse(raw);
    if (!parsed.success) throw new DaoServiceError(422, "VALIDATION_FAILED", "投票字段不完整或结算比例不合法");
    const actor = normalizeAddress(actorId);
    ...
    return withTransaction(this.pool, async (client) => {
      ...
      if (round === undefined) throw new DaoServiceError(404, "DAO_CASE_NOT_FOUND", "仲裁案件不存在");
      if (round.status !== "voting") throw new DaoServiceError(409, "DAO_CASE_NOT_VOTING", "案件当前不能投票");
      if (now > round.voting_deadline) throw new DaoServiceError(409, "DAO_VOTING_CLOSED", "案件投票期限已结束");
      const panel = await client.query(
        "SELECT 1 FROM dao_arbitration_panel_members WHERE round_id=$1 AND actor_id=$2",
        [roundId, actor],
      );
      if (panel.rows[0] === undefined) throw new DaoServiceError(403, "DAO_VOTE_FORBIDDEN", "当前钱包不在本案仲裁小组中");
      ...
      try {
        await client.query(
          ...
          [voteId, roundId, actor, parsed.data.decision, parsed.data.releaseBasisPoints,
            parsed.data.agentResponsibility, parsed.data.reasoning, now],
        );
      } catch (error) {
        if (databaseCode(error) === "23505") throw new DaoServiceError(409, "DAO_ALREADY_VOTED", "当前钱包已经提交过本案投票");
        throw error;
      }
      ...
      try {
        decision = await finalizeDaoDecision(client, round, outcome, votes.rows, now);
      } catch (error) {
...
export async function createDaoRoundForDispute(
  ...
  await db.query(
    ...
    [roundId, disputeId, selectionSeed, config.panelSize, config.quorum,
      new Date(now.getTime() + config.votingWindowMs), now],
  );
  const assigned = await assignRound(db, disputeId, taskId, now);
  return assigned;
}

async function upsertMembership(db: QueryExecutor, snapshot: DaoMembershipSnapshot, now: Date): Promise<MembershipRow> {
  ...
  const result = await db.query<MembershipRow>(
    `INSERT INTO dao_memberships(
     ...
     RETURNING actor_id,staked_amount_minor::text,eligible,exit_available_at,sync_tx_hash,
               sync_block_number::text,synced_at`,
    [snapshot.actorId, snapshot.chainId.toString(), snapshot.contractAddress,
      snapshot.stakedAmountMinor.toString(), snapshot.eligible, snapshot.exitAvailableAt,
      snapshot.syncTxHash, snapshot.syncBlockNumber.toString(), now],
  );
...
async function assignAwaitingPanels(db: QueryExecutor, now: Date): Promise<void> {
  const config = await readDaoConfig(db);
  ...
  for (const round of rounds.rows) {
    const assigned = await assignRound(db, round.dispute_id, round.task_id, now);
...
async function readDaoConfig(db: QueryExecutor): Promise<{ panelSize: number; quorum: number; votingWindowMs: number }> {
  const result = await db.query<{ panel_size: number; quorum: number; voting_window_seconds: number }>(
  ...
  return { panelSize: row.panel_size, quorum: row.quorum, votingWindowMs: row.voting_window_seconds * 1_000 };
}
...
async function assignRound(
  ...
  // 因数据库返回顺序变化而更换仲裁成员。
  const roundResult = await db.query<{ id: string; selection_seed: string; panel_size: number; status: string }>(
  ...
  if (round.status !== "awaiting_panel") return { roundId: round.id, status: "voting" };
  ...
  const candidates = await db.query<{ actor_id: string; staked_amount_minor: string }>(
  ...
  const selected = [...candidates.rows]
    .sort((left, right) => selectionScore(round.selection_seed, left.actor_id).localeCompare(selectionScore(round.selection_seed, right.actor_id)))
    .slice(0, round.panel_size);
  if (selected.length < round.panel_size) return { roundId: round.id, status: "awaiting_panel" };
  ...
  await db.query("UPDATE dao_arbitration_rounds SET status='voting' WHERE id=$1", [round.id]);
  return { roundId: round.id, status: "voting" };
}

function resolveOutcome(votes: readonly VoteRow[], quorum: number): ResolvedOutcome | null {
  ...
  for (const vote of votes) grouped.set(vote.decision, [...(grouped.get(vote.decision) ?? []), vote]);
  const winner = [...grouped.entries()].find(([, decisionVotes]) => decisionVotes.length >= quorum);
  if (winner === undefined) return null;
  const [decision, decisionVotes] = winner;
  const releaseBasisPoints = decision === "partial_release"
    ? median(decisionVotes.map((vote) => vote.release_basis_points))
    : decision === "release" ? 10_000 : 0;
  return {
    decision,
    releaseBasisPoints,
    responsibility: responsibilityConsensus(decisionVotes),
    reasoning: decisionVotes.map((vote) => vote.reasoning.trim()).join("\n\n").slice(0, 5_000),
  };
}
...
export async function finalizeDaoDecision(
  ...
  const settlementContext = await loadArbitrationSettlementContext(db, round.task_id, round.dispute_id);
  const releaseAmount = outcome.decision === "refund"
    ? 0n
    : outcome.decision === "release"
      ? settlementContext.escrowAmountMinor
      : settlementContext.escrowAmountMinor * BigInt(outcome.releaseBasisPoints) / 10_000n;
  ...
  await db.query(
    ...
    [decisionId, round.dispute_id, `dao:${round.round_id}`, outcome.decision,
      plan.releaseAmountMinor.toString(), plan.refundAmountMinor.toString(),
      outcome.decision === "refund" ? null : plan.platformFeeMinor.toString(),
      outcome.decision === "refund" ? null : plan.agentAmountMinor.toString(), outcome.responsibility,
      outcome.reasoning, now, outcome.releaseBasisPoints, plan.decisionHash, plan.evidenceRoot],
  );
  ...
  if (outcome.decision === "refund") {
    ...
    await db.query(
      ...
      [round.task_id, decisionId, JSON.stringify(plan.payouts), plan.settlementManifestHash,
        plan.evidenceRoot, plan.decisionHash, now],
    );
  }
  ...
  await new PgAuditLogWriter(db).write({
    ...
    beforeSummary: { status: "evidence_collection", voteCount: allVotes.length },
    afterSummary: {
      decisionId,
      decision: outcome.decision,
      releaseBasisPoints: outcome.releaseBasisPoints,
...
function selectionScore(seed: string, actorId: string): string {
  return solidityPackedKeccak256(["bytes32", "address"], [seed, getAddress(actorId)]);
}

function responsibilityConsensus(votes: readonly VoteRow[]): AgentResponsibility {
  const counts = new Map<AgentResponsibility, number>();
  for (const vote of votes) counts.set(vote.agent_responsibility, (counts.get(vote.agent_responsibility) ?? 0) + 1);
  const sorted = [...counts.entries()].sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]));
  if (sorted.length === 0 || (sorted[1]?.[1] ?? -1) === sorted[0]?.[1]) return "not_determined";
  return required(sorted[0], "DAO_RESPONSIBILITY_NOT_FOUND")[0];
}

function median(values: readonly number[]): number {
  const sorted = [...values].sort((left, right) => left - right);
  return required(sorted[Math.floor(sorted.length / 2)], "DAO_PARTIAL_RATIO_NOT_FOUND");
}
...
function normalizeAddress(value: string): string {
  try {
    ...
    throw new DaoServiceError(422, "INVALID_WALLET_ADDRESS", "钱包地址格式无效");
  }
}
...
function chainError(error: unknown): DaoServiceError {
  ...
  return new DaoServiceError(code === "DAO_TRANSACTION_NOT_CONFIRMED" ? 409 : 422, code, messages[code] ?? "无法核验 DAO 链上成员状态");
}
