...
export async function readDaoRewards(
  provider: Pick<DaoCaseRpcProvider, "getNetwork" | "getBlockNumber" | "getBlock" | "call">,
  config: Readonly<{ chainId: bigint; caseAddress: string; membershipAddress: string; ydTokenAddress: string; confirmations: number }>,
  actorId: string,
) {
  const actor = address(actorId);
  if (!Number.isSafeInteger(config.confirmations) || config.confirmations < 1 || config.chainId <= 0n) throw new Error("INVALID_REWARD_CONFIG");
  if ((await provider.getNetwork()).chainId !== config.chainId) throw new Error("REWARD_CHAIN_MISMATCH");
  ...
  const call = async (to: string, abi: Interface, method: string, args: readonly unknown[] = []) => {
    const raw = await provider.call({ to, data: abi.encodeFunctionData(method, args), blockTag: height });
  ...
  const [poolResult, memberResult] = await Promise.all([
    call(court, directory, "rewards"), call(court, directory, "membership"),
  ]);
  if (address(memberResult) !== address(config.membershipAddress)) throw new Error("REWARD_MEMBERSHIP_MISMATCH");
  const poolAddress = address(poolResult);
  const [tokenResult, amountResult] = await Promise.all([
    call(poolAddress, daoRewardInterface, "ydToken"), call(poolAddress, daoRewardInterface, "claimable", [actor]),
  ]);
  const ydTokenAddress = address(tokenResult);
  if (ydTokenAddress !== address(config.ydTokenAddress)) throw new Error("REWARD_TOKEN_MISMATCH");
  const decimals = z.bigint().parse(await call(ydTokenAddress, token, "decimals"));
  if (decimals !== 18n) throw new Error("REWARD_TOKEN_DECIMALS_MISMATCH");
  const claimableMinor = z.bigint().nonnegative().parse(amountResult).toString();
  if ((await provider.getBlock(height))?.hash !== block.hash) throw new Error("REWARD_BLOCK_REORG");
  return {
    status: "ready" as const, actorId: actor, chainId: config.chainId.toString(),
    caseAddress: court, poolAddress, ydTokenAddress, decimals: 18 as const,
    claimableMinor, blockNumber: height.toString(),
  };
}
