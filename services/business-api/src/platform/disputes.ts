import { transitionTaskStatus, type TaskStatus } from "./task-state";

export type DisputeParty = "publisher" | "agent";
export type DisputeStatus = "evidence_collection" | "decided" | "executed" | "cancelled";
export type DecisionType = "release" | "partial_release" | "refund";
export type DecisionExecutionStatus = "decided" | "submitted" | "executed" | "failed";
export type AgentResponsibility = "agent_at_fault" | "agent_not_at_fault" | "shared" | "not_determined";
...
export function openDispute(input: Readonly<{
  disputeId: string;
  actorId: string;
  reason: string;
  now: Date;
  ...
  const taskStatus = transitionTaskStatus(input.context.taskStatus, { type: "dispute_opened", disputeId: input.disputeId });
  return {
    dispute: {
      id: input.disputeId,
...
export function submitDisputeEvidence(input: Readonly<{
  evidenceId: string;
 ...
 */
export function decideDispute(input: Readonly<{
  decisionId: string;
  dispute: DisputeRecord;
  actorId: string;
  ...
  if (!input.actorRoles.has("arbitrator")) throw new Error("ARBITRATION_FORBIDDEN");
  if (input.dispute.status !== "evidence_collection") throw new Error("DISPUTE_ALREADY_DECIDED");
  if (input.reason.trim().length < 10) throw new Error("ARBITRATION_REASON_REQUIRED");
...
export function submitArbitrationExecution(decision: ArbitrationDecision, txHash: string): ArbitrationDecision {
  if (decision.executionStatus !== "decided" && decision.executionStatus !== "failed") {
  ...
  return { ...decision, executionStatus: "submitted", executionTxHash: txHash, executedAt: null };
}
...
export function confirmArbitrationExecution(
  ...
  if (decision.executionStatus !== "submitted" || decision.executionTxHash !== confirmedTxHash) {
    throw new Error("ARBITRATION_TRANSACTION_NOT_PENDING");
  }
  const taskEvent = decision.type === "refund"
    ? { type: "arbitration_refund_confirmed" as const, txHash: confirmedTxHash }
    : { type: "arbitration_release_confirmed" as const, txHash: confirmedTxHash };
  return {
    decision: { ...decision, executionStatus: "executed", executedAt: confirmedAt },
    taskStatus: transitionTaskStatus(taskStatus, taskEvent),
  };
}

/** 普通验收、超时退款等入口统一调用本函数，争议期间没有绕过冻结的特殊分支。 */
export function assertNormalEscrowOperationAllowed(taskStatus: TaskStatus): void {
...
function validatePayout(input: Readonly<{
  type: DecisionType;
  escrowAmountMinor: bigint;
  releaseAmountMinor: bigint;
  refundAmountMinor: bigint;
  ...
  if (input.escrowAmountMinor <= 0n || input.releaseAmountMinor < 0n || input.refundAmountMinor < 0n
    || input.releaseAmountMinor + input.refundAmountMinor !== input.escrowAmountMinor) {
    throw new Error("ARBITRATION_PAYOUT_NOT_CONSERVED");
  }
  if (input.type === "release" && (input.releaseAmountMinor !== input.escrowAmountMinor || input.refundAmountMinor !== 0n)) {
    throw new Error("ARBITRATION_PAYOUT_MISMATCH");
  }
  if (input.type === "refund" && (input.refundAmountMinor !== input.escrowAmountMinor || input.releaseAmountMinor !== 0n)) {
    throw new Error("ARBITRATION_PAYOUT_MISMATCH");
  }
  if (input.type === "partial_release" && (!input.partialReleaseEnabled || input.releaseAmountMinor === 0n || input.refundAmountMinor === 0n)) {
