...
import type { EscrowChainClient, ObservedEscrowEvent } from "./escrow-chain-client";
import type { EscrowRepository } from "./escrow-repository";
import { EscrowService, type EscrowRuntimeConfig } from "./escrow-service";
...
describe("EscrowService worker", () => {
  it("scans from the leased cursor and applies an event only at 12 confirmations", async () => {
    ...
    await expect(service.runWorker(new Date("2026-08-23T00:00:00Z"))).resolves.toMatchObject({
      leaseAcquired: true,
      fromBlock: "9",
      toBlock: "20",
      observed: 1,
  ...
  it("keeps an event pending before the threshold and exposes its current confirmations", async () => {
    const repository = fakeRepository({
      ...
      listPending: vi.fn(async () => [{ id: "event-2", blockNumber: 9n, blockHash: HASH_A }]),
    });
    const chain = fakeChain({ getHeadBlockNumber: vi.fn(async () => 19n) });

    await new EscrowService(repository, chain, CONFIG).runWorker();

    expect(repository.recordPendingCheck).toHaveBeenCalledWith(expect.objectContaining({ eventId: "event-2", confirmations: 11n }));
    expect(repository.applyCanonicalConfirmation).not.toHaveBeenCalled();
    expect(repository.releaseCursor).toHaveBeenCalledWith(31_337n, CONTRACT, "lease-2");
  });
  ...
  it("does no network or database mutation when another worker owns the cursor lease", async () => {
    ...
    await expect(new EscrowService(repository, chain, CONFIG).runWorker()).resolves.toMatchObject({ leaseAcquired: false });
  ...
  it("releases the lease after a provider failure so the next schedule can recover", async () => {
    const repository = fakeRepository({ claimCursor: vi.fn(async () => ({ token: "lease-3", nextBlock: 1n })) });
    const chain = fakeChain({ getHeadBlockNumber: vi.fn(async () => { throw new Error("RPC_UNAVAILABLE"); }) });
    await expect(new EscrowService(repository, chain, CONFIG).runWorker()).rejects.toThrow("RPC_UNAVAILABLE");
    expect(repository.releaseCursor).toHaveBeenCalledWith(31_337n, CONTRACT, "lease-3");
  });
});

function fakeRepository(overrides: Partial<EscrowRepository> = {}): EscrowRepository {
  return {
    prepareIntent: vi.fn(async () => { throw new Error("UNUSED"); }),
    recordSubmission: vi.fn(async () => { throw new Error("UNUSED"); }),
    markSubmissionFailed: vi.fn(async () => { throw new Error("UNUSED"); }),
    findOwnedStatus: vi.fn(async () => null),
    resetFailedIntent: vi.fn(async () => { throw new Error("UNUSED"); }),
    ...
    recordRefundFailure: vi.fn(async () => ({ number: 1, status: "retry_pending" as const, nextAttemptAt: new Date(), error: "failed" })),
    ...overrides,
  };
}

function fakeChain(overrides: Partial<EscrowChainClient> = {}): EscrowChainClient {
  return {
    chainId: 31_337n,
    contractAddress: CONTRACT,
