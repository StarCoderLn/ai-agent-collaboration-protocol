 ...
 * 各自持有连接的仓储对象。
 */
export class PgAgentRepository implements AgentRepository, AgentProfileRepository {
...
export class PgAgentCredentialStore {
  ...
  async replace(agentId: string, encryptedSecret: string): Promise<{ keyVersion: number; wasConfigured: boolean }> {
    const result = await this.db.query<{ key_version: number; inserted: boolean }>(
      `INSERT INTO agent_credentials (agent_id, encrypted_secret, key_version)
       VALUES ($1, $2, 1)
    ...
    const row = result.rows[0];
    if (!row) {
      throw new Error(`PgAgentCredentialStore.replace: agent ${agentId} 的凭证覆盖写未返回结果`);
    }
