...
发布者可以继续让 AI 按自由文本目标重新规划，或在 React Flow 中增删阶段和依赖。v2 规划
协议从 active Agent 目录读取真实分类及输入/输出契约，只把可执行的完整组合交给模型；确认
事务再次校验每个阶段至少有一个兼容 Agent，目录漂移时停在草案并指出具体阶段。模型和前端
都不能凭展示标签虚构契约，也不能直接写候选、报价或资金事实。

正式选人既支持逐阶段比较，也支持在候选区一键采用所有剩余阶段的第一推荐项。批量操作不接收
浏览器提交的 Agent 或金额，而是在一个数据库事务内锁定并读取各节点最新候选快照；已有人工
选择保持不变，任一阶段候选失效则全部回滚。选完后只冻结准确总价并进入待托管，不创建托管
意图、不扣款或派发。逐阶段选择成功后界面自动聚焦下一未选阶段，失败则留在原阶段处理。
