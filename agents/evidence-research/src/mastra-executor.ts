...
export class MastraResearchExecutor implements ResearchExecutor {
  ...
  async run(input: ResearchTaskInput, context: ResearchExecutionContext = {}) {
    ...
    const synthesisAgent = new Agent({
      id: "evidence-research-agent",
      // name 是面向用户/调试界面的展示名；id 是协议和代码使用的稳定标识，两者不要混用。
      name: "论文调研报告 Agent",
      model: this.#model,
      instructions: [
        // Prompt 是第一道约束；finalizeReport 的代码级白名单是不可由模型绕过的第二道约束。
        "You create evidence-grounded research reports, not publishable academic claims.",
...
export function buildSynthesisPrompt(
  ...
  return [
    ...
    "Return an evidence synthesis with at least two sections and explicit limitations.",
