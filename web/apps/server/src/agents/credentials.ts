 ...
 *
 * 与 `patch-agent.ts`（T-004）一致地与具体 HTTP 框架解耦（不导入 Hono 类型），只依赖
...
export async function replaceAgentCredentials(
	...
	const { encryptedSecret } = await deps.credentialEncryptor.encryptCredential(
		parsed.data.credentialSecret,
	);
	const { keyVersion, wasConfigured } = await deps.credentialStore.replace(
		agentId,
		encryptedSecret,
	);
