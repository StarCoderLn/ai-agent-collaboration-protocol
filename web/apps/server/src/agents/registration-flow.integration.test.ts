...
import type {
	AgentRepository as CreateAgentRepository,
	CreatedAgent,
} from "./agent-repository.js";
import { type CreateAgentDeps, createAgent } from "./create-agent.js";
import {
 ...
 *
 * 说明：web/apps/server 尚未有可用的 Postgres/AWS KMS 测试环境（无 DATABASE_URL、
...
function makeFakeKms(): KmsLike {
	const send = vi.fn(
		async (
			_command: GenerateDataKeyCommand,
...
class InMemoryIdempotencyStore implements IdempotencyStore {
	private readonly records = new Map<
		string,
		{ operationType: string; snapshot: ResponseSnapshot | null }
	>();
	...
	async reserve(key: string, operationType: string, _expiresAt: Date) {
		...
		if (existing === undefined) {
			throw new Error("幂等记录在同一同步调用中意外消失");
		}
	...
	async commitResponse(key: string, snapshot: ResponseSnapshot) {
		...
		if (!existing || existing.snapshot !== null) {
			throw new Error(
				`InMemoryIdempotencyStore.commitResponse: key "${key}" 未处于 pending 状态`,
			);
		}
...
function makeCreateDeps(overrides?: {
	...
	repository: InMemoryCreateAgentRepository | CreateAgentRepository;
} {
	const repository =
		overrides?.repository ?? new InMemoryCreateAgentRepository();
	const idempotency = new Idempotency(
		overrides?.idempotencyStore ?? new InMemoryIdempotencyStore(),
	);
	const auditLogWriter =
		overrides?.auditLogWriter ?? new InMemoryAuditLogWriter();
	return {
		repository,
...
describe("Agent 注册集成流程：必填与兼容字段校验", () => {
	const REQUIRED_FIELD_CASES: Array<{
		field: string;
		...
		{
			field: "payoutWalletAddress",
			overrides: { payoutWalletAddress: "not-an-address" },
		},
	...
	for (const { field, overrides } of REQUIRED_FIELD_CASES) {
		it(`rejects creation with 422 VALIDATION_FAILED when "${field}" is missing/invalid (${JSON.stringify(overrides)})`, async () => {
			...
			await expect(
				createAgent(body, "idem-key-1", WALLET_ADDRESS, deps),
			).rejects.toMatchObject({
				httpStatus: 422,
				body: { error_code: "VALIDATION_FAILED" },
			});
			expect(
				(deps.repository as InMemoryCreateAgentRepository).rows,
			).toHaveLength(0);
		});
	}
	...
	it("rejects creation when tags is entirely absent from the request body", async () => {
		...
		await expect(
			createAgent(body, "idem-key-tags", WALLET_ADDRESS, deps),
		).rejects.toMatchObject({
			httpStatus: 422,
			body: { error_code: "VALIDATION_FAILED" },
		});
	...
	it("accepts a fully valid submission and persists the row", async () => {
		...
		expect(
			(deps.repository as InMemoryCreateAgentRepository).rows[0]
				?.payoutWalletAddress,
		).toBe(PAYOUT_WALLET_ADDRESS);
	});
...
describe("Agent 注册集成流程：凭证不可明文读取", () => {
	it("never persists the plaintext credentialSecret anywhere in the stored row", async () => {
		...
		expect(Buffer.from(row.encryptedSecret, "base64").length).toBeGreaterThan(
			plaintext.length,
		);
	});
	...
	it("does not include the plaintext or ciphertext secret in the HTTP response body", async () => {
		...
		expect(Object.keys(result.body as Record<string, unknown>).sort()).toEqual([
			"agentId",
			"status",
		]);
	});
...
class InMemoryAgentCredentialStore implements AgentCredentialStore {
	private readonly byAgentId = new Map<
		string,
		{ encryptedSecret: string; keyVersion: number }
	>();
...
describe("凭证替换集成流程：审计日志写入 + 不可明文读取", () => {
	function makeCredentialDeps(agent: Agent) {
		...
		const credentialEncryptor: CredentialEncryptor = {
			encryptCredential: (plaintext: string) =>
				encryptor.encryptCredential(plaintext),
		};
		return {
			agentRepository,
			credentialStore,
	...
	it("stores only ciphertext and writes an audit entry that never contains the secret", async () => {
		...
		if (stored === undefined) throw new Error("凭证替换后缺少持久化记录");
	...
	it("increments key_version and marks wasConfigured=true on a second replace", async () => {
		...
		expect(deps.auditLogWriter.entries[1]?.beforeSummary).toEqual({
			configured: true,
		});
	});
...
describe("Agent 注册集成流程：重复提交幂等", () => {
	it("replays the exact same response on a duplicate submission without writing a second row", async () => {
		...
		const deps = {
			repository,
			encryptor: makeEncryptor(),
			idempotency: new Idempotency(store),
	...
	it("rejects a concurrent duplicate (same key, prior request still pending) with a retryable 409", async () => {
		...
		await expect(
			createAgent(validCreateBody(), "idem-key-inflight", WALLET_ADDRESS, deps),
		).rejects.toMatchObject({
			httpStatus: 409,
	...
	it("treats a missing Idempotency-Key as a hard validation error before touching the repository", async () => {
		...
		await expect(
			createAgent(validCreateBody(), undefined, WALLET_ADDRESS, deps),
		).rejects.toMatchObject({
			httpStatus: 400,
