 ...
 *
 * 框架无关（不依赖 Hono 或 AWS Lambda 具体类型），与 T-002 `EnvelopeEncryptor` 同一模式：
 * 依赖通过构造参数注入，供任意 HTTP 入口（Hono / Lambda handler）调用，
 * 也便于单元测试不依赖真实数据库、KMS 或幂等存储。正式 Hono 路由已经装配
...
export async function createAgent(
	...
	const encrypted =
		parsed.data.credentialSecret === undefined
			? null
