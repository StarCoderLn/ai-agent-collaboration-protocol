...
import { type PatchAgentDeps, patchAgent } from "./patch-agent.js";
...
function makeDeps(agent: Agent | null): {
	...
	return {
		deps: { agentRepository, auditLogWriter },
		findById,
		applyPatch,
		writeAudit,
	};
...
describe("patchAgent", () => {
	it("updates allowed fields and writes an audit log scoped to changed fields only", async () => {
		...
		expect(entry.beforeSummary).toEqual({
			name: "Original Name",
			email: "provider@example.com",
		});
		expect(entry.afterSummary).toEqual({
			name: "New Name",
			email: "new@example.com",
		});
	...
	it("rejects providerWalletAddress key as well as walletAddress", async () => {
		...
		await expect(
			patchAgent(deps, {
				...
				actorId: agent.providerWalletAddress,
				rawBody: {
					providerWalletAddress: "0x0000000000000000000000000000000000dEaD",
				},
			}),
		).rejects.toMatchObject({ code: "WALLET_ADDRESS_IMMUTABLE" });
	});
