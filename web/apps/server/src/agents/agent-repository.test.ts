...
describe("PgAgentRepository payout wallet", () => {
	it("公开快速 HTTP Agent 不创建伪凭证记录", async () => {
		...
		await new PgAgentRepository(db).createAgentWithCredential(
			createInput(),
			null,
		);
		...
		expect(
			query.mock.calls.some(([sql]) =>
				String(sql).includes("INSERT INTO agent_credentials"),
			),
		).toBe(false);
	});
	...
	it("persists NULL instead of inventing a contact email for new registrations", async () => {
		...
		if (insertCall === undefined)
			throw new Error("测试必须执行 Agent 档案写入");
	...
	it("persists owner and payout addresses in distinct columns", async () => {
		...
		expect(insertSql).toContain(
			"provider_wallet_address, payout_wallet_address",
		);
