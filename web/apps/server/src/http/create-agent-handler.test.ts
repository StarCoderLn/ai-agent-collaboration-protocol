...
import {
	type CreateAgentHttpDeps,
	createAgentHttpHandler,
} from "./create-agent-handler.js";
...
function makeRequest(body: unknown, headers?: Record<string, string>): Request {
	return new Request("https://marketplace-api.internal/api/agents", {
		method: "POST",
		headers: {
...
describe("createAgentHttpHandler", () => {
	...
	it("returns 403 WALLET_OWNERSHIP_MISMATCH when walletAddress does not match the authenticated actor", async () => {
		...
		const response = await handler(
			makeRequest(validBody({ walletAddress: WALLET_ADDRESS })),
		);
