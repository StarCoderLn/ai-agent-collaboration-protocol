...
export type DispatchTransitionRouteContext = Readonly<{
...
export function createDispatchTransitionHandler(
	deps: DispatchTransitionHttpDeps,
) {
	return async (
		request: Request,
		...
		if (!isUuid(id))
			return errorResponse(404, "TASK_NOT_FOUND", "任务不存在", false);
