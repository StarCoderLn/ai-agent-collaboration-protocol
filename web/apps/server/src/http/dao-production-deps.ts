...
export function loadDaoChainRuntimeConfigFromEnv() {
	const chainId = positiveBigint(
		...
		getRequiredEnv("ARBITRATION_DAO_CHAIN_ID"),
	);
	const escrowChainId = positiveBigint(
		"ESCROW_CHAIN_ID",
		getRequiredEnv("ESCROW_CHAIN_ID"),
	);
	if (chainId !== escrowChainId)
		throw new Error("DAO 与 Escrow 必须配置在同一条链");
	return Object.freeze({
		...
		foundingArbitrators: normalizeFoundingArbitrators(
			getOptionalEnv("DAO_FOUNDING_ARBITRATOR_ADDRESSES", "").trim() === ""
				? []
				: getOptionalEnv("DAO_FOUNDING_ARBITRATOR_ADDRESSES", "")
						.split(",")
...
export function createProductionDaoDeps(
	resolveActorId: DaoHttpDeps["resolveActorId"],
): DaoHttpDeps {
	...
	return {
		...
		service: {
			candidatePool: () =>
				required(service, "DAO_SERVICE_NOT_CONFIGURED").candidatePoolOverview(),
			overview: (actorId) =>
				required(service, "DAO_SERVICE_NOT_CONFIGURED").overview(actorId),
			sync: (actorId, raw, key) =>
				idempotent(key, `dao.membership.sync:${actorId.toLowerCase()}`, () =>
					required(service, "DAO_SERVICE_NOT_CONFIGURED").syncMembership(
						actorId,
						raw,
						new Date(),
					),
				),
			vote: (roundId, actorId, raw, key) =>
				idempotent(key, `dao.vote:${roundId}:${actorId.toLowerCase()}`, () =>
					required(service, "DAO_SERVICE_NOT_CONFIGURED").vote(
						roundId,
						actorId,
						raw,
						new Date(),
...
async function idempotent(
	...
	if (reservation.existing !== null)
		return objectBody(reservation.existing.body);
