 ...
 * 事务收敛）。
 *
 * 使用 Web 标准 `Request`/`Response`（Hono 与主流 AWS Lambda
 ...
 * 任一步失败都必须整体回滚，不留半成品记录（AGENTS.md「Marketplace API 工程教训」：
...
export function createAgentHttpHandler(deps: CreateAgentHttpDeps) {
	return async function handleCreateAgent(request: Request): Promise<Response> {
		...
		const idempotencyKey =
			request.headers.get(IDEMPOTENCY_KEY_HEADER) ?? undefined;
		...
		try {
			const result = await deps.runInTransaction<CreateAgentHttpResult>(
				(txDeps) => createAgent(rawInput, idempotencyKey, actorId, txDeps),
			);
			return withCredentialedCors(
			...
			if (err instanceof ApiError) {
				return withCredentialedCors(
					jsonResponse(err.httpStatus, err.body),
					deps.allowedOrigin,
				);
			}
