 ...
 * 共享同一个事务，任一步失败整体回滚（AGENTS.md「Marketplace API 工程教训」：
...
function buildTransactionalDeps(
	...
	encryptor: CreateAgentDeps["encryptor"],
): CreateAgentDeps {
	return {
 ...
 */
export function createProductionCreateAgentDeps(): Pick<
	CreateAgentHttpDeps,
...
> {
	...
	const encryptor = {
		encryptCredential(plaintextSecret: string) {
			envelopeEncryptor ??= new EnvelopeEncryptor({
	...
	return {
		runInTransaction: (fn) =>
			withTransaction(pool, (client) =>
				fn(buildTransactionalDeps(client, encryptor)),
			),
