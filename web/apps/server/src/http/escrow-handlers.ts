...
export function createPublisherEscrowHandlers(deps: PublisherEscrowHttpDeps) {
	return {
		prepare: (request: Request, context: EscrowRouteContext) =>
			withActorAndTask(request, context, deps, (taskId, actorId) =>
				deps.prepare(taskId, actorId),
			),
		status: (request: Request, context: EscrowRouteContext) =>
			withActorAndTask(request, context, deps, (taskId, actorId) =>
				deps.status(taskId, actorId),
			),
		retry: (request: Request, context: EscrowRouteContext) =>
			withActorAndTask(request, context, deps, (taskId, actorId) =>
				deps.retry(taskId, actorId),
			),
		submission: async (
			request: Request,
...
export function createInternalEscrowWorkerHandler(
	deps: InternalEscrowHttpDeps,
) {
...
function internalFailure(
	...
	return Response.json(
		{ error_code: code, message: "链上托管同步失败", retryable },
		{ status },
	);
}
