...
export type AgentLifecycleRouteContext = Readonly<{
...
export function createAgentLifecycleHandlers(deps: AgentLifecycleHttpDeps) {
	return {
		pause: (request: Request, context: AgentLifecycleRouteContext) =>
			providerTransition(deps, request, context, "manual_pause"),
		resume: (request: Request, context: AgentLifecycleRouteContext) =>
			providerTransition(deps, request, context, "manual_resume"),
		delist: (request: Request, context: AgentLifecycleRouteContext) =>
			providerTransition(deps, request, context, "provider_delist"),
	};
}
...
async function providerTransition(
	...
	return callTransition(deps, () =>
		deps.transitionAgent(target.agentId, actor, event, target.idempotencyKey),
	);
}
...
async function targetAndKey(
	...
	if (!isUuid(id))
		return response(
			deps,
			404,
			errorBody("AGENT_NOT_FOUND", "Agent 不存在或无权访问", false),
		);
...
async function callTransition(
	deps: AgentLifecycleHttpDeps,
	call: () => Promise<TaskServiceResult>,
): Promise<Response> {
	try {
