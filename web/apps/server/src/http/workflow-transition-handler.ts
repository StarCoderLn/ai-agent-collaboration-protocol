...
export function createWorkflowTransitionHandler(
	deps: WorkflowTransitionHttpDeps,
) {
	return async (
		request: Request,
		...
		try {
			return Response.json(serialize(await deps.apply(id, nodeId, rawInput)), {
				status: 200,
			});
		} catch (error) {
			...
			return errorResponse(
				500,
				"WORKFLOW_TRANSITION_INTERNAL_ERROR",
				"工作节点状态迁移失败",
				true,
			);
		}
...
function serialize(value: AppliedWorkflowTransition) {
	return {
		...value,
		nodeVersion: value.nodeVersion.toString(),
		runVersion: value.runVersion.toString(),
	};
}
