 ...
 *
 * `web/apps/server` 与唯一正式前端 `web/apps/web` 是不同源部署（design.md
...
export function withCredentialedCors(
	response: Response,
 ...
 * 先发一次 `OPTIONS` 预检；路由模块若没有导出 `OPTIONS`，会返回默认响应，
 * 不带任何 CORS 头，预检失败导致真正的请求根本发不出去（本地 vitest/静态构建测不到
