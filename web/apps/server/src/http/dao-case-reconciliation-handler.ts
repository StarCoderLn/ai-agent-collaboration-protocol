...
export function createDaoCaseReconciliationHandler(
	deps: DaoCaseReconciliationHttpDeps,
) {
	return async (request: Request): Promise<Response> => {
		...
		try {
			const result =
				parsed.data.operation === "retry_reverted_command"
					? await deps.retryReverted(parsed.data)
					: await deps.resolveFinalReorg(parsed.data);
			return Response.json(result, { status: 200 });
		} catch (error) {
			if (error instanceof DaoCaseRecoveryError)
