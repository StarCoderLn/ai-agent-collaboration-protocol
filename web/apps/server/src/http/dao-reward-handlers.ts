 ...
 */
export function createDaoRewardsHandler(
	deps: Readonly<{
		...
		try {
			...
			if (request.method === "PATCH") {
				...
				try {
					...
					return response({ error_code: "VALIDATION_FAILED" }, 400);
				}
				const parsed = z
					.object({ ids: z.array(z.uuid()).min(1).max(10) })
					.strict()
					.safeParse(raw);
				if (!parsed.success || deps.markRead === undefined)
					return response({ error_code: "VALIDATION_FAILED" }, 422);
			...
			if (error instanceof SessionInvalidError)
				return response({ error_code: "UNAUTHENTICATED" }, 401);
