...
export function createDaoRewardReconciliationHandler(
	deps: DaoRewardReconciliationHttpDeps,
) {
	return async (request: Request): Promise<Response> => {
		...
		try {
			return Response.json(await deps.resolveCursorReorg(parsed.data), {
				status: 200,
			});
		} catch (error) {
			if (error instanceof DaoRewardRecoveryError)
				return failure(error.statusCode, error.code, false, error.message);
