...
function makeDeps(
	...
	const agentRepository: AgentRepository = {
		...
		applyPatch: vi.fn(async () => {
			throw new Error(
				"applyPatch should not be called by the credentials handler",
			);
		}),
	};

	const encryptCredential = vi.fn(async (plaintext: string) => ({
		encryptedSecret: `enc(${plaintext})`,
	}));
	const credentialEncryptor: CredentialEncryptor = { encryptCredential };

	const replace = vi.fn(
		async (_agentId: string, _encryptedSecret: string) => storeResult,
	);
...
function makeRequest(body: unknown): Request {
	return new Request(
		"https://marketplace-api.internal/api/agents/agent-1/credentials",
		{
			method: "PUT",
...
describe("createReplaceCredentialsHttpHandler", () => {
	it("wires an authenticated owner's request through to a 200 with keyVersion and configured=true", async () => {
		...
		const response = await handler(
			makeRequest({ credentialSecret: "top-secret-key" }),
			{
				params: { id: agent.id },
			},
		);
	...
	it("returns 403 AGENT_ACCESS_DENIED when the authenticated actor does not own the agent", async () => {
		...
		const response = await handler(
			makeRequest({ credentialSecret: "hijacked-secret" }),
			{
				params: { id: agent.id },
			},
		);
	...
	it("returns 404 AGENT_NOT_FOUND when the agent does not exist", async () => {
		...
		const response = await handler(
			makeRequest({ credentialSecret: "secret" }),
			{
				params: { id: "missing" },
			},
		);
