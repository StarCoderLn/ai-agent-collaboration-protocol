...
export function createDisputeHandlers(deps: DisputeHttpDeps) {
	return {
		open: (request: Request, context: DisputeRouteContext) =>
			writeRequest(request, context, deps, (id, raw, actor, key) =>
				deps.service.open(id, raw, actor, key),
			),
		evidence: (request: Request, context: DisputeRouteContext) =>
			writeRequest(request, context, deps, (id, raw, actor, key) =>
				deps.service.submitEvidence(id, raw, actor, key),
			),
		decision: (request: Request, context: DisputeRouteContext) =>
			writeRequest(request, context, deps, (id, raw, actor, key) =>
				deps.service.decide(id, raw, actor, key),
			),
		read: async (
			request: Request,
			...
			if (id === null)
				return failure(deps, 404, "DISPUTE_NOT_FOUND", "争议不存在", false);
			try {
				return serviceResult(deps, await deps.service.read(id, actor));
			} catch (error) {
				return disputeFailure(deps, error);
			}
		},
	};
}
...
async function writeRequest(
	...
	) => Promise<{ statusCode: number; body: Readonly<Record<string, unknown>> }>,
): Promise<Response> {
	const actor = await resolveActor(request, deps);
	if (actor instanceof Response) return actor;
	const id = await routeId(context);
	if (id === null)
		return failure(deps, 404, "RESOURCE_NOT_FOUND", "资源不存在", false);
	let raw: unknown;
	try {
...
function disputeFailure(deps: DisputeHttpDeps, error: unknown): Response {
	if (
		...
		error instanceof DisputeServiceError
	) {
...
function serviceResult(
	deps: DisputeHttpDeps,
	result: { statusCode: number; body: Readonly<Record<string, unknown>> },
): Response {
