...
describe("workflow handlers", () => {
	...
	it("认证失败不调用仓储", async () => {
		const dependencies = deps({
			resolveActorId: vi.fn(async () => {
				throw new SessionInvalidError("invalid");
			}),
		});
		const response = await createWorkflowHandlers(dependencies).detail(
			new Request(`http://api.local/api/tasks/${taskId}/workflow`),
	...
	it("不存在与越权统一返回任务不可见", async () => {
		const response = await createWorkflowHandlers(
			deps({
		...
		).detail(new Request(`http://api.local/api/tasks/${taskId}/workflow`), {
			params: Promise.resolve({ id: taskId }),
		});
	...
	it("只把预算偏好和幂等证据交给工作流命令，不把它解释为托管金额", async () => {
		...
		expect(await response.json()).toMatchObject({
			budgetPreferenceMinor: "50000000",
		});
	});
	...
	it("校验节点路径并把能力修正交给独立命令", async () => {
		...
		const response = await createWorkflowHandlers(
			...
			new Request(
				`http://api.local/api/tasks/${taskId}/workflow-nodes/${nodeId}/preferences`,
				{
					method: "PATCH",
					headers: {
						"content-type": "application/json",
						"idempotency-key": "capability-pref-1",
					},
