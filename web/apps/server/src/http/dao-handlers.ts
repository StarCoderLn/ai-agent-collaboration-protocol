...
export function createDaoHandlers(deps: DaoHttpDeps) {
	return {
		...
		candidatePool: async (): Promise<Response> => {
			try {
				return success(deps, await deps.service.candidatePool(), 200);
			} catch (error) {
				return failureOf(deps, error);
			}
		},
		overview: async (request: Request): Promise<Response> => {
			...
			try {
				return success(deps, await deps.service.overview(actor), 200);
			} catch (error) {
				return failureOf(deps, error);
			}
		},
...
function failureOf(deps: DaoHttpDeps, error: unknown): Response {
	if (error instanceof DaoServiceError) {
		return response(deps, error.statusCode, {
			error_code: error.code,
			message: error.message,
			retryable: false,
		});
	}
	return response(deps, 500, {
		error_code: "DAO_INTERNAL_ERROR",
		message: "DAO 服务暂时不可用",
		retryable: true,
	});
}
