...
export type AgentAdmissionRouteContext = Readonly<{
...
export function createAgentAdmissionHandlers(deps: AgentAdmissionHttpDeps) {
	return {
		retry: (request: Request, context: AgentAdmissionRouteContext) =>
			retry(deps, request, context),
	};
}
...
async function retry(
	...
	try {
		...
		return response(
			deps,
			503,
			errorBody("AGENT_ADMISSION_UNAVAILABLE", "自动验证服务暂不可用", true),
		);
	}
}
