...
function publisherDeps(
	overrides: Partial<PublisherExecutionHttpDeps> = {},
...
describe("execution HTTP boundaries", () => {
	it("requires the internal service token before applying an Agent callback", async () => {
		...
		const response = await createInternalExecutionHandlers(deps).reportStatus(
			new Request(
				`http://api.local/api/internal/tasks/${TASK_ID}/execution/status`,
				{ method: "POST", body: "{}" },
			),
			...
			new Request(
				`http://api.local/api/internal/tasks/${TASK_ID}/execution/status`,
				{
					...
					headers: {
						...
						"idempotency-key": "status:task:request",
					},
					body,
				},
			),
	...
	it("reads acceptance terms only for the authenticated publisher and requested result", async () => {
		...
		const response = await createPublisherExecutionHandlers(
			deps,
		).previewAcceptance(
			new Request(
				`http://api.local/api/tasks/${TASK_ID}/acceptance-preview?resultId=${resultId}`,
			),
			{ params: Promise.resolve({ id: TASK_ID }) },
		);
		expect(response.status).toBe(200);
		expect(deps.previewAcceptance).toHaveBeenCalledWith(
			TASK_ID,
			{ resultId },
			"publisher-1",
		);
