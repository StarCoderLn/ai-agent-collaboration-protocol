...
describe("HttpAgentConnectionProbe", () => {
	it("本地模式允许公开 HTTP Agent，并固定探测同域 /healthz", async () => {
		...
		const result = await new HttpAgentConnectionProbe(true).probe({
	...
	it("有访问密钥时使用标准 Bearer 认证", async () => {
		const endpoint = await startAgentServer({
			expectedAuthorization: "Bearer provider-token",
		});

		await expect(
			new HttpAgentConnectionProbe(true).probe({
				serviceEndpoint: endpoint,
	...
	it("把 401 响应转换为可读的访问密钥错误", async () => {
		const endpoint = await startAgentServer({
			expectedAuthorization: "Bearer correct-token",
		});
	...
	it("拒绝不符合健康检查契约的 JSON", async () => {
		...
		await expect(
			new HttpAgentConnectionProbe(true).probe({ serviceEndpoint: endpoint }),
	...
	it("正式模式拒绝 HTTP 与私网目标", async () => {
		...
		await expect(
			probe.probe({ serviceEndpoint: "http://agent.example.com/run" }),
		).rejects.toMatchObject({ code: "AGENT_ENDPOINT_HTTPS_REQUIRED" });
		await expect(
			probe.probe({ serviceEndpoint: "https://127.0.0.1/run" }),
