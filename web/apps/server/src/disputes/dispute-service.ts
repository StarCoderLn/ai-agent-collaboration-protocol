...
export function createDisputeService(
	...
	return {
		open: (
			...
			write(key, `task.dispute.open:${taskId}`, idempotency, () =>
		...
		submitEvidence: (
			...
			write(key, `dispute.evidence.submit:${disputeId}`, idempotency, () =>
				repository.submitEvidence(
					disputeId,
					actorId,
					parsed(submitEvidenceSchema, raw),
					now(),
				),
			),
		decide: (
			...
			write(key, `dispute.decision.create:${disputeId}`, idempotency, () =>
				repository.decide(
					disputeId,
					actorId,
					parsed(decideDisputeSchema, raw),
					now(),
				),
			),
		read: (disputeId: string, actorId: string) =>
			repository.read(disputeId, actorId),
	};
}
...
async function write(
	...
	if (idempotency === undefined)
		throw new Error("DISPUTE_IDEMPOTENCY_NOT_CONFIGURED");
	...
	if (reservation.existing !== null) {
		return {
			...
			body: objectBody(reservation.existing.body),
		};
	}
...
function parsed<Output>(
	...
	if (!result.success)
		throw new DisputeServiceError(422, "VALIDATION_FAILED", "请求字段格式无效");
	return result.data;
}
