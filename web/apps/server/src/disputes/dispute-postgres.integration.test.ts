...
import {
	type ObservedEscrowEvent,
	taskKeyForTaskId,
} from "../escrow/escrow-chain-client";
...
integration("dispute PostgreSQL and escrow execution vertical slice", () => {
	let sharedPool: Pool;
	beforeAll(() => {
		sharedPool = new Pool({ connectionString: requiredDatabaseUrl() });
	});
	afterAll(async () => {
	...
	it("允许正式工作流从粗粒度 matching 投影发起争议", async () => {
		...
		try {
			...
			await pool.query("UPDATE tasks SET status='matching' WHERE id=$1", [
				fixture.taskId,
			]);

			await expect(
				disputeWrite(pool, (service) =>
					service.open(
						fixture.taskId,
						{
	...
	it("freezes normal settlement, audits a role-checked decision, and executes only after chain confirmation", async () => {
		...
		try {
			...
			const opened = await disputeWrite(pool, (service) =>
				service.open(
					fixture.taskId,
					{
			...
			await expect(
				disputeWrite(pool, (service) => service.read(disputeId, PUBLISHER)),
			).resolves.toMatchObject({
				body: { escrowAmountMinor: "10000000", viewerRole: "publisher" },
			});
			await expect(
				pool.query("SELECT status FROM escrow_execution_jobs WHERE id=$1", [
					fixture.acceptanceJobId,
				]),
			...
			).resolves.toMatchObject({ body: { party: "agent" } });
			await expect(
				disputeWrite(pool, (service) => service.read(disputeId, AGENT_WALLET)),
			).resolves.toMatchObject({
				body: { viewerRole: "agent", fundsFrozen: true },
			});
				...
				disputeWrite(pool, (service) =>
					service.decide(
						disputeId,
						{
							...
							refundAmountMinor: "10000000",
							agentResponsibility: "agent_at_fault",
							reason: "关键验收条件未满足，决定全额退回发布者。",
						},
						PUBLISHER,
						forbiddenKey,
					),
				),
			).rejects.toMatchObject({ code: "ARBITRATION_FORBIDDEN" });
			await pool.query(
				"INSERT INTO platform_actor_roles(actor_id,role,granted_by) VALUES ($1,'arbitrator','integration-test')",
				[arbitrator],
			);

			const decided = await disputeWrite(pool, (service) =>
				service.decide(
					disputeId,
					{
						type: "refund",
			...
			await expect(
				new EscrowExecutionWorker(pool, operator, {
					leaseMs: 60_000,
					maxAttempts: 3,
					baseRetryMs: 1_000,
				}).runOne(new Date("2026-08-23T02:00:00Z")),
			).resolves.toMatchObject({ status: "submitted", txHash: TX_HASH });
           ...
           JOIN arbitration_decisions decision ON decision.dispute_id=dispute.id WHERE task.id=$1`,
			...
			const pending = required(
				(await escrow.listPending(31_337n, CONTRACT, 10))[0],
			);
			await expect(
          ...
          WHERE task.id=$1`,
			...
			expect(afterConfirmation.rows[0]).toEqual({
				...
				job_status: "executed",
				audit_actions:
					"dispute.decision.create,dispute.evidence.submit,dispute.execution.confirmed,dispute.execution.submitted,dispute.open",
			});
			await expect(
				disputeWrite(pool, (service) =>
					service.read(disputeId, `0x${"99".repeat(20)}`),
				),
...
async function disputeWrite<T>(
	pool: EscrowDatabase,
	action: (service: ReturnType<typeof createDisputeService>) => Promise<T>,
): Promise<T> {
