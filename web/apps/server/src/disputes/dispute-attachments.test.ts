...
describe("争议附件复用任务安全规则", () => {
	...
	it.each(["1", "1024"])(
		...
		async (sizeBytes) => {
			await expect(
				submit(createRepository(), "submitEvidence", {
					description: "原始执行证据",
					attachments: [{ ...attachment, sizeBytes }],
				}),
			).resolves.toMatchObject({
				statusCode: 201,
				body: { party: "publisher" },
			});
		},
	);
	...
	it.each([
		{
			attachments: [{ ...attachment, sizeBytes: "1025" }],
			code: "EVIDENCE_ATTACHMENT_INVALID",
		},
		{
		...
		},
		{
			...
			code: "EVIDENCE_ATTACHMENT_LIMIT",
		},
	])(
		"继续按当前分类拒绝超限或非白名单附件 %j",
		async ({ attachments, code }) => {
			await expect(
				submit(createRepository(), "submitEvidence", {
					description: "原始执行证据",
					attachments,
...
function submit(
	repository: PgDisputeRepository,
	entry: "open" | "submitEvidence",
	...
	return entry === "open"
		? repository.open(
				taskId,
				publisher,
				{ reason: "交付结果未达到约定验收要求", initialEvidence: input },
				now,
			)
...
function createRepository(): PgDisputeRepository {
	...
	query.mockImplementation(async (sql) => {
		...
		if (sql.includes("attachment_category_limits"))
			return {
				rows: [
					{
						max_files: 2,
						max_file_size_bytes: "1024",
						allowed_mime_types: ["application/pdf", "application/x-executable"],
					},
				],
				rowCount: 1,
			};
		if (sql.includes("CROSS JOIN dispute_config"))
			return {
				rows: [
					{
						status: "executing",
						...
						agent_provider_ids: [`0x${"22".repeat(20)}`],
						evidence_window_seconds: 3600,
					},
				],
				rowCount: 1,
			};
		if (sql.includes("FOR UPDATE OF dispute,task"))
			return {
				rows: [
					{
						id: disputeId,
						...
						evidence_deadline: new Date(now.getTime() + 3600_000),
						funds_frozen: true,
						created_at: now,
						updated_at: now,
						publisher_id: publisher,
	...
	return new PgDisputeRepository(db, {
		chainId: 31337n,
