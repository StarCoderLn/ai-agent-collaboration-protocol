...
export type EditableTaskDraft = Readonly<
	Omit<TaskDraft, "categoryId" | "deadline"> & {
		categoryId: string | null;
		deadline: Date | null;
	}
>;

export type TaskFieldError = Readonly<{
	field: string;
	code: string;
	message: string;
}>;
...
const EXECUTABLE_EXTENSIONS =
	/\.(?:exe|dll|dmg|pkg|app|sh|bat|cmd|com|msi|jar)$/i;
...
export function validateTaskDraft(
	...
	if ([...input.title.trim()].length < 6)
		errors.push(error("title", "TITLE_TOO_SHORT", "标题至少需要 6 个字符"));
	// 用户可以只用标题启动需求澄清流程；说明字段只保证非空，短需求的结构化与补全由
	// 首个 PRD Agent 负责，不能把 Agent 的工作提前转嫁给任务发布者。
	if (input.description.trim().length === 0)
		errors.push(
			error("description", "DESCRIPTION_REQUIRED", "任务说明不能为空"),
		);
	...
	if (input.deliverableFormat.trim().length === 0)
		errors.push(
			error(
				"deliverableFormat",
				"DELIVERABLE_FORMAT_REQUIRED",
				"请填写交付格式",
			),
		);
	if (input.requiredCapability.trim().length === 0)
		errors.push(
			error(
				"requiredCapability",
				"REQUIRED_CAPABILITY_REQUIRED",
				"请填写所需能力",
			),
		);
	...
	if (input.deadline.getTime() < now.getTime() + config.minExecutionPeriodMs) {
		errors.push(
			error(
				"deadline",
				"DEADLINE_TOO_SOON",
				`截止时间至少应晚于当前 ${Math.ceil(config.minExecutionPeriodMs / 60_000)} 分钟`,
			),
		);
	}
	...
	errors.push(
		...validateMatchingTags(input.tags, config.forbiddenTags).map((issue) =>
			error("tags", issue.code, issue.message),
		),
	);
	errors.push(
		...validateTaskAttachments(input.attachments, config.attachmentLimit),
	);
	return errors;
}
 ...
 */
export function inspectTaskDraftCompleteness(
	input: EditableTaskDraft,
): CompleteTaskDraftResult {
	const issues: TaskFieldError[] = [];
	if (input.categoryId === null)
		issues.push(error("categoryId", "CATEGORY_REQUIRED", "请选择任务分类"));
	if (input.deadline === null)
		issues.push(error("deadline", "DEADLINE_REQUIRED", "请填写任务截止时间"));
	if (
		issues.length > 0 ||
	...
	return {
		success: true,
		draft: { ...input, categoryId: input.categoryId, deadline: input.deadline },
	};
}
...
function validatePricing(
	...
	if (pricing.type === "fixed") {
		if (
			pricing.amountMinor < config.minBudgetMinor ||
			pricing.amountMinor > config.maxBudgetMinor
		) {
			errors.push(
				error(
					"pricing.amountMinor",
	...
	if (
		pricing.minAmountMinor < config.minBudgetMinor ||
		pricing.maxAmountMinor > config.maxBudgetMinor ||
...
function validateAttachments(
	...
	for (const [index, attachment] of attachments.entries()) {
		...
		if (
			...
			attachment.sizeBytes > limit.maxFileSizeBytes
		) {
			errors.push(
				error(
					field,
					"ATTACHMENT_SIZE_EXCEEDED",
					`该分类单个附件上限为 ${formatBytes(limit.maxFileSizeBytes)}`,
				),
			);
		}
		const mimeType = attachment.mimeType.toLocaleLowerCase();
		if (
			EXECUTABLE_MIME_TYPES.has(mimeType) ||
			EXECUTABLE_EXTENSIONS.test(attachment.name)
		) {
			errors.push(
				error(field, "EXECUTABLE_ATTACHMENT_FORBIDDEN", "平台不接收可执行文件"),
			);
