...
describe("computeAgentScoreSnapshot", () => {
	it("prevents one five-star rating from outranking mature high-quality evidence", () => {
		// 这是小样本失真的关键反例：如果直接计算算术平均，一条五星会高于一百条四星；
		// 贝叶斯先验应让证据充分且表现稳定的成熟 Agent 排在前面。
		...
		const matureAgent = computeAgentScoreSnapshot(
			{
				...
				responseTimes: Array.from({ length: 100 }, () => ({
					seconds: 90,
					respondedAt: NOW,
				})),
		...
		expect(newAgent.dimensions.qualityFeedback.lifetimeValue).toBeLessThan(
			matureAgent.dimensions.qualityFeedback.lifetimeValue,
		);
	...
	it("keeps rule version, raw sample size, recent/lifetime values and deterministic output", () => {
		...
		expect(first.dimensions.qualityFeedback.recentValue).toBeGreaterThan(
			first.dimensions.qualityFeedback.lifetimeValue,
		);
...
describe("dispute rate and rating boundary", () => {
	it("does not penalize withdrawn or agent-not-at-fault disputes", () => {
		...
		expect(
			computeDisputeRate(["agent_at_fault", "agent_not_at_fault", "withdrawn"]),
		).toBe(0.5);
	});
