...
describe("redactTaskForAudience", () => {
	...
	it("does not reveal whether a private task exists to unauthorized audiences", () => {
		...
		expect(() => redactTaskForAudience(privateTask, "unauthenticated")).toThrow(
			TaskNotVisibleError,
		);
		expect(redactTaskForAudience(privateTask, "publisher")).toMatchObject({
			access: "full",
			publisherId: "publisher-secret",
		});
	});
...
describe("updateTaskModeSettings", () => {
	it("locks settings after acceptance and requires an automatic acceptor", () => {
		...
		expect(() =>
			updateTaskModeSettings({ status: "executing", before, after: before }),
		).toThrow("TASK_MODE_LOCKED");
		expect(() =>
			updateTaskModeSettings({
				status: "matching",
				before,
