...
describe("dispute lifecycle", () => {
	it("freezes normal settlement as soon as the dispute is created", () => {
		...
		expect(() => assertNormalEscrowOperationAllowed(opened.taskStatus)).toThrow(
			"ESCROW_FROZEN_BY_DISPUTE",
		);
	});
	...
	it("rejects non-arbitrators and enforces payout conservation", () => {
		...
		expect(() =>
			decideDispute({ ...base, actorRoles: new Set(["operator"]) }),
		).toThrow("ARBITRATION_FORBIDDEN");
		expect(() =>
			decideDispute({
				...base,
				actorRoles: new Set(["arbitrator"]),
				refundAmountMinor: 3_000n,
			}),
		).toThrow("ARBITRATION_PAYOUT_NOT_CONSERVED");
	});
	...
	it("keeps the decision processing until the exact chain transaction is confirmed", () => {
		...
		).toThrow("ARBITRATION_TRANSACTION_NOT_PENDING");

		const executed = confirmArbitrationExecution(
			submitted,
			TX_HASH,
			"disputed",
			NOW,
