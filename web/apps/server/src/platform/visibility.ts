...
export type AssignmentModeConfig =
	...
	| Readonly<{
			mode: "automatic";
			priceCapMinor: bigint;
			rankingBasis: string;
			fallbackOnFail: "manual" | "cancel";
	  }>;
 ...
 */
export function redactTaskForAudience(
	task: TaskRecordForAudience,
	audience: TaskAudience,
) {
	const hasFullAccess =
		audience === "publisher" ||
		audience === "assigned_agent" ||
		audience === "authorized_ops";
	if (task.visibility === "private" && !hasFullAccess)
...
export function updateTaskModeSettings(
	input: Readonly<{
		status: TaskStatus;
		before: Readonly<{
			visibility: "public" | "private";
...
function validateAssignmentMode(mode: AssignmentModeConfig): void {
	if (
		mode.mode === "automatic" &&
		(mode.priceCapMinor <= 0n || mode.rankingBasis.trim().length === 0)
	) {
...
function validateAcceptanceMode(mode: AcceptanceModeConfig): void {
	if (
		mode.mode === "automatic" &&
		(mode.acceptorId.trim().length === 0 ||
			mode.ruleVersion.trim().length === 0)
	) {
