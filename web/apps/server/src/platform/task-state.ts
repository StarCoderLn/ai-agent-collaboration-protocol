...
export type TaskTransitionEvent =
	| { type: "submit" }
	| { type: "workflow_quote_confirmed"; amountMinor: bigint }
...
const transitions: Readonly<
	Record<
		TaskStatus,
		Readonly<Partial<Record<TaskTransitionEvent["type"], TaskStatus>>>
	>
> = {
	// 发布后先形成工作流并选择各阶段 Agent。只有全部报价被冻结，任务才允许进入托管；
	// 这条状态边界防止浏览器绕过选人步骤，继续按用户预估预算直接存入资金。
	...
	matching: {
		assignment_locked: "awaiting_agent_acceptance",
		workflow_final_accepted: "pending_settlement",
		workflow_settlement_confirmed: "settled",
		dispute_opened: "disputed",
		deadline_elapsed: "timed_out",
	},
	awaiting_agent_acceptance: {
	...
	executing: {
		result_submitted: "awaiting_review",
	...
	awaiting_review: {
		...
		result_accepted: "pending_settlement",
		workflow_final_accepted: "pending_settlement",
		workflow_settlement_confirmed: "settled",
		dispute_opened: "disputed",
	},
	...
	execution_failed: {
		execution_retry_requested: "matching",
		dispute_opened: "disputed",
	},
	settled: {},
	disputed: {
		arbitration_release_confirmed: "settled",
		arbitration_refund_confirmed: "refunded",
	},
	refunded: {},
...
export class InvalidTaskTransitionError extends Error {
	...
	constructor(
		...
		super(`${from} cannot apply ${event.type}`);
	}
}
...
export function calculatePlatformFee(
	...
	if (
		amountMinor <= 0n ||
		config.feeBasisPoints < 0n ||
		config.feeBasisPoints > 10_000n ||
		config.gasFallbackMinor < 0n
	)
		throw new Error("INVALID_FEE_INPUT");
	const proportional = (amountMinor * config.feeBasisPoints + 9_999n) / 10_000n;
	const fee =
		proportional > config.gasFallbackMinor
			? proportional
			: config.gasFallbackMinor;
...
export function validateTaskInput(
	...
	if (
		input.budgetMinor < config.minBudgetMinor ||
		input.budgetMinor > config.maxBudgetMinor
	)
		errors.push("BUDGET_OUT_OF_RANGE");
	if (input.deadline.getTime() < now.getTime() + config.minExecutionPeriodMs)
		errors.push("DEADLINE_TOO_SOON");
	if (input.tags.some((tag) => config.forbiddenTags.has(tag)))
		errors.push("FORBIDDEN_TAG");
	return errors;
}
