...
integration("escrow PostgreSQL synchronization", () => {
	...
	afterEach(async () => {
		for (const taskId of taskIds.splice(0)) {
			...
			await pool.query("DELETE FROM reconciliation_alerts WHERE task_id=$1", [
				taskId,
			]);
			...
			await pool.query("DELETE FROM workflow_node_events WHERE task_id=$1", [
				taskId,
			]);
			...
			await pool.query(
				"DELETE FROM task_workflow_edges WHERE workflow_run_id IN (SELECT id FROM task_workflow_runs WHERE task_id=$1)",
				[taskId],
			);
			await pool.query("DELETE FROM task_workflow_nodes WHERE task_id=$1", [
				taskId,
			]);
			await pool.query("DELETE FROM task_workflow_runs WHERE task_id=$1", [
				taskId,
			]);
	...
	it("deduplicates a deposit event and advances awaiting_escrow exactly once after confirmation", async () => {
		...
		await expect(
			repository.applyCanonicalConfirmation({
	...
	it("freezes operations and keeps the task awaiting escrow when the confirmed amount differs", async () => {
		...
		await repository.observe(
			depositEvent(taskId, 3n, 11n, `0x${"77".repeat(32)}`),
		);
		const event = required(
			(await repository.listPending(CHAIN_ID, CONTRACT, 10))[0],
		);
	...
	it("marks a pre-transition reorg orphaned without advancing the task", async () => {
		...
		await repository.observe(
			depositEvent(taskId, 2n, 12n, `0x${"88".repeat(32)}`),
		);
		const event = required(
			(await repository.listPending(CHAIN_ID, CONTRACT, 10))[0],
		);
		...
		await expect(
			pool.query("SELECT status,status_version::text FROM tasks WHERE id=$1", [
				taskId,
			]),
		).resolves.toMatchObject({
			rows: [{ status: "awaiting_escrow", status_version: "0" }],
		});
	});
	...
	it("keeps pending checks, canonical rechecks, and reconciliation inside one chain contract", async () => {
		...
		const scopes = [
			...
			{
				chainId: CHAIN_ID,
				contractAddress: FOREIGN_CONTRACT,
				blockNumber: 22n,
		...
		for (const scope of scopes) {
			const taskId = await insertAwaitingEscrowTask(
				pool,
				taskIds,
				agentIds,
				2n,
		...
		for (const scope of scopes) {
			const event = required(
				(
					await repository.listPending(scope.chainId, scope.contractAddress, 10)
				)[0],
			);
		...
		const rechecks = await repository.listCanonicalRechecks(
			CHAIN_ID,
			CONTRACT,
			10,
		);
		...
		const reconciliation = await repository.listReconciliationCandidates(
			CHAIN_ID,
			CONTRACT,
			10,
		);
	...
	it("keeps a failed refund retryable and escalates only after the configured limit", async () => {
		...
		const now = new Date("2026-08-23T01:00:00Z");
		await expect(
			repository.recordRefundFailure(taskId, "RPC_TIMEOUT", now, 3, 1_000),
		).resolves.toMatchObject({
			number: 1,
