...
integration("escrow execution worker PostgreSQL recovery", () => {
	...
	it("persists a signed transaction before broadcast and replays the exact raw bytes without signing again", async () => {
			...
			rows: [
				{
					...
					tx_hash: TX_HASH,
					last_error_code: "RPC_RESPONSE_LOST",
				},
			],
		});
		await expect(
			pool.query("SELECT status_version::text FROM tasks WHERE id=$1", [
				fixture.taskId,
			]),
		).resolves.toMatchObject({ rows: [{ status_version: "4" }] });

		await expect(
			worker.runOne(new Date(FIRST_RUN.getTime() + 1_001)),
		).resolves.toEqual({
			claimed: true,
			jobId: fixture.jobId,
			status: "submitted",
		...
		expect(operator.broadcast).toHaveBeenNthCalledWith(2, {
			txHash: TX_HASH,
			rawTransaction: RAW_TRANSACTION,
		});
		...
		expect(submitted.rows[0]).toEqual({
			...
			status_version: "5",
			event_count: "1",
			event_type: "task.settlement_submitted",
		});
	});
	...
	it("moves an exhausted arbitration refund to manual review and freezes further fund operations", async () => {
		...
		await expect(worker.runOne(FIRST_RUN)).resolves.toMatchObject({
			status: "retry_pending",
		});
		await expect(
			worker.runOne(new Date(FIRST_RUN.getTime() + 1_001)),
		...
		await expect(
			worker.runOne(new Date(FIRST_RUN.getTime() + 3_002)),
		...
		expect(evidence.rows[0]).toEqual({
			...
			alert_code: "ESCROW_EXECUTION_DEAD_LETTER",
		});
	...
	it("cancels a stale acceptance job when the task is already disputed and never calls the operator", async () => {
		...
		await pool.query(
			"UPDATE tasks SET status='disputed',status_version=status_version+1 WHERE id=$1",
			[fixture.taskId],
		);
		const operator: EscrowOperatorClient = {
			prepare: vi.fn(async () => ({
		...
		await expect(
			new EscrowExecutionWorker(pool, operator, workerConfig()).runOne(
				FIRST_RUN,
			),
		).resolves.toEqual({
			claimed: false,
		...
		await expect(
			pool.query(
				"SELECT status,last_error_code FROM escrow_execution_jobs WHERE id=$1",
				[fixture.jobId],
			),
		).resolves.toMatchObject({
			rows: [{ status: "cancelled", last_error_code: "TASK_STATE_CHANGED" }],
		});
	...
	it("只有发布者完成最终验收后才广播一次性工作流分账，并保留独立事件语义", async () => {
		...
		const jobId = await insertWorkflowSettlementJob(
			pool,
			fixture.taskId,
			fixture.runId,
		);
		await expect(
			worker.runOne(new Date(FIRST_RUN.getTime() + 1)),
		).resolves.toEqual({
			claimed: true,
			jobId,
			status: "submitted",
...
async function insertAcceptanceFixture(pool: Pool, taskIds: string[]) {
	const taskId = await insertTaskAndIntent(
		pool,
		taskIds,
		"pending_settlement",
		4,
...
async function insertWorkflowSettlementJob(
	pool: Pool,
	taskId: string,
	runId: string,
): Promise<string> {
...
async function cleanupFixture(pool: Pool, taskId: string): Promise<void> {
	...
	await pool.query("DELETE FROM escrow_execution_jobs WHERE task_id=$1", [
		taskId,
	]);
	await pool.query(
		"DELETE FROM arbitration_decisions WHERE dispute_id IN (SELECT id FROM disputes WHERE task_id=$1)",
		[taskId],
	);
	await pool.query(
		"DELETE FROM dispute_evidence WHERE dispute_id IN (SELECT id FROM disputes WHERE task_id=$1)",
		[taskId],
	);
