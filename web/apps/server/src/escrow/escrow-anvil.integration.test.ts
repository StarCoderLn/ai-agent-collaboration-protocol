...
const integration =
	...
	PAYMENT_TOKEN_ADDRESS === undefined
		? describe.skip
		: describe;
...
integration("escrow Anvil contract adapter", () => {
	it("reads a real Deposited log and escrowOf record", async () => {
		...
		const deposit = events.find(
			(event) => event.taskKey === taskKeyForTaskId(TASK_ID),
		);
		expect(deposit).toMatchObject({
			chainId: 31_337n,
			contractAddress: required(CONTRACT_ADDRESS).toLowerCase(),
			taskKey:
				"0x31e5891f6803041a37cfae842c5bf47aa89df5130d6a8ba235cdd9041744763f",
