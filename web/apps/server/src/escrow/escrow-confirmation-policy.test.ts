...
describe("escrow confirmation policy", () => {
	it("uses two confirmations for local and test-chain development", () => {
		expect(
			resolveEscrowRequiredConfirmations({
				chainId: 31_337n,
	...
	it("fails closed when production deployment omits the threshold", () => {
		...
		).toThrow("生产环境必须显式配置 ESCROW_REQUIRED_CONFIRMATIONS");
	});
	...
	it("rejects zero and malformed configured values", () => {
		expect(() =>
			resolveEscrowRequiredConfirmations({
				chainId: 1n,
				environment: "development",
				configuredConfirmations: "0",
			}),
		).toThrow("ESCROW_REQUIRED_CONFIRMATIONS 必须是正整数");
		expect(() =>
			resolveEscrowRequiredConfirmations({
				chainId: 1n,
				environment: "development",
				configuredConfirmations: "2.5",
			}),
		).toThrow("ESCROW_REQUIRED_CONFIRMATIONS 必须是正整数");
	});
});
