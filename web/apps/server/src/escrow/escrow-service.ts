...
export class EscrowService {
	...
	async runWorker(now: Date = new Date()): Promise<EscrowWorkerResult> {
		...
		try {
			...
			if (lease.nextBlock <= head) {
				...
				toBlock = minBigint(
					head,
					lease.nextBlock + this.config.maxBlockSpan - 1n,
				);
				const events = await this.chain.getEvents(fromBlock, toBlock);
				for (const event of events)
					if (await this.repository.observe(event)) observed += 1;
				...
				await this.repository.releaseCursor(
					this.chain.chainId,
	...
	private async processPending(
		...
		Readonly<{ confirmed: number; orphaned: number; needsReview: number }>
	> {
		...
		for (const event of pending) {
			const confirmations =
				head >= event.blockNumber ? head - event.blockNumber + 1n : 0n;
	...
	private async recheckCanonicalBlocks(now: Date): Promise<number> {
		...
		for (const event of events) {
			...
			if (
				(await this.repository.recordCanonicalRecheck(event.id, hash, now)) ===
				"needs_review"
			)
				needsReview += 1;
		}
