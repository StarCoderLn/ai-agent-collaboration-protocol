...
export interface EscrowChainClient {
	...
	getEvents(
		fromBlock: bigint,
		toBlock: bigint,
	): Promise<readonly ObservedEscrowEvent[]>;
...
export class EthersEscrowChainClient implements EscrowChainClient {
	...
	async getEvents(
		...
		return logs.map((log) =>
			parseEscrowLog(log, this.chainId, this.contractAddress),
		);
	}
...
export function encodeReleaseCall(
	taskKey: string,
	payee: string,
	agentGrossAmount: bigint,
	feeAmount: bigint,
): string {
	assertBytes32(taskKey, "INVALID_TASK_KEY");
	if (agentGrossAmount < 0n || feeAmount < 0n || feeAmount > agentGrossAmount)
		throw new Error("INVALID_RELEASE_AMOUNT");
	return escrowInterface.encodeFunctionData("release", [
		taskKey,
...
export function encodeMilestoneReleaseCall(
	taskKey: string,
	payee: string,
	agentGrossAmount: bigint,
	feeAmount: bigint,
...
export function encodeDisputeRefundCall(
	taskKey: string,
	decisionHash: string,
	evidenceRoot: string,
): string {
	...
	assertBytes32(evidenceRoot, "INVALID_EVIDENCE_ROOT");
	return escrowInterface.encodeFunctionData("refundDispute", [
		taskKey,
		decisionHash,
		evidenceRoot,
	]);
...
function isTupleResult(
	value: unknown,
): value is Readonly<{ 0: unknown; 1: unknown; 2: unknown; 3: unknown }> {
	return (
		typeof value === "object" &&
		value !== null &&
		0 in value &&
...
export function createJsonRpcEscrowClient(
	input: Readonly<{
		rpcUrl: string;
		chainId: bigint;
		contractAddress: string;
