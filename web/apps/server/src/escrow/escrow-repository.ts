...
export class PgEscrowRepository implements EscrowRepository {
	...
	async prepareIntent(
		...
		return withTransaction(this.db, async (client) => {
          ...
          WHERE task.id=$1 FOR UPDATE OF task`,
			...
			if (task === undefined)
				throw new EscrowRepositoryError("TASK_NOT_FOUND", "任务不存在", 404);
			...
			if (task.currency.toUpperCase() !== "USDC") {
				throw new EscrowRepositoryError(
					"ESCROW_CURRENCY_UNSUPPORTED",
					"任务托管仅支持 USDC",
					422,
				);
			}
			if (
				task.quoted_total_minor === null ||
				BigInt(task.quoted_total_minor) <= 0n
			) {
				throw new EscrowRepositoryError(
					"WORKFLOW_QUOTE_REQUIRED",
					"请先为所有阶段选择 Agent 并确认总价",
					409,
				);
			}
			if (!/^0x[0-9a-f]{40}$/.test(task.publisher_id.toLowerCase())) {
				throw new EscrowRepositoryError(
					"PUBLISHER_WALLET_INVALID",
					"发布者钱包地址无效",
					422,
				);
			}
         ...
         RETURNING task_id::text,chain_id::text,contract_address,task_key,payer_wallet,
			...
			const intent = mapIntent(
				required(result.rows[0], "ESCROW_INTENT_NOT_WRITTEN"),
			);
			if (
	...
	async findOwnedIntent(
		taskId: string,
		publisherId: string,
	): Promise<EscrowIntent | null> {
		const result = await this.db.query<IntentRow>(
	...
	async claimCursor(
       ...
       RETURNING next_block::text,lease_token::text`,
		...
		return row === undefined
			? null
			: { token: row.lease_token, nextBlock: BigInt(row.next_block) };
	}
	...
	async listConfirmable(
		...
		const highestBlock =
			head + 1n >= requiredConfirmations
				? head - requiredConfirmations + 1n
				: -1n;
	...
	async applyCanonicalConfirmation(
		...
		return withTransaction(this.db, async (client) => {
           ...
           FROM escrow_sync WHERE id=$1 FOR UPDATE`,
			...
			if (
				input.canonicalBlockHash === null ||
				input.canonicalBlockHash !== event.block_hash
			) {
				await client.query(
					`UPDATE escrow_sync SET status=$2,confirmations=$3,canonical_checked_at=$4,updated_at=$4,
	...
	async recordCanonicalRecheck(
		...
		return withTransaction(this.db, async (client) => {
			...
			if (canonicalHash === row.block_hash) {
				await client.query(
					"UPDATE escrow_sync SET canonical_checked_at=$2,updated_at=$2 WHERE id=$1",
					[eventId, now],
				);
				return "canonical";
			}
	...
	async recordRefundFailure(
		...
		if (error.length < 1 || error.length > 300)
			throw new Error("INVALID_REFUND_ERROR");
		return withTransaction(this.db, async (client) => {
			const task = await client.query<{ status: TaskStatus }>(
				"SELECT status FROM tasks WHERE id=$1 FOR UPDATE",
				[taskId],
			);
			if (task.rows[0] === undefined)
				throw new EscrowRepositoryError("TASK_NOT_FOUND", "任务不存在", 404);
			...
			const attempt = nextRefundAttempt(
				latest.rows[0]?.attempt_no ?? 0,
				now,
				maxAttempts,
				baseDelayMs,
				error,
...
async function applyEventToTask(
	...
	if (state === undefined)
		return markNeedsReview(db, input, "ESCROW_INTENT_NOT_FOUND");
	...
	if (input.payload.type === "WorkflowSettled") {
		return applyAtomicWorkflowSettlement(
			db,
			{ ...input, payload: input.payload },
			state,
			amount,
	...
	if (input.payload.type === "Deposited") {
		const txMatches =
			state.deposit_tx_hash === null || state.deposit_tx_hash === input.txHash;
		if (
       ...
       WHERE dispute.task_id=$1 AND decision.decision='refund' AND decision.execution_status='submitted'`,
		...
		if (
			state.task_status !== "disputed" ||
			decision.rows[0]?.execution_tx_hash !== input.txHash
		) {
...
async function applyMilestoneRelease(
	...
	if (
		...
		BigInt(ledgerRow.refundable_amount_minor) <
			payload.milestoneGrossAmountMinor
	) {
	...
	const intent = await db.query(
      ...
      WHERE task_id=$1 AND released_amount_minor=$4
		...
		[
			input.taskId,
			nextReleased.toString(),
			input.now,
			alreadyReleased.toString(),
		],
	);
	...
	if (intent.rowCount !== 1 || run.rowCount !== 1)
		throw new Error("MILESTONE_LEDGER_UPDATE_FAILED");
...
async function applyAtomicWorkflowSettlement(
	...
	if (
		...
		payload.payer !== state.payer_wallet ||
		payload.escrowAmountMinor !== escrowAmount ||
		payload.totalGrossAmountMinor + payload.payerRefundAmountMinor !==
			escrowAmount ||
	...
	if (
		job === undefined ||
		job.settlement_manifest_hash !== payload.settlementManifestHash ||
	...
	const payouts = z
		...
		.min(1)
		.max(32)
		.safeParse(job.workflow_payouts);
	if (!payouts.success)
		return markNeedsReview(db, input, "WORKFLOW_SETTLEMENT_PAYOUTS_INVALID");
	...
	if (
		totals.gross !== payload.totalGrossAmountMinor ||
		totals.fee !== payload.totalFeeAmountMinor
	) {
	...
	await db.query(
		...
		[
			input.taskId,
			payload.totalGrossAmountMinor.toString(),
			payload.payerRefundAmountMinor.toString(),
			input.now,
		],
	);
	await db.query(
...
async function markNeedsReview(
	...
	await db.query(
		"UPDATE escrow_intents SET status='needs_review',failure_reason=$2,updated_at=$3 WHERE task_id=$1",
		[input.taskId, code, input.now],
	);
...
function expectedOnchainState(
	status: EscrowIntent["status"],
): OnchainEscrowRecord["state"] {
	if (status === "confirmed" || status === "partially_released")
		return "deposited";
