...
export class EnvelopeEncryptor {
	...
	constructor(config: EnvelopeEncryptorConfig, kmsClient?: KmsLike) {
		...
		this.kms = kmsClient ?? new KMSClient(kmsClientConfigFromEnvironment());
	}
	...
	): Promise<EncryptCredentialResult> {
	...
	private async getDataKeyLease(): Promise<DataKeyLease> {
		...
		if (!response.Plaintext || !response.CiphertextBlob || !response.KeyId) {
			throw new Error(
				"EnvelopeEncryptor: KMS GenerateDataKey 返回缺少 Plaintext/CiphertextBlob/KeyId",
			);
		}
...
/**
 * 生产环境不配置 endpoint，由 AWS SDK 根据 region 选择真实 KMS；LocalStack 显式配置
 * `AWS_ENDPOINT_URL`，让同一信封加密实现访问本机兼容端点。配置属于受信任的服务端启动
 * 输入，但仍在创建 SDK 客户端前校验绝对 HTTP(S) URL，避免拼写错误延迟到首次注册。
 */
export function kmsClientConfigFromEnvironment(): KMSClientConfig {
	const rawEndpoint = process.env.AWS_ENDPOINT_URL?.trim();
	if (!rawEndpoint) return {};
	const endpoint = new URL(rawEndpoint);
	if (endpoint.protocol !== "http:" && endpoint.protocol !== "https:") {
		throw new Error("AWS_ENDPOINT_URL 必须是绝对 HTTP(S) URL");
	}
