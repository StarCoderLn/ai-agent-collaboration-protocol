...
const DEFAULT_FILE_MIME_TYPES = new Set([
	"application/pdf",
	"image/png",
	"image/jpeg",
	"text/plain",
...
export class PgExecutionRepository implements ExecutionRepository {
	...
	async reportStatus(
		...
		const locked = await this.lockExecution(
			taskId,
			input.assignmentId,
			input.agentId,
		);
		...
		if (input.state === "failed") {
			return this.recordExecutionFailure(
				taskId,
				input,
				idempotencyKey,
				fingerprint,
		...
		try {
			next = reportExecutionProgress(snapshot, input.progress);
		} catch (error) {
			throw executionDomainError(error);
		}
		...
		await this.db.query(
			"UPDATE tasks SET status_version=$2,updated_at=now() WHERE id=$1",
			[taskId, version.toString()],
		);
		await this.db.query(
	...
	async submitResults(
		...
		const formats = input.results.map((entry) =>
		...
		let next: ExecutionSnapshot;
		try {
			next = submitTaskResults(
				executionSnapshot(locked),
				formats,
		...
		const batchRow = await this.db.query<{
			batch_no: number;
			submission_batch: string;
		}>(
		...
		await this.db.query(
			"UPDATE task_results SET is_latest=FALSE WHERE task_id=$1 AND is_latest=TRUE",
			[taskId],
		);
		...
		for (const [index, artifact] of input.results.entries()) {
         ...
         RETURNING id::text,submission_batch::text,batch_no,result_index,summary,artifact_kind,
           body_or_file_ref,mime_type,size_bytes::text,generated_at,note,is_latest,submitted_at`,
			...
			stored.push(
				serializeResult(
					required(inserted.rows[0], "RESULT_NOT_INSERTED"),
					true,
				),
			);
		}
		...
		await this.db.query(
			"UPDATE tasks SET status=$2,status_version=$3,updated_at=now() WHERE id=$1",
			[taskId, next.status, version.toString()],
		);
		await this.db.query(
	...
	async listResults(
		...
		return resultOf(200, {
			taskId,
			results: rows.rows.map((row) => serializeResult(row, true)),
		});
	}
	...
	async previewAcceptance(
		...
		const terms = await this.readAcceptanceTerms(
			taskId,
			input.resultId,
			actorId,
			false,
	...
	async accept(
		...
		let nextStatus: TaskStatus;
		try {
			nextStatus = transitionTaskStatus(terms.status, {
		...
		await this.db.query(
         ...
         task_id,source,source_ref,action,payee,agent_gross_amount_minor,fee_amount_minor,status,next_attempt_at
			...
			[
				taskId,
				acceptanceId,
				terms.payout_wallet_address.toLowerCase(),
				gross.toString(),
				fee.toString(),
			],
		);
	...
	async requestRework(
			...
			next = requestTaskRework(
				{
					...
					reworkCount: requestNo - 1,
					maxReworkCount: task.max_rework_count,
				},
       ...
       VALUES ($1,$2,$3,$4,$5) RETURNING id::text`,
		...
		await this.db.query(
			"UPDATE task_execution_state SET progress=95,updated_at=now() WHERE task_id=$1",
			[taskId],
		);
	...
	async readStatus(
		...
		return resultOf(200, {
			...
			lastReportedAt: current.last_reported_at?.toISOString() ?? null,
			executionState: current.execution_state ?? "running",
			estimatedCompletionAt:
				current.estimated_completion_at?.toISOString() ?? null,
	...
	private async resultLimits(taskId: string): Promise<{
		...
		const allowedFileMimeTypes = new Set(
			(row.allowed_mime_types ?? [...DEFAULT_FILE_MIME_TYPES]).map((value) =>
		...
		return {
			maxFileSizeBytes:
				row.max_file_size_bytes === null
					...
					: BigInt(row.max_file_size_bytes),
...
function assertFreshAcceptancePreview(
	input: AcceptResultInput,
	...
	if (
		input.expectedStatusVersion !== statusVersion ||
...
/**
 * Agent 接单确认与 Marketplace API 任务迁移跨越持久化 outbox 边界。已接单分配暂时搭配下面两种
...
function validateArtifacts(
	...
	for (const artifact of input.results) {
		...
		const allowed =
			artifact.kind === "inline" ? INLINE_MIME_TYPES : allowedFiles;
		if (!allowed.has(mime))
			throw new ExecutionServiceError(
				422,
...
function executionDomainError(error: unknown): ExecutionServiceError {
	...
	return new ExecutionServiceError(
		mapped[0],
		message === "REWORK_LIMIT_REACHED" ? "REWORK_LIMIT_EXCEEDED" : message,
		mapped[1],
	);
}
