...
/**
 * Marketplace API 到 Go Dispatch Engine 的唯一内部 HTTP 适配器。它统一附加内部 token 与
 * 用户身份，并限制响应体；上层服务无需了解内部路径、超时和错误响应格式。
 */
export class DispatchEngineClient implements DispatchEngineGateway {
	constructor(
		...
		if (baseUrl.length === 0 || internalToken.length === 0)
			throw new Error("DISPATCH_ENGINE_NOT_CONFIGURED");
	}
	...
	confirm(
		...
		return this.request(
			"POST",
			`/internal/tasks/${taskId}/assignments`,
			actorId,
			{ agentId },
			idempotencyKey,
		);
	...
	latestAssignment(
		...
		return this.request(
			"GET",
			`/internal/tasks/${taskId}/assignments/latest`,
			actorId,
		);
	}
	retryExecution(
		...
		return this.request(
			"POST",
			`/internal/tasks/${taskId}/execution-retry`,
			actorId,
			undefined,
			idempotencyKey,
		);
	...
	workflowNodeCandidates(
		...
		return this.request(
			"GET",
			`/internal/tasks/${taskId}/workflow-nodes/${nodeId}/candidates`,
			actorId,
		);
	}
	recordTaskExposure(
		...
		// input 原样转发给 Go 的权威校验边界；客户端不在此处推断候选是否合法。
		return this.request(
			"POST",
			`/internal/tasks/${taskId}/candidate-exposures`,
			actorId,
			input,
		);
	}
	recordWorkflowNodeExposure(
		...
		// 节点身份由 URL 表达，防止请求正文中的可控字段覆盖服务端路由上下文。
		return this.request(
			"POST",
			`/internal/tasks/${taskId}/workflow-nodes/${nodeId}/candidate-exposures`,
			actorId,
			input,
		);
	}
	rematchWorkflowNode(
		...
		return this.request(
			"POST",
			`/internal/tasks/${taskId}/workflow-nodes/${nodeId}/rematch`,
			actorId,
		);
	}
	confirmWorkflowNode(
	...
	latestWorkflowNodeAssignment(
		...
		return this.request(
			"GET",
			`/internal/tasks/${taskId}/workflow-nodes/${nodeId}/assignments/latest`,
			actorId,
		);
	}

	/**
	 * Agent 生命周期状态只由 Go 分发引擎迁移。Marketplace API 传递经 SIWE 验证的操作者，
	...
	transitionAgent(
		...
		return this.request(
			"POST",
			`/internal/agents/${agentId}/transitions`,
			actorId,
			{ event },
			idempotencyKey,
			"provider",
	...
	private async request(
		...
		if (Number.isFinite(contentLength) && contentLength > maxResponseBytes)
			throw new Error("DISPATCH_RESPONSE_TOO_LARGE");
		...
		try {
			...
			throw new Error("DISPATCH_RESPONSE_INVALID_JSON");
		}
		return {
