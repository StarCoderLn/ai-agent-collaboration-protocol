...
function validInput() {
	return {
		...
		description:
			"实现公开任务列表、详情、候选匹配、执行追踪与人工验收的完整业务流程。",
		acceptanceCriteria:
			"所有关键路径都有可重复执行的测试，桌面端和移动端均可使用。",
...
class MemoryTaskRepository implements TaskRepository {
	...
	async loadCreationContext(): Promise<TaskCreationContext | null> {
		return this.context;
	}
	async createDraft(
		publisherId: string,
		input: Parameters<TaskRepository["createDraft"]>[1],
	): Promise<StoredTask> {
		this.createCount += 1;
	...
	async updateDraft(
		...
		input: Parameters<TaskRepository["updateDraft"]>[3],
	): Promise<StoredTask | null> {
	...
	async updateModeSettings(
		...
		publisherId: string,
		expectedVersion: bigint,
		settings: Parameters<TaskRepository["updateModeSettings"]>[3],
	): Promise<StoredTask | null> {
		const task = await this.findOwned(taskId, publisherId);
	...
	async updateMatchCriteria(
		...
		publisherId: string,
		expectedVersion: bigint,
		input: Parameters<TaskRepository["updateMatchCriteria"]>[3],
	): Promise<StoredTask | null> {
		const task = await this.findOwned(taskId, publisherId);
		if (
			task === null ||
	...
	async submit(
		...
		const next: StoredTask = {
			...task,
			draft: { ...task.draft, tags: canonicalTags },
			categoryVersion,
			status: "planning",
			statusVersion: expectedVersion + 1n,
		};
	...
	async archive(
		...
		if (
			task === null ||
			!["draft", "planning"].includes(task.status) ||
			task.statusVersion !== expectedVersion
		)
			return null;
		const archived = {
...
class MemoryEventWriter implements TaskEventWriter {
	...
	async write(event: Parameters<TaskEventWriter["write"]>[0]) {
...
function makeDeps() {
	...
	const deps: TaskCommandDeps = {
		...
		auditLogWriter,
		eventWriter,
		workflowPlanner: { ensure: async () => undefined },
...
describe("task command service", () => {
	it("creates a canonical draft and replays the same idempotent response", async () => {
		...
		const first = await createTaskDraft(
			validInput(),
			"publisher",
			"create-1",
			deps,
		);
		const replay = await createTaskDraft(
			validInput(),
			"publisher",
			"create-1",
			deps,
		...
		expect(repository.tasks.get("task-1")?.draft.tags).toEqual([
			"agent",
			"next.js",
		]);
	...
	it("soft-archives only pre-funding tasks and preserves an auditable idempotent result", async () => {
		const { deps, repository, auditLogWriter, eventWriter } = makeDeps();
		await createTaskDraft(validInput(), "publisher", "create-archive", deps);

		const first = await archiveOwnedTask(
			"task-1",
			"publisher",
			"archive-1",
			deps,
		);
		const replay = await archiveOwnedTask(
			"task-1",
			"publisher",
			"archive-1",
			deps,
		...
		expect(auditLogWriter.entries.at(-1)?.action).toBe("task.archive");
		expect(eventWriter.events.at(-1)).toMatchObject({
			eventType: "task.archived",
			statusVersion: 1n,
		});
	});

	it("rejects deletion after a task enters funding or fulfillment", async () => {
		const { deps, repository } = makeDeps();
		await createTaskDraft(validInput(), "publisher", "create-locked", deps);
		const task = repository.tasks.get("task-1");
		if (task === undefined) throw new Error("TASK_FIXTURE_REQUIRED");
		repository.tasks.set("task-1", { ...task, status: "awaiting_escrow" });

		await expect(
			archiveOwnedTask("task-1", "publisher", "archive-locked", deps),
		).rejects.toMatchObject({
			code: "TASK_ARCHIVE_NOT_ALLOWED",
			statusCode: 409,
		});
		expect(repository.tasks.has("task-1")).toBe(true);
	});
	...
	it("saves an incomplete draft, incrementally edits it, and reports missing publish fields in preview", async () => {
		...
		expect(repository.tasks.get("task-1")?.draft.title).toBe(
			"先记录这个产品想法",
		);
		...
		expect(
			(preview.body.issues as readonly { code: string }[]).map(
				(issue) => issue.code,
			),
		).toEqual(["CATEGORY_REQUIRED", "DEADLINE_REQUIRED"]);
	...
	it("rejects submitting an incomplete draft with field-level errors", async () => {
		...
		await expect(
			submitTaskDraft("task-1", "publisher", "submit-partial", deps),
		).rejects.toMatchObject({
	...
	it("revalidates a draft against changed timing config before submit", async () => {
		...
		repository.context = {
			...BASE_CONTEXT,
			minExecutionPeriodMs: 3 * 60 * 60_000,
		};

		await expect(
			submitTaskDraft("task-1", "publisher", "submit-1", deps),
		).rejects.toMatchObject({
			code: "VALIDATION_FAILED",
			statusCode: 422,
		});
	...
	it("submits atomically without inventing a planning budget", async () => {
		const { deps, repository, eventWriter, auditLogWriter } = makeDeps();
		await createTaskDraft(validInput(), "publisher", "create-1", deps);
		await previewOwnedTask("task-1", "publisher", deps);
		...
		expect(submitted.body.status).toBe("planning");
		expect(submitted.body.planning).toMatchObject({
			estimatedBudgetMinor: null,
		});
		expect(eventWriter.events).toHaveLength(1);
		expect(eventWriter.events[0]).toMatchObject({
			statusVersion: 1n,
	...
	it("publishes without pricing and identifies canonical capabilities from natural language", async () => {
		const { deps, repository } = makeDeps();
		const input = validInput();
		const { pricing: _pricing, ...withoutPricing } = input;
		await createTaskDraft(
			{ ...withoutPricing, tags: [] },
			"publisher",
			"create-no-budget",
		...
		expect(submitted.body).toMatchObject({
			status: "planning",
			planning: { estimatedBudgetMinor: null },
		});
		expect(repository.tasks.get("task-1")?.draft).toMatchObject({
			pricing: null,
			tags: ["agent", "next.js"],
		});
	});
	...
	it("replays a successful submit after the first request changed the task state", async () => {
		const { deps } = makeDeps();
		await createTaskDraft(validInput(), "publisher", "create-1", deps);
		const first = await submitTaskDraft(
			"task-1",
			"publisher",
			"submit-replay",
			deps,
		);
		const replay = await submitTaskDraft(
			"task-1",
			"publisher",
			"submit-replay",
			deps,
	...
	it("accepts and normalizes a valid custom tag outside the platform vocabulary", async () => {
		const { deps, repository } = makeDeps();
		await createTaskDraft(
			{ ...validInput(), tags: ["  RAG Workflow  ", "rag   workflow"] },
			"publisher",
			"create-1",
			deps,
		);
		await expect(
			submitTaskDraft("task-1", "publisher", "submit-custom-tag", deps),
	...
	it("audits mode changes and rejects them after an Agent has accepted the task", async () => {
		const { deps, repository, auditLogWriter } = makeDeps();
		await createTaskDraft(validInput(), "publisher", "create-1", deps);
		const changed = await updateOwnedTaskModeSettings(
			"task-1",
			{
				assignmentMode: {
					mode: "automatic",
	...
	it("adjusts only matching-safe fields, emits evidence, and replays idempotently", async () => {
		...
		const first = await updateOwnedTaskMatchCriteria(
			"task-1",
			patch,
			"publisher",
			"criteria-1",
		...
		const replay = await updateOwnedTaskMatchCriteria(
			"task-1",
			patch,
			"publisher",
			"criteria-1",
	...
	it("rejects unsafe fields, non-matching states, and another publisher without leaking existence", async () => {
		...
		await expect(
			updateOwnedTaskMatchCriteria(
				"task-1",
				{ pricing: { type: "fixed", amountMinor: "20000" } },
				"publisher",
				"criteria-unsafe",
				deps,
			),
	...
	it("does not reveal another publisher's task", async () => {
		...
		await createTaskDraft(validInput(), "publisher", "create-1", deps);
		await expect(
			previewOwnedTask("task-1", "attacker", deps),
