...
import {
	IncompleteTaskProjectionError,
	projectStoredTask,
	projectStoredTaskOwnerSummary,
...
export async function listTaskMarket(
	url: string,
	...
	const categoryId = optionalParsed(
		params.get("category"),
		uuidSchema,
		"任务分类格式不正确",
	);
	const status = optionalParsed(
		params.get("status"),
		taskStatusSchema,
		"任务状态筛选值不正确",
	);
	...
	const page = await repository.listPublicMarket({
		keyword,
		categoryId,
		tag,
		status,
	...
	return {
		statusCode: 200,
		body: {
			tasks: page.tasks.map((task) => projectStoredTask(task, "public")),
			total: page.total,
			limit,
			offset,
		},
	...
	try {
		if (record.task.visibility === "public") {
			return {
				statusCode: 200,
				body: {
					task: projectStoredTask(record.task, "public"),
					access: "public",
				},
			};
		}
		...
		const audience =
			...
			record.task.publisherId.toLocaleLowerCase() === normalizedActor
				? "publisher"
				: normalizedActor !== null &&
						record.assignedProviderWallets.some(
							(wallet) => wallet.toLocaleLowerCase() === normalizedActor,
						)
					? "assigned_agent"
					: "unauthenticated";
		return {
			statusCode: 200,
			body: {
				task: projectStoredTask(record.task, audience),
				access: audience,
			},
		};
	} catch (error) {
		if (
			error instanceof TaskNotVisibleError ||
			error instanceof IncompleteTaskProjectionError
		) {
			throw new TaskServiceError(404, "TASK_NOT_FOUND", "任务不存在或无权访问");
		}
		throw error;
...
export async function getMarketStats(
	...
	return {
		...
		body: {
			...
			stats: await repository.readMarketStats(),
		},
	};
}
...
export async function getPublisherTaskStats(
	...
	return {
		statusCode: 200,
		body: {
			source: "publisher_tasks",
			stats: await repository.readPublisherStats(actorId),
		},
	};
}
...
export async function listPublisherTasks(
	...
	return {
		statusCode: 200,
		body: { tasks: tasks.map(projectStoredTaskOwnerSummary), limit, offset },
	};
}

function optionalParsed<Schema extends z.ZodType>(
	raw: string | null,
	schema: Schema,
	message: string,
	...
	if (!parsed.success)
		throw new TaskServiceError(422, "VALIDATION_FAILED", message);
	return parsed.data;
}
...
function boundedInteger(
	...
	if (!/^\d+$/.test(raw))
		throw new TaskServiceError(422, "VALIDATION_FAILED", `${field} 必须是整数`);
