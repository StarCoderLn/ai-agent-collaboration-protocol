...
type TaskDispatchErrorCode =
	| "TASK_NOT_FOUND"
	| "IDEMPOTENCY_KEY_REQUIRED"
	| WorkflowSelectionErrorCode;
...
/**
 * 浏览器提交的候选曝光证据。所有身份字段都属于待验证声明：Marketplace API 先校验发布者，
 * Dispatch Engine 再将 record、task、node、agent 和 position 与冻结快照交叉核对。
 */
...
export class TaskDispatchService {
	...
	async recordTaskExposure(
		taskId: string,
		actorId: string,
		input: CandidateExposureInput,
	): Promise<TaskServiceResult> {
		// 即使曝光不改变任务状态，也可能泄露私有候选与报价，因此必须先执行任务归属校验。
		await this.assertPublisher(taskId, actorId);
		return this.dispatch.recordTaskExposure(taskId, actorId, input);
	}

	async recordWorkflowNodeExposure(
		taskId: string,
		nodeId: string,
		actorId: string,
		input: CandidateExposureInput,
	): Promise<TaskServiceResult> {
		// nodeId 不作为独立授权主体；先验证父任务发布者，再由 Go 仓储验证节点隶属关系。
		await this.assertPublisher(taskId, actorId);
		return this.dispatch.recordWorkflowNodeExposure(
			taskId,
			nodeId,
			actorId,
			input,
	...
	async confirmWorkflowNode(
		...
		return this.dispatch.confirmWorkflowNode(
			taskId,
			nodeId,
			agentId,
			actorId,
	 ...
	 * Marketplace API 的权威状态机消费该事实回到 matching，保留原托管和完整历史。
	 */
	 ...
	 * outbox 原子性仍由分发引擎负责，Marketplace API 只承担发布者授权和幂等键门禁。
	 */
	async retryWorkflowNodeExecution(
		...
		return this.dispatch.retryWorkflowNodeExecution(
			taskId,
			nodeId,
			actorId,
			idempotencyKey,
