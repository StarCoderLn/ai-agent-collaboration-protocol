...
const integerString = z
	.string()
	.regex(/^\d+$/)
	.refine((value) => BigInt(value) <= MAX_PG_BIGINT, "金额超出 BIGINT 范围");
...
const pricingSchema = z.discriminatedUnion("type", [
	...
	z
		.object({
			...
			minAmountMinor: integerString,
			maxAmountMinor: integerString,
		})
		.strict(),
]);
...
const taskDraftPatchSchema = z
	.object({
		...
		tags: z
			.array(z.string().trim().min(1).max(MAX_MATCHING_TAG_LENGTH))
...
const taskModeSettingsSchema = z
	.object({
		visibility: z.enum(["public", "private"]).optional(),
		assignmentMode: assignmentModeSchema.optional(),
		acceptanceMode: acceptanceModeSchema.optional(),
	...
	.refine(
		(value) =>
			...
			value.assignmentMode !== undefined ||
			value.acceptanceMode !== undefined,
		"至少提交一个模式设置",
	);

// 资金已经托管后，预算、币种和任务正文不能再通过“重新匹配”入口改写。这里仅开放
// 会影响候选资格、但不会改变托管金额或交付契约的三个字段。
const taskMatchCriteriaPatchSchema = z
	.object({
		categoryId: uuid.optional(),
		tags: z
			.array(z.string().trim().min(1).max(MAX_MATCHING_TAG_LENGTH))
...
export function parseTaskDraftPatch(raw: unknown): ParseTaskDraftPatchResult {
	...
	const value = parsed.data;
	type MutableDraftPatch = {
		-readonly [Key in keyof EditableTaskDraft]?: EditableTaskDraft[Key];
	};
	...
	if (value.acceptanceCriteria !== undefined)
		draft.acceptanceCriteria = value.acceptanceCriteria;
	if (value.deliverableFormat !== undefined)
		draft.deliverableFormat = value.deliverableFormat;
	...
	if (value.requiredCapability !== undefined)
		draft.requiredCapability = value.requiredCapability;
	if (value.attachments !== undefined)
		draft.attachments = value.attachments.map(parseAttachment);
	...
	if (value.assignmentMode !== undefined)
		result.assignmentMode = parseAssignmentMode(value.assignmentMode);
	if (value.acceptanceMode !== undefined)
		result.acceptanceMode = parseAcceptanceMode(value.acceptanceMode);
	return { success: true, data: result };
}

/** 模式更新接口只接受三个配置字段，避免借该入口绕过草稿锁定规则修改核心任务内容。 */
export function parseTaskModeSettings(
	raw: unknown,
): ParseTaskModeSettingsResult {
	...
	if (parsed.data.visibility !== undefined)
		result.visibility = parsed.data.visibility;
	if (parsed.data.assignmentMode !== undefined)
		result.assignmentMode = parseAssignmentMode(parsed.data.assignmentMode);
	if (parsed.data.acceptanceMode !== undefined)
		result.acceptanceMode = parseAcceptanceMode(parsed.data.acceptanceMode);
	return { success: true, data: result };
}
 ...
 */
export function parseTaskMatchCriteriaPatch(
	raw: unknown,
): ParseTaskMatchCriteriaPatchResult {
	...
	if (parsed.data.categoryId !== undefined)
		result.categoryId = parsed.data.categoryId;
	if (parsed.data.tags !== undefined) result.tags = parsed.data.tags;
	if (parsed.data.deadline !== undefined)
		result.deadline = new Date(parsed.data.deadline);
	return { success: true, data: result };
}
...
function parsePricing(value: z.infer<typeof pricingSchema>): TaskPricing {
	return value.type === "fixed"
		...
		: {
				type: "range",
				minAmountMinor: BigInt(value.minAmountMinor),
				maxAmountMinor: BigInt(value.maxAmountMinor),
			};
}
...
function parseAssignmentMode(
	value: z.infer<typeof assignmentModeSchema>,
): AssignmentModeConfig {
...
function parseAcceptanceMode(
	value: z.infer<typeof acceptanceModeSchema>,
): AcceptanceModeConfig {
	return value.mode === "manual"
		? { mode: "manual" }
		: {
