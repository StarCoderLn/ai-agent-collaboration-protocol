...
import { type TaskStatus, transitionTaskStatus } from "../platform/task-state";
import {
 ...
 */
export class PgDispatchTransitionRepository
	implements DispatchTransitionRepository
{
	...
	async apply(
		...
		if (
			...
			!assignmentStatusSupportsEvent(assignment.status, input.eventType)
		) {
			throw new DispatchTransitionError(
		...
		try {
			nextStatus = transitionTaskStatus(
				task.status,
				asTaskTransitionEvent(input),
			);
		} catch (error) {
