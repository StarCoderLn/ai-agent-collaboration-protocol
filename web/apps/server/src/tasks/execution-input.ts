...
const executionProgressInputSchema = z
	.object({
		agentId: uuid,
		assignmentId: uuid,
	...
	.refine(validEta, {
		...
		message: "预计完成时间不能早于上报时间",
	});

const executionNeedsInputSchema = z
	.object({
		agentId: uuid,
		assignmentId: uuid,
...
const executionFailureInputSchema = z
	.object({
		agentId: uuid,
		assignmentId: uuid,
...
export const resultSubmissionInputSchema = z
	.object({
		agentId: uuid,
		assignmentId: uuid,
...
const expectedSettlementSchema = z
	.object({
...
export const acceptancePreviewInputSchema = z
	.object({ resultId: uuid })
	.strict();
export const acceptResultInputSchema = z
	.object({
		resultId: uuid,
	...
	.strict();
export const reworkInputSchema = z
	.object({
		resultId: uuid,
...
export type WorkflowExecutionStatusInput = z.infer<
	typeof workflowExecutionStatusInputSchema
>;
export type ResultSubmissionInput = z.infer<typeof resultSubmissionInputSchema>;
export type AcceptancePreviewInput = z.infer<
	typeof acceptancePreviewInputSchema
>;
...
function validEta(value: {
	...
	return (
		value.estimatedCompletionAt === undefined ||
		new Date(value.estimatedCompletionAt).getTime() >=
			new Date(value.reportedAt).getTime()
	);
}
