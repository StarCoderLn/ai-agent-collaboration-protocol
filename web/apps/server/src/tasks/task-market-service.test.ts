...
function task(overrides: Partial<StoredTask> = {}): StoredTask {
	return {
		...
		draft: {
			...
			description:
				"实现公开市场、候选匹配和可验证交付，并确保私密任务不会被未授权用户枚举。",
...
): TaskReadRepository & { listPublicMarket: ReturnType<typeof vi.fn> } {
	return {
		findForAudience: vi.fn(async () => ({
			task: storedTask,
			assignedProviderWallets: ["0x2222222222222222222222222222222222222222"],
		})),
...
describe("task market service", () => {
	it("uses the exact same public projection for market list, detail, and publisher preview", async () => {
		...
		const market = await listTaskMarket(
			`http://api.local/api/market/tasks?keyword=Agent&category=${CATEGORY_ID}&tag=NEXT.JS&status=matching`,
			repo,
		);
		...
		expect(repo.listPublicMarket).toHaveBeenCalledWith(
	...
	it("hides private tasks from strangers but allows the assigned provider wallet", async () => {
		...
		await expect(
			getTaskDetail(TASK_ID, null, privateRepo),
		...
		const assigned = await getTaskDetail(
			TASK_ID,
		...
		expect(assigned.body.task).toMatchObject({
			...
			acceptanceCriteria: task().draft.acceptanceCriteria,
		});
	});
	...
	it("validates filters and keeps public-market and publisher statistics as separate sources", async () => {
		const repo = repository();
		await expect(
			listTaskMarket("http://api.local/api/market/tasks?limit=500", repo),
		).rejects.toMatchObject({ code: "VALIDATION_FAILED" });
		await expect(
			listTaskMarket(
				"http://api.local/api/market/tasks?category=not-a-uuid",
				repo,
			),
		).rejects.toMatchObject({ code: "VALIDATION_FAILED" });
		await listTaskMarket(
			"http://api.local/api/market/tasks?status=execution_failed",
			repo,
		);
		expect(repo.listPublicMarket).toHaveBeenLastCalledWith(
			expect.objectContaining({ status: "execution_failed" }),
		);
		await expect(getMarketStats(repo)).resolves.toEqual({
			statusCode: 200,
			body: { source: "public_market", stats: { total: 9, matching: 3 } },
		});
		await expect(
			getPublisherTaskStats(task().publisherId, repo),
		).resolves.toEqual({
			statusCode: 200,
			body: { source: "publisher_tasks", stats: { total: 4, pending: 2 } },
		});
	});
	...
	it("returns publisher-owned drafts and private tasks without exposing attachment storage refs", async () => {
		...
		expect(
			(result.body.tasks as readonly Record<string, unknown>[])[0],
		).not.toHaveProperty("attachments");
		expect(repo.listOwnedTasks).toHaveBeenCalledWith(
			"0x1111111111111111111111111111111111111111",
			25,
			0,
		);
