...
export class PgWorkflowFeedbackRepository
	implements WorkflowFeedbackRepository
{
	...
	async submit(
		...
		if (
			target === undefined ||
			target.publisher_id.toLowerCase() !== actorId.toLowerCase()
		) {
		...
		if (
			!["pending_settlement", "settled", "disputed", "refunded"].includes(
				target.task_status,
			)
		) {
			throw new WorkflowFeedbackServiceError(
				409,
				"WORKFLOW_FEEDBACK_NOT_READY",
				"完成阶段验收后才能提交反馈",
			);
		}
		if (
			target.node_status !== "accepted" ||
			target.agent_id === null ||
			target.assignment_id === null
		) {
			throw new WorkflowFeedbackServiceError(
				409,
				"WORKFLOW_NODE_NOT_ACCEPTED",
				"只有已验收的 Agent 交付可以评价",
			);
		}
		...
		try {
			...
			if (isUniqueViolation(error)) {
				throw new WorkflowFeedbackServiceError(
					409,
					"WORKFLOW_FEEDBACK_ALREADY_SUBMITTED",
					"该阶段已经提交过反馈",
				);
			}
			throw error;
	...
	async list(taskId: string, actorId: string): Promise<WorkflowFeedbackResult> {
		...
		if (
			task === undefined ||
			task.publisher_id.toLowerCase() !== actorId.toLowerCase()
		) {
			throw new WorkflowFeedbackServiceError(
				404,
				"TASK_NOT_FOUND",
