...
const richDesignPreview = z
	.object({
		...
		metrics: z
			.array(
				z.object({ label: nonEmptyText, value: nonEmptyText }).passthrough(),
			)
			.max(4),
		sections: z
			.array(
	...
	.superRefine((preview, context) => {
		...
		if (
			texts.some((text) =>
				/(?:待补充|占位|placeholder|\bxxx\b|\btodo\b)/i.test(text),
			)
		) {
			context.addIssue({
				code: "custom",
		...
		if (
			preview.sections.some((section) => /(?:区域|区)$/.test(section.title))
		) {
			context.addIssue({
				code: "custom",
				path: ["sections"],
...
const renderedDesignScreen = z
	...
	.superRefine((screen, context) => {
		...
		if (
			screen.viewport.width !== expected.width ||
			screen.viewport.height !== expected.height
		) {
			context.addIssue({
				code: "custom",
				path: ["viewport"],
...
const renderedDesignScreens = z
	.array(renderedDesignScreen)
	.length(2)
	.superRefine((screens, context) => {
		if (new Set(screens.map((screen) => screen.id)).size !== 2) {
			context.addIssue({
				code: "custom",
				message: "设计稿必须同时包含桌面端和移动端",
			});
		}
	});
...
export function evaluateAutomaticAcceptance(
	input: Readonly<{
		taskId: string;
	...
	if (
		result === undefined ||
		result.kind !== "inline" ||
		result.mimeType.toLocaleLowerCase() !== "application/json"
	) {
...
function schemaFor(
	contract: string,
): (typeof acceptanceSchemas)[SupportedContract] | null {
