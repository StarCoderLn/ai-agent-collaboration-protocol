...
export type WorkflowPreferenceResult = Readonly<{
	...
	body: Readonly<{
		...
		nodePreferences: readonly Readonly<{
			nodeId: string;
...
export async function updateOwnedWorkflowBudgetPreference(
	...
	if (!parsed.success) {
		throw new WorkflowPreferenceError(
			422,
			"VALIDATION_FAILED",
			"预算上限格式不正确",
		);
	...
	const preference =
		parsed.data.budgetPreferenceMinor === null
			? null
			: BigInt(parsed.data.budgetPreferenceMinor);
	if (
		preference !== null &&
		(preference < MIN_TASK_BUDGET_MINOR || preference > MAX_TASK_BUDGET_MINOR)
	) {
		throw new WorkflowPreferenceError(
			422,
			"VALIDATION_FAILED",
			"预算上限须在 1–100,000 USDC 之间",
		);
	}
	...
	if (reservation.existing !== null)
		return reservation.existing as WorkflowPreferenceResult;
	...
	if (
		run === undefined ||
		run.publisher_id.toLocaleLowerCase() !== input.actorId.toLocaleLowerCase()
	) {
		throw new WorkflowPreferenceError(
			404,
			"TASK_NOT_FOUND",
	...
	for (const [index, node] of nodesResult.rows.entries()) {
		...
		nodePreferences.push({
			nodeId: node.id,
			pricePreferenceMinor: amount?.toString() ?? null,
		});
	}
