...
export class WorkflowStateError extends Error {
	constructor(
		readonly code: "INVALID_NODE_TRANSITION" | "INVALID_WORKFLOW_GRAPH",
		message: string,
	) {
		super(message);
	}
...
const NODE_TRANSITIONS: Readonly<
	Record<
		WorkflowNodeStatus,
		Partial<Record<WorkflowNodeEvent["type"], WorkflowNodeStatus>>
	>
> = {
...
export function unlockReadyWorkflowNodes(
	...
	return nodes.map((node) => {
		...
		const ready = upstream.every(
			(nodeId) => statusById.get(nodeId) === "accepted",
		);
...
export function aggregateWorkflowStatus(
	...
	if (nodes.some((node) => node.status === "awaiting_review"))
		return "awaiting_review";
	if (
		nodes.every(
			(node) => node.status === "selecting" || node.status === "selected",
		)
	)
		return "planning";
...
export function assertValidWorkflowGraph(
	...
	for (const edge of edges) {
		if (
			...
			throw new WorkflowStateError(
				"INVALID_WORKFLOW_GRAPH",
				"workflow edge endpoints are invalid",
			);
		}
