 ...
 */
export class PgWorkflowSelectionRepository
	implements WorkflowSelectionRepository
{
	...
	async select(
		...
		return withTransaction(this.pool, async (client) => {
			...
			if (reservation.existing !== null)
				return asSelectionResult(reservation.existing);
          ...
          FOR UPDATE OF task,run,node,distribution`,
			...
			if (
				row === undefined ||
				row.publisher_id.toLowerCase() !== input.actorId.toLowerCase()
			) {
				throw new WorkflowSelectionRepositoryError(
					"WORKFLOW_NODE_NOT_FOUND",
					"工作节点不存在或无权访问",
			...
			const taskAllowsSelection =
				row.task_status === "planning" || row.task_status === "awaiting_escrow";
			const nodeAllowsSelection =
				row.node_status === "selecting" || row.node_status === "selected";
			if (
				!taskAllowsSelection ||
				row.run_status !== "planning" ||
				!nodeAllowsSelection
			) {
				throw new WorkflowSelectionRepositoryError(
					"WORKFLOW_SELECTION_LOCKED",
           ...
           FROM escrow_intents intent WHERE intent.task_id=$1 FOR UPDATE`,
			...
			const candidate = parsedCandidates.data.find(
			...
			if (
				row.selected_agent_id === input.agentId &&
				row.agreed_amount_minor === candidate.quoteMinor
			) {
				const currentTotals = await readWorkflowTotals(
					client,
					row.workflow_run_id,
				);
          ...
          WHERE id=$1 AND task_id=$2`,
			...
			await client.query(
				...
				[row.distribution_id, input.agentId],
			);
			await client.query(
         ...
         VALUES ($1,$2,$3,$4,$5::jsonb,$6)`,
			...
			if (allSelected) {
				...
				if (row.task_status === "planning") {
					...
					if (transitioned !== "awaiting_escrow")
				...
				await emitTaskEvent(client, {
					...
					eventType:
						row.task_status === "planning"
							? "task.workflow_quote_confirmed"
							: "task.workflow_quote_revised",
					payload:
						row.task_status === "planning"
			...
			await new PgAuditLogWriter(client).write({
				...
				actorType: "publisher",
				action: replacingSelection
					? "workflow.candidate.replace"
					: "workflow.candidate.select",
...
async function readWorkflowTotals(
	db: QueryExecutor,
	workflowRunId: string,
): Promise<WorkflowTotals> {
