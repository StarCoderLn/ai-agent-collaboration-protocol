	...
	if (insertedRunId !== undefined) {
		...
		for (const node of plan.nodes) {
         ...
         RETURNING id::text`,
			...
			if (insertedNodeId === undefined)
				throw new Error("WORKFLOW_NODE_NOT_INSERTED");
		...
		for (const edge of plan.edges) {
			...
			if (sourceNodeId === undefined || targetNodeId === undefined)
				throw new Error("WORKFLOW_EDGE_NODE_NOT_FOUND");
			await db.query(
 ...
 */
export async function activateFormalWorkflow(
	db: QueryExecutor,
	taskId: string,
	activatedAt: Date,
): Promise<void> {
	...
	if (
		run === undefined ||
		run.status !== "planning" ||
		run.quoted_total_minor === null ||
		run.quote_confirmed_at === null
	) {
		throw new WorkflowRepositoryError(
			409,
			"WORKFLOW_CREATION_NOT_ALLOWED",
	...
	if (
		...
		nodeResult.rows.some((node) => node.status !== "selected")
	) {
		throw new WorkflowRepositoryError(
			409,
			"WORKFLOW_CREATION_NOT_ALLOWED",
...
export async function readOwnedFormalWorkflow(
	...
	if (workflow === null) {
		throw new WorkflowRepositoryError(
			404,
			"WORKFLOW_NOT_FOUND",
			"该任务尚未生成正式多 Agent 工作流",
		);
	}
	return workflow;
}

async function findFormalWorkflow(
	db: QueryExecutor,
	taskId: string,
): Promise<FormalWorkflowGraph | null> {
	...
	const acceptanceByNode = new Map(
		acceptanceRows.rows.map((row) => [row.workflow_node_id, row]),
	);
	const reworkByNode = new Map(
		latestReworkRows.rows.map((row) => [row.workflow_node_id, row]),
	);
	return {
		run: {
			id: run.id,
		...
		nodes: nodeResult.rows.map((node) => {
			...
			return {
				...
				selection:
					node.selected_agent_id === null ||
					node.selected_agent_name === null ||
					node.selected_amount_minor === null
						? null
						: {
				...
				assignment:
					node.assignment_id === null ||
					node.agent_id === null ||
					node.agent_name === null ||
					node.assignment_status === null ||
					node.agreed_amount_minor === null ||
					node.assignment_accept_by === null
						? null
						: {
				...
				execution:
					node.progress === null || node.execution_state === null
						? null
						: {
				...
				candidateRecord:
					node.candidate_record_id === null ||
					node.candidate_rule_version === null
						? null
						: {
				...
				acceptance:
					acceptance === undefined
						...
						: {
								...
								release:
									acceptance.release_status === null
										? null
										: {
												status: acceptance.release_status,
												txHash: acceptance.release_tx_hash,
											},
							},
				latestRework:
					rework === undefined
...
function groupLatestResults(
	rows: readonly LatestResultRow[],
): ReadonlyMap<string, readonly LatestResultRow[]> {
