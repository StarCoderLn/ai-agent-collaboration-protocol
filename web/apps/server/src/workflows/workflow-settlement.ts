...
export function buildWorkflowSettlementPlan(
	...
	if (lines.length === 0 || lines.length > 32)
		throw new Error("WORKFLOW_PAYOUT_COUNT_INVALID");
	const normalized = lines.map((line) => normalizeLine(line));
	const totalGrossAmountMinor = normalized.reduce(
		(sum, line) => sum + line.grossAmountMinor,
		0n,
	);
	const totalFeeAmountMinor = normalized.reduce(
		(sum, line) => sum + line.feeAmountMinor,
		0n,
	);
	const runHash = textHash(workflowRunId);

	const settlementManifestHash = keccak256(
		coder.encode(
...
function normalizeLine(line: AcceptedWorkflowSettlementLine) {
	...
	if (
		line.grossAmountMinor <= 0n ||
		line.feeAmountMinor < 0n ||
		line.feeAmountMinor > line.grossAmountMinor ||
		line.sizeBytes < 0n ||
