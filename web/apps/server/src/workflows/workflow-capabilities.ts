...
export async function updateOwnedWorkflowNodeCapabilities(
	...
	if (!parsed.success) {
		throw new WorkflowCapabilityError(
			422,
			"VALIDATION_FAILED",
			`能力标签最多 ${MAX_MATCHING_TAG_COUNT} 个`,
		);
	}
	if (
	...
	if (reservation.existing !== null)
		return reservation.existing as WorkflowCapabilityResult;
	...
	if (
		node === undefined ||
		node.publisher_id.toLocaleLowerCase() !== input.actorId.toLocaleLowerCase()
	) {
	...
	if (
		node.task_status !== "planning" ||
		node.run_status !== "planning" ||
		node.node_status !== "selecting" ||
		node.selected_agent_id !== null
	) {
		throw new WorkflowCapabilityError(
			409,
			"WORKFLOW_CAPABILITY_LOCKED",
			"该阶段已冻结 Agent 报价，不能再调整能力需求",
		);
	}

	const { canonicalByAlias, forbiddenTags } = await loadMatchingTagTaxonomy(db);
	const normalizedTags = normalizeMatchingTags(
		parsed.data.tags,
		canonicalByAlias,
	);
	...
	await db.query(
     ...
     VALUES ($1,$2,$3,'capability_requirements_updated',$4::jsonb,$5)`,
		[
			...
			input.taskId,
			nextVersion.toString(),
			JSON.stringify({ tags: normalizedTags }),
			input.updatedAt,
		],
	);
