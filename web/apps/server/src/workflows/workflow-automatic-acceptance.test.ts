...
function validDesign(taskId: string) {
	...
	return {
		...
		pages: [
			{ id: "dashboard", name: "任务工作台", purpose: "查看任务执行状态" },
		],
		components: [
			{ id: "status", name: "状态卡片", responsibility: "展示当前状态" },
		],
			...
			sections: [
				{
					...
					description: "最近执行状态",
					items: [
						item("后台管理系统", "执行中", "primary"),
						item("品牌设计", "已完成", "success"),
					],
				},
				{
					id: "agents",
					...
					description: "自动更新",
					items: [
						item("需求 Agent", "已完成", "success"),
						item("设计 Agent", "进行中", "primary"),
					],
				},
				{
					id: "settlement",
					...
					description: "全部验收后释放",
					items: [
						item("需求确认", "质量已通过", "success"),
						item("界面设计", "待验收", "warning"),
					],
				},
			],
		},
...
function designScreen(id: "desktop" | "mobile", width: number, height: number) {
	const content = `${`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><rect width="${width}" height="${height}" fill="#ffffff"/>`.padEnd(
		520,
		" ",
	)}</svg>`;
	return {
