...
type AssignmentRow = {
	status: "pending_ack" | "accepted" | "accept_failed" | "cancelled";
};
type InboxRow = {
	task_id: string;
 ...
 */
export class PgWorkflowTransitionRepository
	implements WorkflowTransitionRepository
{
	...
	async apply(
		...
		if (replay !== undefined) {
			if (
				replay.task_id !== taskId ||
				replay.workflow_node_id !== workflowNodeId ||
				replay.assignment_id !== input.assignmentId ||
			...
			return result(
				...
				workflowNodeId,
				input,
				replay.resulting_status,
				BigInt(replay.resulting_version),
				run.status,
				BigInt(run.version),
				true,
			);
		...
		if (
			...
			!assignmentSupports(assignment.status, input.eventType)
		) {
			throw new WorkflowTransitionError(
		...
		if (
			...
			input.eventType === "assignment_failed"
		) {
	...
	): Promise<{ status: WorkflowRunStatus; version: string }> {
