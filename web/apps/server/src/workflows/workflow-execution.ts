...
import {
	type AcceptedWorkflowSettlementLine,
	buildWorkflowSettlementPlan,
} from "./workflow-settlement";
import {
...
export class PgWorkflowExecutionRepository {
	...
	async reportStatus(
		...
		const locked = await this.lockNode(
			taskId,
			workflowNodeId,
			input.assignmentId,
			input.agentId,
		);
		const replay = await this.callbackReplay(
			key,
			"execution_status",
			taskId,
			input.assignmentId,
		...
		const event =
			input.state === "failed"
				...
				: { type: "progress_reported" as const };
		...
		const progress =
			input.state === "failed" ? (locked.progress ?? 0) : input.progress;
		const executionState =
			input.state === "failed" ? "failed" : (input.state ?? "running");
       ...
       ON CONFLICT (workflow_node_id) DO UPDATE SET assignment_id=EXCLUDED.assignment_id,
		...
		await this.writeNodeEvent(
			workflowNodeId,
			taskId,
			nextVersion,
			input.state === "failed"
				? "workflow_node.execution_failed"
				: "workflow_node.execution_progress",
			{
	...
	async submitResults(
		...
		const batch = required(
			batchResult.rows[0],
			"WORKFLOW_RESULT_BATCH_NOT_CREATED",
		);
		await this.db.query(
		...
		for (const [index, artifact] of input.results.entries()) {
			const content =
				artifact.kind === "inline" ? artifact.content : artifact.storageRef;
			const size =
         ...
         RETURNING id::text,submission_batch::text,batch_no,result_index,summary,artifact_kind,
           body_or_file_ref,mime_type,size_bytes::text,generated_at,note,is_latest,submitted_at`,
				[
					...
					artifact.kind,
					content,
					artifact.mimeType.toLocaleLowerCase(),
					size.toString(),
					new Date(artifact.generatedAt),
					artifact.note ?? null,
				],
			);
			const storedRow = required(
				inserted.rows[0],
				"WORKFLOW_RESULT_NOT_INSERTED",
			);
		...
		await this.writeNodeEvent(
			...
			taskId,
			nextVersion,
			"workflow_node.results_submitted",
			{
				assignmentId: input.assignmentId,
				status: nextStatus,
		...
		if (automaticAcceptance.kind === "passed") {
			const resultId = required(
				storedIds[0],
				"AUTOMATIC_ACCEPTANCE_RESULT_NOT_FOUND",
			);
		...
		const result = resultOf(201, {
			...
			nodeStatus: nextStatus,
			nodeVersion: nextVersion.toString(),
			runStatus: submittedProjection.runStatus,
			runVersion: submittedProjection.runVersion.toString(),
			batchNo: batch.batch_no,
			submissionBatch: batch.submission_batch,
			results: stored,
			automaticAcceptance:
	...
	async previewAcceptance(
		...
		if (!uuid.safeParse(resultId).success)
			throw new WorkflowExecutionError(
				422,
				"VALIDATION_FAILED",
				"resultId 格式不正确",
		...
		return resultOf(200, {
			...
			workflowNodeId,
			resultId,
			nodeStatus: terms.node_status,
			nodeVersion: terms.node_version,
	...
	async accept(
		...
		if (
			input.expectedNodeVersion !== terms.node_version ||
			JSON.stringify(input.expectedSettlement) !== JSON.stringify(settlement)
		) {
			throw new WorkflowExecutionError(
				409,
	...
	private async finalizeAcceptance(
		...
		const settlement = settlementOf(terms);
		const nextStatus = transition(terms.node_status, {
		...
		const acceptance = await this.db.query<{ id: string }>(
         ...
         platform_fee_minor,agent_amount_minor,fee_rule_version
			...
			[
				...
				settlement.agentAmountMinor,
				settlement.feeRuleVersion,
			],
		);
		const acceptanceId = required(
			acceptance.rows[0],
			"WORKFLOW_ACCEPTANCE_NOT_INSERTED",
		).id;
		...
		if (projection.runStatus === "completed") {
			...
			await this.queueFinalSettlement(
				taskId,
		...
		return resultOf(200, {
			...
			runVersion: projection.runVersion.toString(),
			settlement,
			acceptanceMode: audit.mode,
		});
	}

	private async queueFinalSettlement(
		taskId: string,
		...
		if (task.publisher_id.toLowerCase() !== acceptedBy.toLowerCase()) {
			throw new WorkflowExecutionError(
				403,
				"FINAL_SETTLEMENT_REQUIRES_PUBLISHER",
				"最终结算必须由任务发布者验收授权",
			);
		}
		...
		if (
			...
			Number(required(nodeCount.rows[0], "WORKFLOW_NOT_FOUND").count)
		) {
			throw new WorkflowExecutionError(
				409,
		...
		const lines: AcceptedWorkflowSettlementLine[] = lineRows.rows.map(
			(row) => ({
				nodeId: row.node_id,
		...
		const totalBudgetMinor = BigInt(
			required(runRows.rows[0], "WORKFLOW_NOT_COMPLETED").total_budget_minor,
		);
		if (plan.totalGrossAmountMinor > totalBudgetMinor) {
			throw new WorkflowExecutionError(
				409,
				"WORKFLOW_SETTLEMENT_EXCEEDS_ESCROW",
				"Agent 成交总额超过托管金额",
			);
		}
		await this.db.query(
       ...
       ON CONFLICT (source,source_ref) DO NOTHING`,
		...
		const nextStatus = transitionTaskStatus(task.status, {
			type: "workflow_final_accepted",
		});
	...
	async requestRework(
		...
		const requestNo = required(
			requestNumber.rows[0],
			"REWORK_NUMBER_NOT_CREATED",
		).request_no;
		...
		await this.writeNodeEvent(
			...
			taskId,
			nextVersion,
			"workflow_node.rework_requested",
			{
				requestId,
	...
	private async unlockDownstream(
		workflowRunId: string,
		taskId: string,
	): Promise<void> {
...
function validateResults(input: ResultSubmissionInput): void {
	for (const artifact of input.results) {
		if (
			artifact.kind === "inline" &&
			!INLINE_MIME_TYPES.has(artifact.mimeType.toLocaleLowerCase())
		) {
			throw new WorkflowExecutionError(
				422,
				"RESULT_FORMAT_INVALID",
				`不支持结果格式 ${artifact.mimeType}`,
			);
		}
	}
}
