...
describe("workflow settlement evidence", () => {
	...
	it("收款金额只改变结算清单，制品变化只改变证据摘要", () => {
		...
		const moneyChanged = buildWorkflowSettlementPlan("run-1", [
			{ ...LINE, grossAmountMinor: 9_000_000n },
		]);
		const artifactChanged = buildWorkflowSettlementPlan("run-1", [
			{ ...LINE, bodyOrFileRef: "changed" },
		]);
		expect(moneyChanged.settlementManifestHash).not.toBe(
			original.settlementManifestHash,
		);
		expect(moneyChanged.evidenceRoot).toBe(original.evidenceRoot);
		expect(artifactChanged.settlementManifestHash).toBe(
			original.settlementManifestHash,
		);
	...
	it("拒绝空清单和费用超过 Agent 成交额的非法状态", () => {
		expect(() => buildWorkflowSettlementPlan("run-1", [])).toThrow(
			"WORKFLOW_PAYOUT_COUNT_INVALID",
		);
		expect(() =>
			buildWorkflowSettlementPlan("run-1", [
				{ ...LINE, feeAmountMinor: LINE.grossAmountMinor + 1n },
			]),
