 ...
 */
export function planFormalWorkflow(
	input: WorkflowPlanningInput,
): PlannedWorkflow {
	const templates = templatesFor(input);
	const nodes = templates.map(
		(template, index): PlannedWorkflowNode => ({
			...template,
			positionIndex: index,
	...
	const edges = nodes.slice(1).map(
		(node, index): PlannedWorkflowEdge => ({
...
function templatesFor(input: WorkflowPlanningInput): readonly NodeTemplate[] {
	...
	if (input.taskCategoryId === CATEGORY_REQUIREMENTS)
	...
	const software =
		input.taskCategoryId === CATEGORY_CODING ||
		[
			"next.js",
			"typescript",
			"frontend",
	...
	if (
		input.taskCategoryId === CATEGORY_RESEARCH ||
		input.taskCategoryId === CATEGORY_DOCUMENT ||
		tags.has("research")
	) {
		return [
			{
	...
	return [
		{
			...
			title: "任务执行与交付",
			description:
				"由匹配当前分类和能力要求的 Agent 完成任务并提交可验收制品。",
			categoryId: input.taskCategoryId,
			tags: [...input.taskTags],
			requiredCapability: input.requiredCapability,
			inputContract: "TaskContract",
...
function softwareTemplates(): readonly NodeTemplate[] {
	return [
		...
		},
		{
			...
			title: "Coding 开发实现",
			description:
				"继承用户任务需求与上游设计制品，生成可运行代码、预览与测试依据。",
...
function requirementsTemplate(): NodeTemplate {
	return {
		...
		title: "需求澄清与 PRD",
		description:
			"澄清目标、边界、用户故事和验收标准，输出后续节点可直接消费的任务清单。",
 ...
 */
export function allocateWorkflowBudgetPreference(
	total: bigint,
	weights: readonly number[],
): readonly bigint[] {
	if (total <= 0n)
		throw new Error("workflow budget preference must be positive");
	if (
		weights.length === 0 ||
