...
integration("workflow execution PostgreSQL transaction boundary", () => {
	...
	it("节点派发与结果提交会同步任务主状态，并保持重复回调幂等", async () => {
		...
		expect(state.rows[0]).toEqual({
			...
			status_version: "6",
			event_types:
				"task.assignment_locked,task.agent_accepted,task.results_submitted",
		});

		// 旧版本可能已经持久化节点结果，却没有同步任务投影。运营恢复只在确有分叉时
		// 写入一次修复事件，重复执行不得继续增加版本或制造重复通知。
		await client.query("UPDATE tasks SET status='matching' WHERE id=$1", [
			fixture.taskId,
		]);
		await expect(
			reconcileWorkflowTaskProjection(client, {
	...
	it("完成进度、返工、重新交付和验收，并只解锁依赖已满足的下游节点", async () => {
		...
		const graph = await readOwnedFormalWorkflow(
			client,
			fixture.taskId,
			PUBLISHER,
		);
		const requirementsNode = graph.nodes.find(
			(node) => node.id === fixture.requirementsNodeId,
		);
	...
	it("拒绝乱序和跨内容复用幂等键，同时允许相同结果提交安全重放", async () => {
		...
		await expect(
			repository.submitResults(
				...
				{
					...payload,
					results: [{ ...payload.results[0], content: "被篡改的另一份内容" }],
				},
				"requirements-result-replay",
	...
	it("失败节点持久化并回读校验阶段，重新执行时清空它", async () => {
		...
		await expect(
			transitions.apply(fixture.taskId, fixture.requirementsNodeId, {
				eventId: randomUUID(),
				assignmentId: fixture.assignmentId,
		...
		await expect(
			...
			rows: [{ failure_code: null, failure_stage: null }],
		});
	});
	...
	it("失败节点的新 assignment 从独立进度开始且不会继承旧执行批次", async () => {
		...
		await expect(
			client.query<{ assignment_id: string; progress: number }>(
				`SELECT assignment_id::text,progress FROM workflow_node_execution_state
        WHERE workflow_node_id=$1`,
				[fixture.requirementsNodeId],
			),
		).resolves.toMatchObject({
			rows: [{ assignment_id: replacementAssignmentId, progress: 10 }],
		});
	});
	...
	it("所有阶段验收完成后只创建一笔原子分账，并在链上确认后统一释放与退款", async () => {
		...
		await insertPreAcceptedNode(
			client,
			fixture,
			fixture.designNodeId,
			"DesignArtifact",
		);
		await insertPreAcceptedNode(
			client,
			fixture,
			fixture.codingNodeId,
			"CodeArtifact",
		);
		const submitted = await repository.submitResults(
			fixture.taskId,
		...
		const pending = required(
			(await escrow.listPending(31_337n, ESCROW_CONTRACT, 20)).find(
		...
		const settledGraph = await readOwnedFormalWorkflow(
			client,
			fixture.taskId,
			PUBLISHER,
		);
		expect(settledGraph.nodes).toHaveLength(3);
		expect(settledGraph.nodes.map((node) => node.acceptance?.release)).toEqual([
	...
	await client.query(
       ...
       artifact_kind,body_or_file_ref,mime_type,size_bytes,generated_at
		...
		[
			...
			randomUUID(),
			`${artifactName} 验收制品`,
			JSON.stringify({ artifact: artifactName }),
			Buffer.byteLength(artifactName),
		],
	);
	await client.query(
	...
	return {
		...
		connect: async () => ({
			query: async (text, params) => {
				if (text === "BEGIN")
					return client.query("SAVEPOINT workflow_escrow_nested");
				if (text === "COMMIT")
	...
	const edges =
		graph === "serial"
			? [
					...
					[designNodeId, codingNodeId, "DesignArtifact"],
				]
			: [
					[requirementsNodeId, designNodeId, "RequirementsArtifact"],
		...
		results: [
			{
				...
				content: JSON.stringify({
					...
					problemStatement:
						"团队缺少统一的任务管理入口，需要建立可追踪、可验收的协作工作台。",
					targetUsers: ["项目经理"],
					goals: ["统一管理任务"],
					userStories: [
						{
...
function settlementOf(result: { body: Readonly<Record<string, unknown>> }) {
	...
	if (!isSettlement(settlement))
		throw new Error("WORKFLOW_SETTLEMENT_REQUIRED");
	return settlement;
}
...
function isSettlement(value: unknown): value is Readonly<{
	...
	return (
		typeof value === "object" &&
		value !== null &&
		"grossAmountMinor" in value &&
		typeof value.grossAmountMinor === "string" &&
		"platformFeeMinor" in value &&
		typeof value.platformFeeMinor === "string" &&
		"agentAmountMinor" in value &&
		typeof value.agentAmountMinor === "string" &&
		"feeRuleVersion" in value &&
		typeof value.feeRuleVersion === "string"
	);
}
