...
function fixture() {
	...
	const encoded = daoRewardInterface.encodeEventLog(event, [
		source,
		recipient,
		20n,
		2,
	...
	const rpc = {
		...
		getBlock: vi.fn().mockImplementation(async (number: number) => ({
			number,
			hash,
			timestamp: 100,
		})),
...
describe("奖励确认事件索引", () => {
	it("只读至确认区块，解析固定受益人和奖励类型", async () => {
		const f = fixture();
		expect(await f.chain.scan(0)).toMatchObject({
			to: 9,
			events: [
				{
					type: "allocated",
		...
		expect(f.rpc.getLogs).toHaveBeenCalledWith(
			expect.objectContaining({ address: pool, fromBlock: 0, toBlock: 9 }),
		);
		expect(await f.chain.scan(10)).toBeNull();
		f.rpc.getLogs.mockResolvedValueOnce([]);
		expect(await f.chain.scan(0, 5)).toMatchObject({ to: 5 });
		expect(f.rpc.getLogs).toHaveBeenLastCalledWith(
			expect.objectContaining({ fromBlock: 0, toBlock: 5 }),
		);
		await expect(f.chain.scan(5, 4)).rejects.toThrow(
			"INVALID_REWARD_SCAN_RANGE",
		);
	});
	...
	it("读取期间端点区块变更时拒绝输出通知事件", async () => {
		const f = fixture();
		f.rpc.getBlock
			.mockResolvedValueOnce({ number: 9, hash, timestamp: 100 })
			.mockResolvedValueOnce({ number: 9, hash, timestamp: 100 })
