	...
	return {
		...
		call: vi.fn(
			async (request: { to: string; data: string; blockTag: number }) => {
				...
				if (call.name === "claimable")
					expect(call.args[0].toLowerCase()).toBe(actor);
...
describe("已赚取 YD 奖励读取", () => {
	it("未加入 DAO 也可读取自己的奖励；所有字段固定在同一确认区块", async () => {
		const provider = rpc();
		await expect(
			readDaoRewards(provider, config, actor),
		).resolves.toMatchObject({
			status: "ready",
			actorId: actor,
			poolAddress: pool,
			ydTokenAddress: yd,
			claimableMinor: "20000000000000000000",
			blockNumber: "9",
		});
		expect(
			provider.call.mock.calls.every(([request]) => request.blockTag === 9),
		).toBe(true);
	});
	it("真实零奖励仍返回 ready，不能与尚未部署混为一谈", async () => {
		await expect(readDaoRewards(rpc(0n), config, actor)).resolves.toMatchObject(
			{ status: "ready", claimableMinor: "0" },
		);
	});
	it("拒绝错误网络、错误成员目录与错误 YD，不给浏览器返回危险收款目标", async () => {
		const provider = rpc();
		await expect(
			readDaoRewards(provider, { ...config, chainId: 1n }, actor),
		).rejects.toThrow("REWARD_CHAIN_MISMATCH");
		await expect(
			readDaoRewards(provider, { ...config, membershipAddress: actor }, actor),
		).rejects.toThrow("REWARD_MEMBERSHIP_MISMATCH");
		await expect(
			readDaoRewards(provider, { ...config, ydTokenAddress: actor }, actor),
		).rejects.toThrow("REWARD_TOKEN_MISMATCH");
	});
	it("拒绝读取期间重组及尚不足确认数的链", async () => {
		...
		await expect(readDaoRewards(provider, config, actor)).rejects.toThrow(
			"REWARD_BLOCK_REORG",
		);
		await expect(
			readDaoRewards(rpc(), { ...config, confirmations: 20 }, actor),
		).rejects.toThrow("REWARD_CONFIRMATIONS_PENDING");
	});
});
...
describe("奖励查询 HTTP 权限", () => {
	it("使用签名会话身份，忽略 URL 自报地址，并禁用共享缓存", async () => {
		...
		const handler = createDaoRewardsHandler({
			allowedOrigin: "https://aicp.test",
			resolveActorId: async () => actor,
			read,
		});
		const response = await handler(
			new Request(`https://api.test/dao/rewards?actor=${pool}`),
		);
	...
	it("未登录不查询奖励，RPC 故障不泄漏凭据或伪装成 0 YD", async () => {
		const read = vi.fn(async () => {
			throw new Error("https://rpc.test/secret");
		});
		const forbidden = createDaoRewardsHandler({
			allowedOrigin: "https://aicp.test",
		...
		const failed = createDaoRewardsHandler({
			allowedOrigin: "https://aicp.test",
			resolveActorId: async () => actor,
			read,
		});
		...
		expect(await response.json()).toEqual({
			error_code: "DAO_REWARDS_UNAVAILABLE",
		});
	});
});
