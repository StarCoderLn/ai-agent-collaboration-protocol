...
const addresses = (prefix: number, count: number) =>
	Array.from(
		{ length: count },
...
describe("DAO candidate pool handoff", () => {
	...
	it("社区不足八人时由全部合格创始成员提供后备", () => {
		...
		expect(
			candidatesForSnapshot(snapshot, [...community, ...founding]),
		).toEqual([...founding, ...community].sort());
	});
	...
	it("八至十一名社区成员时最多保留五名创始成员", () => {
		...
		expect(
			candidatesForSnapshot(snapshot, [...founding, ...community]),
		).toEqual([...community, ...founding.slice(0, 5)].sort());
	});
	...
	it("拒绝重复、少于八人或超过上限的创始名单", () => {
		expect(() =>
			normalizeFoundingArbitrators([...founding, founding[0] ?? ""]),
		).toThrow("DAO_FOUNDING_ARBITRATORS_DUPLICATED");
		expect(() => normalizeFoundingArbitrators(founding.slice(0, 7))).toThrow(
			"DAO_FOUNDING_ARBITRATORS_INSUFFICIENT",
		);
		expect(() => normalizeFoundingArbitrators(addresses(1, 65))).toThrow(
			"DAO_FOUNDING_ARBITRATORS_EXCEEDED",
		);
	});
});
