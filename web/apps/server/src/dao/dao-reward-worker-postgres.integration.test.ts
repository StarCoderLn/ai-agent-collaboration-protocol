...
integration("自动 YD 发放与站内通知 PostgreSQL 契约", () => {
	...
	beforeAll(async () => {
		if (url === undefined || new URL(url).pathname === "/aicp_test")
			throw new Error("ISOLATED_REWARD_DATABASE_REQUIRED");
	...
	afterAll(async () => {
		...
		for (const address of createdPools) {
			await pool.query(
				"DELETE FROM audit_logs WHERE target_type='dao_reward_sync_cursor' AND target_id=$1",
				[`31337:${address}`],
			);
			await pool.query(
				"DELETE FROM dao_reward_attempts WHERE reward_id IN(SELECT id FROM dao_reward_transfers WHERE pool_address=$1)",
				[address],
			);
			await pool.query(
				"DELETE FROM dao_reward_transfers WHERE pool_address=$1",
				[address],
			);
			await pool.query(
				"DELETE FROM dao_reward_sync_cursors WHERE pool_address=$1",
				[address],
			);
		}
		await pool.end();
	...
	function fixture() {
		...
		return {
			...
			receipt,
			operator,
			worker: () => new DaoRewardWorker(pool, chain, operator),
			history: () =>
				rewardHistory(asQueryExecutor(pool), "31337", address, actor, 1),
		};
	}
	...
	it("广播响应丢失后重播原交易，只有确认付款事件才产生一次通知，已读受身份保护", async () => {
		...
		await expect(f.worker().run()).resolves.toMatchObject({
			status: "confirming",
			txHash: f.txHash,
		});
		...
		expect(f.broadcast).toHaveBeenNthCalledWith(2, {
			txHash: f.txHash,
		...
		expect(history).toMatchObject({
			unreadCount: 1,
			paidTotalMinor: f.event.amountMinor,
		});
		expect(history.items).toHaveLength(1);
		const id = history.items[0]?.id ?? "";
		await markRewardNotificationsRead(
	...
	it("并发 worker 只有一个签名，失败回执保留历史并按退避时间重试", async () => {
		...
		const attempts = await pool.query(
			"SELECT status FROM dao_reward_attempts WHERE reward_id IN (SELECT id FROM dao_reward_transfers WHERE pool_address=$1) ORDER BY attempt_no",
			[f.address],
		);
	...
	it("完整奖励语义在确认链重现后受控解除游标冻结并记录审计", async () => {
		...
		const restored = {
			...
			timestamp: new Date("2026-09-06T00:01:00Z"),
		};
		f.scan.mockImplementation(async (from, through) =>
			from === 0 && through === 1
		...
		const input = {
			expectedNextBlock: "2",
			expectedLastBlockHash: blockHash,
			resolutionCode: "canonical_history_restored" as const,
		};
		await expect(recovery.resolveCursorReorg(input)).resolves.toEqual({
			status: "resolved",
			nextBlock: "2",
			lastBlockHash: canonicalHash,
			rewards: 1,
		});
		await expect(recovery.resolveCursorReorg(input)).rejects.toMatchObject({
			code: "REWARD_CURSOR_STATE_CHANGED",
		});
		expect(
			(
				await pool.query(
			...
			{ next_block: "2", last_block_hash: canonicalHash, halted: false },
		]);
		expect(
			(
				await pool.query(
	...
	it("确认链奖励金额变化时拒绝解冻且不改写原投影", async () => {
		...
		f.scan.mockImplementation(async () => ({
			to: 1,
			hash: canonicalHash,
			events: [{ ...f.event, amountMinor: "1", blockHash: canonicalHash }],
		}));
		const recovery = new DaoRewardRecoveryService(pool, f.chain);
		await expect(
			recovery.resolveCursorReorg({
				expectedNextBlock: "2",
				expectedLastBlockHash: blockHash,
				resolutionCode: "canonical_history_restored",
			}),
		...
		).toEqual([{ halted: true, last_block_hash: blockHash }]);
		await expect(f.history()).rejects.toThrow("REWARD_SYNC_NEEDS_REVIEW");
	});
	...
	it("广播尚未返回时必须保持专属 operator 锁，不能让另一 worker 同时接管", async () => {
		...
		f.broadcast.mockImplementationOnce(async () => {
			entered();
			await unblock;
			return f.txHash;
		});
