...
export class DaoRewardRecoveryService {
	...
	async resolveCursorReorg(input: ResolveRewardCursorReorgInput) {
		const cursor = await withTransaction(this.pool, (db) =>
			this.loadCursor(db, input),
		);
		const through = cursor.nextBlock - 1;
		if (through < this.chain.startBlock) {
			throw new DaoRewardRecoveryError(
				409,
				"REWARD_CURSOR_HISTORY_INVALID",
				"冻结游标没有可复核的已同步区块",
			);
		}
	...
	private async loadCursor(
		...
		if (row === undefined)
			throw new DaoRewardRecoveryError(
				404,
				"REWARD_CURSOR_NOT_FOUND",
				"奖励同步游标不存在",
			);
		if (
			!row.halted ||
			row.next_block !== input.expectedNextBlock ||
			row.last_block_hash !== input.expectedLastBlockHash.toLowerCase()
		) {
			throw new DaoRewardRecoveryError(
				409,
				"REWARD_CURSOR_STATE_CHANGED",
				"奖励同步游标已经变化，请刷新后重新核对",
			);
		}
		...
		if (!Number.isSafeInteger(nextBlock)) {
			throw new DaoRewardRecoveryError(
				409,
				"REWARD_CURSOR_OVERFLOW",
				"奖励同步游标超出安全读取范围",
			);
		}
		return {
			nextBlock,
			nextBlockText: row.next_block,
			lastBlockHash: row.last_block_hash,
		};
	}

	private async replayCanonicalHistory(through: number): Promise<
		Readonly<{
	...
	> {
		...
		const checkpoints: Array<
			Readonly<{ blockNumber: number; blockHash: string }>
		> = [];
		...
		while (from <= through) {
			...
			if (batch === null || batch.to < from || batch.to > through) {
				throw new DaoRewardRecoveryError(
					409,
					"REWARD_CANONICAL_RANGE_UNAVAILABLE",
					"当前确认链尚不能完整覆盖原同步范围",
				);
			}
			for (const event of batch.events)
				this.collectCanonicalEvent(rewards, event);
		...
		for (const checkpoint of checkpoints) {
			if (
				(await this.chain.blockHash(checkpoint.blockNumber)) !==
				checkpoint.blockHash
			) {
				throw new DaoRewardRecoveryError(
					409,
					"REWARD_RECONCILIATION_REORGED",
					"奖励对账期间确认链再次变化",
				);
			}
		}
	...
	private collectCanonicalEvent(
		rewards: Map<string, CanonicalReward>,
		event: RewardEvent,
	): void {
		...
		if (event.type === "allocated") {
			if (event.kind === null || existing !== undefined) {
				throw new DaoRewardRecoveryError(
					409,
					"REWARD_CANONICAL_HISTORY_AMBIGUOUS",
					"确认链奖励事件存在重复或非法顺序",
				);
			}
	...
	private async resolve(
		...
		if (
			cursor === undefined ||
			!cursor.halted ||
			cursor.next_block !== expected.nextBlockText ||
		...
		for (const row of rows) {
			...
			if (
				event === undefined ||
				event.allocated.amountMinor !== row.amount_minor ||
				event.allocated.kind !== row.reward_kind ||
				(row.status === "paid") !== (event.paid !== undefined) ||
				(event.paid !== undefined &&
					(event.paid.amountMinor !== row.amount_minor ||
						event.paid.txHash !== row.paid_tx_hash))
			) {
				this.mismatch();
			}
			await db.query(
         ...
         WHERE id=$1`,
				[
					row.id,
					event.allocated.blockNumber,
					event.paid?.blockNumber ?? null,
					event.paid?.blockHash ?? null,
					event.paid?.timestamp ?? null,
				],
			);
		}
		await db.query(
		...
		await new PgAuditLogWriter(db).write({
			...
			beforeSummary: {
				nextBlock: expected.nextBlockText,
				lastBlockHash: expected.lastBlockHash,
				halted: true,
			},
			afterSummary: {
				nextBlock: expected.nextBlockText,
				lastBlockHash: canonicalLastHash,
				halted: false,
				rewards: canonical.size,
