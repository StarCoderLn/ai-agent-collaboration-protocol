...
export const confirmEvidenceSchema = z
	.object({
		action: z.literal("confirmEvidence"),
		evidenceId: z.uuid(),
		txHash: z.string().regex(/^0x[0-9a-fA-F]{64}$/),
	})
	.strict();
export const inspectEvidenceSchema = z
	.object({
		action: z.literal("inspectEvidence"),
		evidenceId: z.uuid(),
		txHash: z.string().regex(/^0x[0-9a-fA-F]{64}$/),
	})
	.strict();
...
export async function confirmDaoEvidence(
	...
	const result = await inspectEvidenceAttempt(
		db,
		chain,
		disputeId,
		actorId,
	...
	if (result.status === "pending")
		throw new DaoServiceError(
			409,
			"EVIDENCE_CONFIRMATIONS_PENDING",
			"证据交易仍未达到确认数，请保留原哈希后重试同步",
		);
	if (result.status === "reverted")
		throw new DaoServiceError(
			409,
			"EVIDENCE_TRANSACTION_REVERTED",
			"证据交易已确认回滚，请核对后重新签名",
		);
	throw new DaoServiceError(
		409,
		"EVIDENCE_TRANSACTION_INVALID",
		"交易不属于当前证据、钱包或案件合约",
	);
}
...
async function inspectEvidenceAttempt(
	...
	if (evidence === undefined)
		throw new DaoServiceError(404, "EVIDENCE_NOT_FOUND", "证据不存在");
	...
	if (evidence.anchor_tx_hash !== null) {
		if (evidence.anchor_tx_hash !== txHash)
			throw new DaoServiceError(
				409,
				"EVIDENCE_ALREADY_ANCHORED",
				"证据已有链上承诺，不能替换交易",
			);
		return {
			evidenceId: input.evidenceId,
	...
	const transaction = await chain.inspectEvidenceTransaction(
		txHash,
		evidence.case_key,
		daoEvidenceKey(input.evidenceId),
		actor,
		expected,
	);
	if (transaction.status === "confirmed") {
		if (transaction.contentHash !== expected)
			throw new DaoServiceError(
				409,
				"EVIDENCE_INTEGRITY_MISMATCH",
				"链上承诺与证据正文不一致",
...
export async function prepareDaoCaseAction(
	...
	if (!parsed.success)
		throw new DaoServiceError(422, "VALIDATION_FAILED", "仲裁操作参数不完整");
	const actor = actorId.toLowerCase();
	...
	const stored = await db.query<{
		...
		"SELECT case_key,chain_id::text,contract_address FROM dao_chain_cases WHERE dispute_id=$1",
		[disputeId],
	);
	const row = stored.rows[0];
	if (
		row === undefined ||
		row.chain_id !== chain.chainId.toString() ||
		row.contract_address !== chain.contractAddress
	) {
		throw new DaoServiceError(
			409,
			"DAO_CHAIN_CASE_NOT_AVAILABLE",
			"本案尚未接入当前链上仲裁合约",
		);
	}
	...
	if (input.action === "vote") {
		if (
			...
			BigInt(snapshot.blockTimestamp) >= BigInt(snapshot.deadline)
		)
			throw closed();
		if (!snapshot.panel.includes(actor))
			throw new DaoServiceError(
				403,
				"DAO_VOTE_FORBIDDEN",
				"当前钱包不在本轮仲裁小组中",
			);
		if (await chain.hasVoted(row.case_key, snapshot.round, actor))
			throw new DaoServiceError(409, "DAO_ALREADY_VOTED", "当前钱包已经投票");
       ...
       ON CONFLICT DO NOTHING`,
			[disputeId, snapshot.round, actor, reasoningHash, input.reasoning],
		);
		data = daoCaseInterface.encodeFunctionData("vote", [
			row.case_key,
			input.releaseBasisPoints,
			reasoningHash,
		]);
	} else if (input.action === "appeal") {
		if (
			snapshot.status !== "appeal_window" ||
			BigInt(snapshot.blockTimestamp) >= BigInt(snapshot.deadline)
		)
			throw closed();
       ...
       lower(task.publisher_id)=$2 OR EXISTS(SELECT 1 FROM task_assignments assignment JOIN agents agent ON agent.id=assignment.agent_id
         WHERE assignment.task_id=task.id AND assignment.status='accepted' AND lower(agent.provider_wallet_address)=$2))`,
			[disputeId, actor],
		);
		if (party.rows.length === 0)
			throw new DaoServiceError(
				403,
				"DAO_APPEAL_FORBIDDEN",
				"只有本案当事人可以申诉",
			);
		if (snapshot.bondPolicy === 0)
			throw new DaoServiceError(
				409,
				"DAO_TERMS_NOT_CONFIGURED",
				"申诉费用与保证金规则尚未配置",
			);
		approvalAmountMinor = (
			BigInt(snapshot.appealBondMinor) + BigInt(snapshot.appealFeeMinor)
		).toString();
	...
	return {
		...
		action: input.action,
		approvalAmountMinor,
		appealBondMinor: snapshot.appealBondMinor,
		appealFeeMinor: snapshot.appealFeeMinor,
		bondPolicy: snapshot.bondPolicy,
		viewerRole: access.body.viewerRole,
...
function closed() {
	return new DaoServiceError(
		409,
		"DAO_CASE_ACTION_CLOSED",
		"本案阶段已变化或操作期限已结束，请刷新案件",
	);
}
