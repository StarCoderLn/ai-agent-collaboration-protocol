...
import {
	type DaoRpcProvider,
	EthersDaoMembershipChainClient,
...
describe("DAO 链上成员资格核验", () => {
	...
	it("只接受目标合约中属于当前登录钱包的已确认成员事件", async () => {
		...
		const client = new EthersDaoMembershipChainClient(
			provider,
			31_337n,
			DAO,
			YD,
		...
		await expect(
			client.verifyMembershipTransaction(TX_HASH, MEMBER),
	...
	it("拒绝借用其他钱包的质押交易获取仲裁资格", async () => {
		...
		await expect(
			client.verifyMembershipTransaction(TX_HASH, MEMBER),
		).rejects.toThrow("DAO_MEMBERSHIP_EVENT_NOT_FOUND");
	});
});
	...
	return {
		...
		call: async ({ data }) => {
			...
			if (selector === daoInterface.getFunction("membershipOf")?.selector) {
				return daoInterface.encodeFunctionResult("membershipOf", [
					[1_000n * 10n ** 18n, 0],
				]);
			}
			...
			if (selector === daoInterface.getFunction("minimumStake")?.selector) {
				return daoInterface.encodeFunctionResult("minimumStake", [
					1_000n * 10n ** 18n,
				]);
			}
