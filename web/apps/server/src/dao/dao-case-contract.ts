...
export function daoEvidenceKey(id: string): string {
	...
	return keccak256(
		toUtf8Bytes(`aicp:arbitration:evidence:v1:${id.toLowerCase()}`),
	);
}
...
export function evidenceContentHash(
	...
	return keccak256(
		AbiCoder.defaultAbiCoder().encode(
			...
			[
				"aicp-evidence-text-and-references-v1",
				daoCaseKey(input.disputeId),
				daoEvidenceKey(input.evidenceId),
				submitter,
				keccak256(toUtf8Bytes(input.description)),
				keccak256(toUtf8Bytes(canonicalJson(input.attachments))),
			],
		),
	);
}
