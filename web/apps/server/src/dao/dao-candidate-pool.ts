...
const candidatePoolSnapshotSchema = z
	.object({
 ...
 */
export function normalizeFoundingArbitrators(
	values: readonly string[],
): readonly string[] {
	const normalized = values.map((value) => getAddress(value).toLowerCase());
	const unique = [...new Set(normalized)].sort();
	if (unique.length !== normalized.length)
		throw new Error("DAO_FOUNDING_ARBITRATORS_DUPLICATED");
	if (unique.length !== 0 && unique.length < 8)
		throw new Error("DAO_FOUNDING_ARBITRATORS_INSUFFICIENT");
...
export function createCandidatePoolSnapshot(
	...
	const communityEligibleAtOpen = new Set(
		eligibleMembers
			...
			.filter((value) => !foundingSet.has(value)),
	).size;
	const phase =
		founding.length === 0 ||
		communityEligibleAtOpen >= COMMUNITY_HANDOFF_THRESHOLD
			? "community"
			: communityEligibleAtOpen >= FOUNDING_MIXED_THRESHOLD
				? "mixed"
				: "bootstrap";
	return {
 ...
 */
export function candidatesForSnapshot(
	raw: unknown,
	eligibleMembers: readonly string[],
): readonly string[] {
	...
	const normalized = [
		...new Set(eligibleMembers.map((value) => getAddress(value).toLowerCase())),
	].sort();
...
export function parseCandidatePoolSnapshot(
	raw: unknown,
): CandidatePoolSnapshot {
