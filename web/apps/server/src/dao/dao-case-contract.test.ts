...
describe("链上案件契约", () => {
	it("JSONB 字段顺序变化不改变承诺，正文、身份、附件引用变化会改变承诺", () => {
		const original = {
			...
			submitter: actor,
			description: "原始证据",
			attachments: [{ name: "a.txt", storageRef: "private:1" }],
		};
		const baseline = evidenceContentHash(original);
		expect(
			evidenceContentHash({
				...original,
				attachments: [{ storageRef: "private:1", name: "a.txt" }],
			}),
		).toBe(baseline);
		expect(
			evidenceContentHash({ ...original, description: "替换证据" }),
		).not.toBe(baseline);
		expect(
			evidenceContentHash({ ...original, submitter: `0x${"33".repeat(20)}` }),
		).not.toBe(baseline);
		expect(
			evidenceContentHash({
				...original,
				attachments: [{ name: "a.txt", storageRef: "private:2" }],
			}),
		).not.toBe(baseline);
	...
	it("开案 ABI 包含独立的当事人和利益冲突名单", () => {
		const data = daoCaseInterface.encodeFunctionData("openCase", [
			hash,
			hash,
			[actor],
			[actor],
	...
	it.each(["awaiting_randomness", "stalled", "final", "none"] as const)(
		"%s 状态不得生成随机重试或退款",
		(status) => {
	...
	it("只有新版恢复状态到达硬截止后才执行公开兜底", () => {
		expect(
			nextCaseAction(
				snapshot({ status: "recovery", blockTimestamp: "99", deadline: "100" }),
			),
		).toBeNull();
		expect(
			nextCaseAction(
