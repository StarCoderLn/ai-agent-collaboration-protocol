...
export type DaoMembershipAction =
	| "stake"
	| "request_exit"
	| "cancel_exit"
	| "withdraw";
...
export interface DaoRpcProvider {
	...
	call(
		transaction: Readonly<{ to: string; data: string }>,
		blockTag?: number,
	): Promise<string>;
}
...
export interface DaoMembershipChainClient {
	...
	verifyMembershipTransaction(
		txHash: string,
		actorId: string,
	): Promise<DaoMembershipSnapshot>;
}
 ...
 */
export class EthersDaoMembershipChainClient
	implements DaoMembershipChainClient
{
	readonly contractAddress: string;
	readonly ydTokenAddress: string;
	...
	constructor(
		...
		if (chainId <= 0n || requiredConfirmations <= 0n)
			throw new Error("INVALID_DAO_CHAIN_CONFIG");
	}

	async verifyMembershipTransaction(
		txHash: string,
		actorId: string,
	): Promise<DaoMembershipSnapshot> {
		...
		if (
			receipt.to === null ||
			normalizedAddress(receipt.to) !== this.contractAddress
		) {
		...
		if (confirmations < this.requiredConfirmations)
			throw new Error("DAO_TRANSACTION_NOT_CONFIRMED");
		const action = membershipActionFromReceipt(
			receipt.logs,
		...
		if (!tupleHasTwoValues(membership))
			throw new Error("DAO_MEMBERSHIP_RESPONSE_INVALID");
		const exitTimestamp = BigInt(String(membership[1]));
		const resolvedYdToken = normalizedAddress(String(ydTokenResult[0]));
		if (resolvedYdToken !== this.ydTokenAddress)
			throw new Error("DAO_YD_TOKEN_MISMATCH");
		return {
			...
			exitAvailableAt:
				exitTimestamp === 0n ? null : unixSecondsToDate(exitTimestamp),
...
export function createJsonRpcDaoMembershipClient(
	input: Readonly<{
		rpcUrl: string;
		chainId: bigint;
		contractAddress: string;
	...
	return new EthersDaoMembershipChainClient(
		new JsonRpcProvider(input.rpcUrl, Number(input.chainId), {
			staticNetwork: true,
		}),
		input.chainId,
...
function membershipActionFromReceipt(
	...
	for (const log of logs) {
		...
		if (
			parsed === null ||
			normalizedAddress(String(parsed.args[0])) !== actorId
		)
			continue;
...
function tupleHasTwoValues(
	value: unknown,
): value is Readonly<{ 0: unknown; 1: unknown }> {
	return (
		typeof value === "object" && value !== null && 0 in value && 1 in value
	);
}

function unixSecondsToDate(value: bigint): Date {
	if (value > BigInt(Math.floor(8_640_000_000_000_000 / 1_000)))
		throw new Error("DAO_EXIT_TIME_INVALID");
