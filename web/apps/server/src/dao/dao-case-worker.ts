...
export class DaoCaseWorker {
	...
	async run(
		...
		Readonly<{ synchronized: number; submitted: number; failed: number }>
	> {
		if (!Number.isSafeInteger(limit) || limit < 1 || limit > 50)
			throw new Error("INVALID_DAO_WORKER_LIMIT");
		const rows = await withTransaction(this.pool, (db) =>
			db.query<{
				dispute_id: string;
				case_key: string;
				parties: unknown;
				candidate_pool_policy: unknown;
			}>(
		...
		for (const row of rows.rows) {
			...
			try {
				...
				if (action === "requestPanel") {
					...
					const excluded = [
						...parties,
						...(snapshot.round === 2 ? snapshot.firstPanel : []),
					];
					const policy = parseCandidatePoolSnapshot(row.candidate_pool_policy);
					const members = await withTransaction(this.pool, (db) =>
						db.query<{ actor_id: string }>(
							`SELECT actor_id FROM dao_memberships WHERE chain_id=$1 AND eligible=TRUE
             AND NOT (actor_id=ANY($2::text[])) ORDER BY actor_id LIMIT 257`,
							[this.chain.chainId.toString(), excluded],
						),
					);
					...
					const eligibilityInput =
						policy.phase === "community"
							? candidatesForSnapshot(policy, stored)
							: stored;
					candidates = candidatesForSnapshot(
						policy,
						await this.chain.eligibleMembers(eligibilityInput),
					);
				}
				await withTransaction(this.pool, async (db) => {
					...
					if (
						action === "requestPanel" &&
						candidates.length < (snapshot.round === 1 ? 3 : 5)
					) {
						await db.query(
							"UPDATE dao_chain_cases SET last_error_code='DAO_PANEL_INSUFFICIENT' WHERE dispute_id=$1",
							[row.dispute_id],
						);
						return;
					}
					await enqueueCaseCommand(
						db,
						row.dispute_id,
						`${snapshot.round}:${action}`,
						daoCaseInterface.encodeFunctionData(
							action,
							action === "requestPanel"
								? [row.case_key, candidates]
								: [row.case_key],
						),
					);
				});
				...
				await withTransaction(this.pool, (db) =>
					markFailure(db, row.dispute_id, daoCaseWorkerErrorCode(error)),
				);
			}
		}
		// 同一 operator 共用 nonce。确认快照不可用时，本轮不能在旧状态上继续签名或重播；
		// 下一轮重新读链成功后自动恢复，已持久化的签名和命令保持原样。
		const submitted = chainReadFailed
			? 0
			: await this.advanceCommands(limit, blockedCases);
	 ...
	 */
	private async advanceCommands(
		limit: number,
		blockedCases: ReadonlySet<string>,
	): Promise<number> {
		...
		try {
         ...
         ORDER BY command.created_at,command.id LIMIT $3`,
				[this.chain.chainId.toString(), this.chain.contractAddress, limit],
			);
			for (const command of commands.rows) {
				...
				try {
					...
					if (command.raw_transaction !== null && command.tx_hash !== null) {
						prepared = {
							rawTransaction: command.raw_transaction,
							txHash: command.tx_hash,
						};
					} else {
						if (Number(command.attempts) >= 5)
							throw new Error("DAO_COMMAND_ATTEMPTS_EXHAUSTED");
						prepared = await this.operator.prepareContractCall(
					...
					if (hash.toLowerCase() !== prepared.txHash.toLowerCase())
						throw new Error("DAO_BROADCAST_HASH_MISMATCH");
...
async function persistPreparedAttempt(
	db: QueryExecutor,
	command: CommandRow,
	prepared: PreparedOperatorTransaction,
): Promise<void> {
	const attemptNo = Number(command.attempts) + 1;
	if (!Number.isSafeInteger(attemptNo) || attemptNo < 1 || attemptNo > 5)
		throw new Error("DAO_COMMAND_ATTEMPTS_EXHAUSTED");
...
async function recordCommandSubmitted(
	db: QueryExecutor,
	commandId: string,
	txHash: string,
): Promise<void> {
	...
	try {
		...
		await db.query(
			"UPDATE dao_case_commands SET status='submitted',error_code=NULL,updated_at=now() WHERE id=$1",
			[commandId],
		);
		await db.query("COMMIT", []);
	} catch (error) {
...
const insufficientPoolSelector = id("InsufficientPool()")
	.slice(0, 10)
	.toLowerCase();
...
export function daoCaseWorkerErrorCode(error: unknown): string {
	if (typeof error === "object" && error !== null) {
		if (readString(error, "code") === "INSUFFICIENT_FUNDS")
			return "DAO_CASE_OPERATOR_INSUFFICIENT_FUNDS";
		const candidates = [
			readString(error, "message"),
			readString(error, "shortMessage"),
			readString(error, "reason"),
			readNestedString(error, "revert", "name"),
			readNestedString(error, "info", "error", "message"),
			readNestedString(error, "error", "message"),
			readString(error, "data"),
			readNestedString(error, "info", "error", "data"),
			readNestedString(error, "error", "data"),
		];
		if (
			candidates.some(
				(value) =>
					value?.includes("InsufficientPool") ||
					value?.toLowerCase().startsWith(insufficientPoolSelector),
			)
		) {
		...
		if (
			candidates.some((value) =>
				value?.toLowerCase().includes("insufficient funds"),
			)
		) {
			return "DAO_CASE_OPERATOR_INSUFFICIENT_FUNDS";
		}
...
async function markFailure(
	...
	await db.query(
		"UPDATE dao_chain_cases SET last_error_code=$2,updated_at=now() WHERE dispute_id=$1",
		[disputeId, code],
	);
