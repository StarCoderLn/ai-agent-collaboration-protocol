...
describe("application locale boundary", () => {
	...
	it("defaults international traffic to English and Chinese browsers to Chinese", () => {
		...
		expect(resolveAppLocale(undefined, "zh-TW,zh;q=0.9,en;q=0.8")).toBe("zh-CN");
	});

	it("keeps Chinese ids readable and interpolates both catalogs", () => {
		expect(translate("zh-CN", "打开钱包账户菜单 {address}", { address: "0x1234" })).toBe("打开钱包账户菜单 0x1234");
		expect(translate("en", "打开钱包账户菜单 {address}", { address: "0x1234" })).toBe("Open wallet account menu 0x1234");
	});
});
