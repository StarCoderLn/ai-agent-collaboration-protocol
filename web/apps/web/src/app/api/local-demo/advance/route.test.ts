...
describe("local demo infrastructure advance", () => {
	...
	it("mines confirmations then asks the authoritative escrow worker to sync", async () => {
		...
		const fetcher = vi
			...
			.mockResolvedValueOnce(
				Response.json({ processed: 1, ranAt: "2026-08-23T00:00:00.000Z" }),
			);
		vi.stubGlobal("fetch", fetcher);
		const response = await POST(request("confirm-deposit"));
		...
		expect(fetcher).toHaveBeenNthCalledWith(
			2,
			"http://127.0.0.1:3000/api/internal/workers/escrow-sync",
			expect.objectContaining({ method: "POST" }),
		);
	});
});

function request(operation: "confirm-deposit" | "confirm-settlement") {
	return new Request("http://localhost:3001/api/local-demo/advance", {
		method: "POST",
		headers: {
...
function localEnvironment() {
	...
	process.env.LOCAL_DEMO_MARKETPLACE_API_URL = "http://127.0.0.1:3000";
	process.env.LOCAL_DEMO_ETHEREUM_RPC_URL = "http://127.0.0.1:8545";
	process.env.LOCAL_DEMO_MINE_BLOCKS = "2";
	process.env.DISPATCH_INTERNAL_TOKEN = "local-internal-token-value";
}
