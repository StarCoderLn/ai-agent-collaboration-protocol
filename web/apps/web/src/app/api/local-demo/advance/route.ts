...
const InputSchema = z
	.object({ operation: z.enum(["confirm-deposit", "confirm-settlement"]) })
	.strict();
...
export async function POST(request: Request): Promise<Response> {
	if (
		...
		return notFound();
	if (request.headers.get("x-aicp-local-demo") !== "advance")
		return Response.json(
			{ error: "local demo confirmation header is missing" },
			{ status: 403 },
		);
	...
	try {
		...
		return Response.json(
			{ error: "invalid local demo operation" },
			{ status: 422 },
		);
	}
	try {
		const businessBase = loopbackBase(
			required("LOCAL_DEMO_MARKETPLACE_API_URL"),
			"LOCAL_DEMO_MARKETPLACE_API_URL",
		);
		const rpcUrl = loopbackBase(
			required("LOCAL_DEMO_ETHEREUM_RPC_URL"),
			"LOCAL_DEMO_ETHEREUM_RPC_URL",
		);
		...
		return Response.json(
			{
				error:
					error instanceof Error ? error.message : "local demo advance failed",
			},
			{ status: 502 },
		);
	}
...
async function runWorker(
	...
	if (!response.ok)
		throw new Error(`local worker ${path} returned HTTP ${response.status}`);
}
...
async function mineBlocks(rpcUrl: string, blocks: number): Promise<void> {
	...
	if (!response.ok)
		throw new Error(`local Ethereum RPC returned HTTP ${response.status}`);
}
...
function loopbackBase(value: string, field: string): string {
	...
	if (
		...
		throw new Error(`${field} must use a loopback URL`);
	if (
		url.username !== "" ||
		url.password !== "" ||
		url.search !== "" ||
