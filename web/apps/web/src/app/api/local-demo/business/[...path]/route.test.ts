...
describe("local Business API same-origin proxy", () => {
	...
	it("is unavailable outside explicit local demo mode", async () => {
		...
		const response = await GET(request("GET", "/api/local-demo/business/market/agents"), context("market", "agents"));
	...
	it("forwards query, auth continuity, idempotency and Set-Cookie without internal credentials", async () => {
		...
		const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify({ ok: true }), {
			status: 201,
			headers: { "content-type": "application/json", "set-cookie": "aicp_session=signed; Path=/; HttpOnly; SameSite=Lax" },
		}));
		vi.stubGlobal("fetch", fetcher);
		const response = await POST(request("POST", "/api/local-demo/business/tasks?preview=true", {
			cookie: "aicp_session=old", "content-type": "application/json", "idempotency-key": "create:task:request-1",
		}, "{\"title\":\"任务\"}"), context("tasks"));
		...
		expect(fetcher).toHaveBeenCalledWith(
			...
			expect.objectContaining({ method: "POST", body: expect.any(ArrayBuffer), redirect: "manual" }),
		);
...
function localEnvironment() {
	...
	process.env.LOCAL_DEMO_BUSINESS_API_URL = "http://127.0.0.1:3100";
}
...
function request(method: string, path: string, headers: Record<string, string> = {}, body?: string) {
	return new Request(`http://127.0.0.1:3301${path}`, { method, headers, ...(body === undefined ? {} : { body }) });
}
