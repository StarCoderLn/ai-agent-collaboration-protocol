...
export default function AgentReviewConsole() {
		...
		(signal?: AbortSignal) => {
			...
			listReviewAgents(filter, signal)
				...
				.catch((error: unknown) => {
					...
					setState(
						error instanceof AgentDirectoryRequestError && error.status === 403
	...
	async function decide(agentId: string, decision: "approve" | "reject") {
		...
		if (reviewReason.length < 4) {
			setMessage(
				t("请填写至少 4 个字符的审核理由，方便提供者理解决定并保留审计依据。"),
			);
			return;
		}
		setMessage(null);
		...
		try {
			...
			setMessage(
				decision === "approve"
					? t("审核通过，Agent 已进入可接单状态")
					: t("已驳回，Agent 需要修正后重新注册"),
			);
			setReviewReasons((current) => ({ ...current, [agentId]: "" }));
			await listReviewAgents(filter).then((agents) =>
		...
		return (
			<ReviewState
				...
				description={
					wallet.status === "error"
						? wallet.error
						: t("审核队列只对具有 agent_reviewer 角色的钱包开放。")
				}
				action={
					<Button
	...
	return (
		<div>
			...
			>
				{reviewStatuses.map((status) => (
					<button
						...
						className={`min-h-10 rounded-lg border px-4 font-medium text-sm ${filter === status ? "border-primary/25 bg-primary-container text-primary" : "bg-card text-muted-foreground"}`}
					>
			...
			{state.kind === "idle" || state.kind === "loading" ? (
				...
				<ReviewState
					icon={SearchCheck}
					title={t("没有 {status} 的 Agent", {
						status: statusLabel(filter, t),
					})}
					description={t("切换上方状态可以查看其他 Agent。")}
				/>
			) : (
				<div className="grid gap-4">
					{state.agents.map((agent) => (
						<ReviewCard
							key={agent.id}
							agent={agent}
							reviewReason={reviewReasons[agent.id] ?? ""}
							...
							onApprove={() => decide(agent.id, "approve")}
							onReject={() => decide(agent.id, "reject")}
						/>
					))}
				</div>
			)}
			...
			<div className="flex flex-wrap items-start justify-between gap-4">
				<div className="min-w-0">
					...
					<p className="mt-1 text-muted-foreground text-sm">
						...
						·{" "}
						{t("提交于 {date}", { date: formatDate(agent.createdAt, locale) })}
					</p>
				</div>
				<p className="font-mono text-muted-foreground text-xs">{agent.id}</p>
			</div>
			<p className="mt-4 whitespace-pre-wrap text-muted-foreground text-sm leading-6">
				{agent.description}
			</p>
			<div className="mt-4 grid gap-3 rounded-lg bg-accent p-4 text-xs sm:grid-cols-2">
				<ReviewFact
					label={t("提供者钱包")}
					value={agent.providerWalletAddress}
				/>
				<ReviewFact label={t("服务地址")} value={agent.serviceEndpoint} />
			</div>
			{agent.status === "pending_review" && (
				<div className="mt-4 border-t pt-4">
					<label
						htmlFor={`review-reason-${agent.id}`}
					...
					<Textarea
						id={`review-reason-${agent.id}`}
						value={reviewReason}
						onChange={(event) => onReviewReasonChange(event.target.value)}
						placeholder={t("例如：服务地址可访问，能力与报价说明一致。")}
						className="mt-3 min-h-24 rounded-lg"
						maxLength={1000}
					/>
					<div className="mt-3 flex flex-wrap gap-2">
						<Button onClick={onApprove} disabled={decisionDisabled}>
							{busy ? (
								<Loader2 className="size-4 animate-spin" />
							) : (
...
function ReviewFact({ label, value }: { label: string; value: string }) {
	return (
		<div className="min-w-0">
			...
			<p className="mt-1 truncate font-mono" title={value}>
				{value}
			</p>
		</div>
	);
