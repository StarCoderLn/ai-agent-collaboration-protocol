...
function ReviewSkeleton() { const { t } = useLocale(); return <div className="grid gap-4" role="status" aria-label={t("正在加载 Agent 审核列表")}>{[0, 1].map((item) => <div key={item} className="rounded-xl border bg-card p-5"><Skeleton className="h-6 w-1/3" /><Skeleton className="mt-3 h-4 w-2/3" /><Skeleton className="mt-6 h-20 w-full" /></div>)}</div>; }
