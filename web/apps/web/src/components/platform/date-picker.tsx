	...
	return (
		<div ref={rootRef} className="relative">
			<button
				...
				disabled={disabled}
				onClick={openPicker}
				className={`group flex h-11 w-full cursor-pointer items-center justify-between gap-3 rounded-lg border bg-background/45 px-3 text-left text-sm outline-none transition-[border-color,box-shadow,background-color] hover:border-primary/45 hover:bg-primary-container/20 focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-50 ${open ? "border-primary bg-primary-container/25 shadow-[0_0_22px_var(--brand-glow)]" : "border-input"}`}
			>
				<span
					className={
			...
			{open &&
				...
				>
					<div className="relative border-primary/15 border-b px-4 py-3">
						...
						<div className="flex items-center justify-between gap-3">
							...
							<div className="text-center">
								...
								<p className="mt-0.5 flex items-center justify-center gap-1 text-primary text-[10px] uppercase tracking-[0.18em]">
					...
					<div className="p-4">
						<div className="grid grid-cols-7 gap-1" aria-hidden>
							{weekdayLabels.map((weekday, index) => (
								<span
									key={`${weekday}-${index}`}
									className="flex h-7 items-center justify-center font-medium text-muted-foreground text-[11px]"
								>
									{weekday}
								</span>
							))}
					...
					<div className="flex items-center justify-between gap-3 border-primary/15 border-t bg-background/25 px-4 py-3">
						<p className="max-w-40 text-muted-foreground text-[11px] leading-4">
							{t("所选日期当天结束前为交付截止时间")}
						</p>
						<div className="flex gap-2">
							...
							<button
								...
								className="flex h-9 cursor-pointer items-center gap-1.5 rounded-lg bg-primary px-3 font-semibold text-primary-foreground text-xs shadow-[0_0_18px_var(--brand-glow)] transition-[filter,box-shadow] hover:brightness-110 hover:shadow-[0_0_26px_var(--brand-glow-strong)] disabled:cursor-not-allowed disabled:opacity-40"
							>
